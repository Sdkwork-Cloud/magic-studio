...
import { CreationChatInput, InputFooterButton } from '../../../components/CreationChatInput';
import { PortalTab, InputAttachment } from '../../../components/CreationChatInput/types';
...
import { useFilmStore, FilmStoreProvider } from '../../film/store/filmStore';
...
import { ModelProvider } from '../../../components/ModelSelector/types'; // Import type
...
const ACTIVE_USERS = [
    { name: '风魔幼', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=feng' },
    { name: '小猫教英文', avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=cat_eng' },
...
const PortalContent: React.FC = () => {
    ...
    const getGenerationType = (tab: PortalTab): GenerationType => {
        switch (tab) {
            ...
            // 'one_click' usually maps to FILM or VIDEO auto mode, let's default to FILM
    ...
    useEffect(() => {
        const loadModels = async () => {
            ...
            if (result.success && result.data) {
                // Map Service Response to UI Component Format
                const mappedProviders: ModelProvider[] = result.data.channels.map((ch, idx) => {
                    ...
                    return {
                        ...
                        name: ch.name,
                        icon: <IconComp size={14} />, // Instantiate the component here
                        color: ch.color,
                        models: ch.models.map(m => ({
                            id: m.model, // Use the API model string as ID
                            name: m.model.split('-').slice(0, 2).join(' '), // Simplify name
    ...
    // Smart Mode Switching based on Attachments
    useEffect(() => {
    ...
    useEffect(() => {
        // Reset gen mode to default when tab changes if current is invalid
    ...
    const handleUpload = async (source: 'local' | 'asset') => {
        if (source === 'local') {
            try {
                // Base types supported everywhere
                let accept = 'image/*, video/*, audio/*';
                
                // Extended text support for Short Drama mode
                if (activeTab === 'short_drama') {
    ...
    // Function to handle inserting quotes
    ...
    const renderFooterControls = () => {
        // Icon logic for Gen Mode button
        const GenModeIcon = currentGenMode.icon;

        return (
            <div className="flex items-center gap-2 whitespace-nowrap">
                
                {/* 0. Main App Mode Selector (Replacement for Top Tabs) */}
                ...
                {/* 2. Generation Mode Selector (NEW) */}
                        ...
                        <div className="w-full mt-20 mb-8 text-center space-y-8">
                            ...
                            <div className="max-w-[1200px] mx-auto w-full" ref={heroSectionRef}>
                                <CreationChatInput 
                                    ...
                                    // Decreased height for better UX
                                    minHeight={88}
