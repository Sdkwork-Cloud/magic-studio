...
import { PromptText } from '../../../components/generate/PromptText';
...
export const GenerationPreview: React.FC<GenerationPreviewProps> = ({ 
    ...
    // If creation mode, we might have multiple results in one task.
    // For view mode, usually 1 item = 1 result. 
    // We simplify by treating creation mode tasks as having 1 main result for now (or handle sub-index).
    // Let's keep `activeIndex` for Creation Mode batch results.
                ...
                <div className="flex-1 overflow-y-auto p-2 xl:p-3 space-y-2 custom-scrollbar">
                    {items.map(item => {
                        ...
                        if (mode === 'creation') {
                            ...
                            thumb = g.url; // Use main image as thumb
                            title = g.title;
                        }
                        ...
                        return (
                            ...
                            >
                                <div className="w-10 h-10 xl:w-12 xl:h-12 rounded-lg bg-black flex-shrink-0 overflow-hidden border border-white/10 relative">
                                    <img src={thumb} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" loading="lazy" />
                                </div>
