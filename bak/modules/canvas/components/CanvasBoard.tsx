...
import { QuadTree, Rect } from '../../../utils/algorithms/QuadTree';
...
export const CanvasBoard: React.FC = () => {
    ...
    const spatialIndex = useMemo(function() {
        ...
        elements.forEach(function(el) { tree.insert(el); });
        return tree;
    }, [elements]); 
    ...
    const { visibleGroups, visibleNodes } = useMemo(() => {
        ...
        const visibleRaw = spatialIndex.retrieve([], queryRect);
            ...
            if (dragRef.current.mode === 'pan') {
                ...
                if (useCanvasStore.getState().selectedIds.size === 1) {
                     ...
                     if (startPos) {
                         ...
                         const rawCandidates2 = spatialIndex.retrieve([], visibleRect);
        ...
        const handleWindowMouseUp = (e: MouseEvent) => {
            ...
            if (dragRef.current.isDragging) {
                     ...
                     if (dragRef.current.pendingSelectionId) {
                         ...
                         if (useCanvasStore.getState().selectedIds.size === 1) {
                             ...
                             if (startPos) {
                                 ...
                                 const visibleRect = { x: -viewport.x / viewport.zoom, y: -viewport.y / viewport.zoom, width: window.innerWidth / viewport.zoom, height: window.innerHeight / viewport.zoom };
                                 const movingIds = new Set(dragRef.current.initialPositions.keys());
                                 const rawCandidates = spatialIndex.retrieve([], visibleRect);
                ...
                if (dragRef.current.mode === 'rubber-band' && marquee) {
                    ...
                    const hitElements = spatialIndex.retrieve([], { x, y, width, height });
                    const hitIds: string[] = [];
