...
import { 
    ...
    ChevronDown, ArrowRightLeft, Layers, ScanFace, Sparkles, Check, Monitor, Smartphone, Square
} from 'lucide-react';
...
import { textRenderer, TextStyle } from '../../magic-cut/engine/text/TextRenderer';
...
import { ImageModelSelector } from '../../image/components/ImageModelSelector';
import { VideoModelSelector } from '../../video/components/VideoModelSelector';
...
export const CanvasNode = React.memo(forwardRef<HTMLDivElement, CanvasNodeProps>(({ 
    ...
    const handleUploadClick = async (e?: React.MouseEvent) => {
        ...
        try {
            ...
            if (files.length > 0) {
                const file = files[0];
                const savedAsset = await assetService.importAsset(file.data, file.name, type as AssetType);
                ...
                <div className="min-w-[120px]">
                    {type === 'video' ? (
                        <VideoModelSelector 
                            ...
                            onChange={(m) => updateElement(id, { data: { ...data, model: m } })} 
                ...
                <div className="flex items-center gap-1">
                    {type === 'video' ? (
                        <VideoModeDropdown 
                            ...
                            onChange={(m) => updateElement(id, { data: { ...data, videoMode: m } })} 
                        />
                    ) : (
                        <AspectRatioSelector 
                            value={(data?.aspectRatio as any) || '1:1'}
                            onChange={(r) => updateElement(id, { data: { ...data, aspectRatio: r as any } })}
                            resolution={(data?.resolution as any) || '2k'}
                            onResolutionChange={(r) => updateElement(id, { data: { ...data, resolution: r as any } })}
    ...
    return (
        <div
            ref={internalRef}
            className={`absolute group touch-none select-none pointer-events-auto ease-out ${isResizing ? 'will-change-[width,height] transition-none' : 'transition-[width,height,box-shadow,transform] duration-300 cubic-bezier(0.16, 1, 0.3, 1)'}`}
        ...
        >
            {lod === 'detail' && (
                <>
                    {selected && !isResizing && selectionCount === 1 && !isMedia && (
                        ...
                        >
                            <div className="w-[480px]">
                                <CreationChatInput 
                                    variant="compact"
                                    value={data?.prompt || ''}
                                    onChange={(val) => updateElement(id, { data: { ...data, prompt: val } }, false)}
