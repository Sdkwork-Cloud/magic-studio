...
import { vfs } from '../../../modules/fs/vfs';
import { pathUtils } from '../../../utils/pathUtils';
import { platform } from '../../../platform';
import { generateUUID } from '../../../utils';
import { storageConfig, LIBRARY_SUBDIRS } from '../../../modules/fs/storageConfig';
import { IBaseService, ServiceResult, Result, Page, PageRequest } from '../../../types/core';
import { MediaResource, MediaResourceType, AnyMediaResource } from '../../../types';
import { mediaAnalysisService } from '../../../services/media/mediaAnalysisService';
...
class AssetService implements IBaseService<Asset> {
    ...
    public async importAsset(
        ...
        // This ensures subsequent analysis uses a browser-friendly URL (blob: or asset://)
        const viewableUrl = await this.resolveAssetUrl({ path: virtualPath });
        
        console.log(`[AssetService] Analyzing media: ${originalName} (${type})`);
        
        const { metadata, thumbnailBlob } = await mediaAnalysisService.analyze(viewableUrl, type);
        ...
        // Pre-cache the viewable URL to avoid re-resolution flicker
    ...
    public toMediaResource(asset: Asset): AnyMediaResource { const typeMap: Record<string, MediaResourceType> = { 'image': MediaResourceType.IMAGE, 'video': MediaResourceType.VIDEO, 'audio': MediaResourceType.AUDIO, 'music': MediaResourceType.MUSIC, 'voice': MediaResourceType.VOICE, 'text': MediaResourceType.TEXT }; const mediaType = typeMap[asset.type] || MediaResourceType.FILE; const effectiveMetadata: any = { ...asset.metadata }; if (effectiveMetadata.thumbnailPath && !effectiveMetadata.thumbnailUrl) { effectiveMetadata.thumbnailUrl = effectiveMetadata.thumbnailPath; } return { id: asset.id, uuid: asset.uuid, name: asset.name, createdAt: asset.createdAt, updatedAt: asset.updatedAt, type: mediaType, path: asset.path, size: asset.size, origin: asset.origin, metadata: effectiveMetadata, duration: asset.metadata.duration, width: asset.metadata.width, height: asset.metadata.height, isFavorite: asset.isFavorite } as AnyMediaResource; }
    public async saveGeneratedAsset( data: string | Uint8Array, type: AssetType, metadata?: Partial<AssetMetadata>, filename?: string, origin: AssetOrigin = 'ai' ): Promise<Asset> { let buffer: Uint8Array; if (typeof data === 'string') { if (data.startsWith('data:')) { const base64 = data.split(',')[1]; const binaryString = atob(base64); buffer = new Uint8Array(binaryString.length); for (let i = 0; i < binaryString.length; i++) { buffer[i] = binaryString.charCodeAt(i); } } else if (data.startsWith('blob:') || data.startsWith('http')) { try { const res = await fetch(data); const blob = await res.blob(); buffer = new Uint8Array(await blob.arrayBuffer()); } catch (e) { throw new Error(`Failed to fetch generated asset from ${data}`); } } else { throw new Error("Invalid data format for asset save."); } } else { buffer = data; } const asset = await this.importAsset(buffer, filename || `gen_${Date.now()}`, type, origin); if (metadata) { asset.metadata = { ...asset.metadata, ...metadata }; const absPath = await this.toAbsolutePath(asset.path); const metaPath = `${absPath}.meta.json`; let existing = {}; try { existing = JSON.parse(await vfs.readFile(metaPath)); } catch {} await vfs.writeFile(metaPath, JSON.stringify({ ...existing, metadata: asset.metadata }, null, 2)); } return asset; }
    async findAll(pageRequest?: PageRequest, typeFilter?: AssetType): Promise<ServiceResult<Page<Asset>>> { await this.ensureInitialized(); let items = this._cache; if (pageRequest?.keyword) items = items.filter(a => a.name.toLowerCase().includes(pageRequest.keyword!.toLowerCase())); if (typeFilter) items = items.filter(a => a.type === typeFilter); const size = pageRequest?.size || 50; const page = pageRequest?.page || 0; const paged = items.slice(page * size, (page + 1) * size); return Result.success({ content: paged, pageable: { pageNumber: page, pageSize: size, offset: page * size, paged: true, unpaged: false, sort: { sorted: false, unsorted: true, empty: true } }, last: (page + 1) * size >= items.length, totalPages: Math.ceil(items.length / size), totalElements: items.length, size, number: page, first: page === 0, numberOfElements: paged.length, empty: paged.length === 0, sort: { sorted: false, unsorted: true, empty: true } }); }
    ...
    async deleteById(id: string): Promise<ServiceResult<void>> { 
        ...
        if (asset) { 
            ...
            // --- FIX: Clean URL cache to avoid stale blobs/urls ---
    ...
    async save(entity: Partial<Asset>): Promise<ServiceResult<Asset>> { throw new Error("Use importAsset"); }
