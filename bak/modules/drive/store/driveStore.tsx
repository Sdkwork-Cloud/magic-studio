  ...
  const uploadFiles = async (droppedFiles?: File[]) => {
      ...
      try {
          ...
          if (droppedFiles && droppedFiles.length > 0) {
              ...
              // OPTIMIZATION: Don't read data if we can avoid it
          ...
          for (const file of files) {
              // Smart Upload: If we have a path (Desktop) and provider supports native import, skip memory load
              if (file.path && driveService.getProvider().hasCapability('native_import')) {
                   await driveService.importFile(currentPath, file.path);
              } else {
                   // Fallback (Web or Drag-Drop): If data is empty (from optimized pickFiles), we must read it now
                   if (file.data.length === 0 && file.path) {
                       // Should not happen with current logic as optimized pick is only for desktop which has importFile
                   }
                   
                   // Ensure data is present (Drag drop always provides data)
