...
import { 
    ...
    Type, MoveHorizontal, MoveVertical, WholeWord,
    CaseLower, CaseUpper, Heading1
} from 'lucide-react';
    ...
    return (
        <>
                    ...
                    <div>
                        ...
                        {hasStroke && (
                            <div className="space-y-2 animate-in fade-in slide-in-from-top-1">
                                <ColorPicker 
                                    label="Color" 
                                    value={getStyle('strokeColor')} 
                                    onChange={(v) => handleColorChange('strokeColor', v)} 
                                />
                                <ScrubbableInput 
                                    label="Width" 
                                    value={strokeWidth} 
                                    onChange={(v) => onStyleChange('strokeWidth', Math.max(0, v))} 
                                    min={0} max={100} 
                                />
                            </div>
                        )}
            ...
            <PropertySection 
                title="Background" 
                enabled={hasBg} 
                onToggle={(val) => onStyleChange('backgroundColor', val ? '#000000' : 'transparent')}
            >
                <div className="space-y-2">
                    <ColorPicker 
                        label="Color" 
                        value={hasBg ? bgColor : '#000000'} 
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <ScrubbableInput 
                            ...
                            onChange={(v) => onStyleChange('backgroundPadding', v)} 
                            min={0} max={200} 
                        />
                        <ScrubbableInput 
                            label="Radius" 
                            value={getStyle('backgroundCornerRadius') || 0} 
                            onChange={(v) => onStyleChange('backgroundCornerRadius', v)} 
                            min={0} max={100} 
                        />
                    </div>
                </div>
            ...
            >
                 <div className="space-y-2">
                    <ColorPicker 
                        label="Color" 
                        value={getStyle('shadowColor') || '#000000'} 
                        onChange={(v) => onStyleChange('shadowColor', v)} 
                    />
                    <div className="grid grid-cols-2 gap-2">
                        <ScrubbableInput label="X" value={getStyle('shadowOffsetX') || 0} onChange={(v) => onStyleChange('shadowOffsetX', v)} />
                        <ScrubbableInput label="Y" value={getStyle('shadowOffsetY') || 0} onChange={(v) => onStyleChange('shadowOffsetY', v)} />
                    </div>
                    <ScrubbableInput label="Blur" value={getStyle('shadowBlur') || 0} onChange={(v) => onStyleChange('shadowBlur', v)} min={0} max={100} />
                </div>
            </PropertySection>
        </>
    );
