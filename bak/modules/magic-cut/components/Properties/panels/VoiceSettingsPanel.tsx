...
import { Mic2, RefreshCw, User, Settings2, Captions, Type } from 'lucide-react';
        ...
        <>
            ...
            <PropertySection title="Speaker">
                <Dropdown 
                    label="Voice ID"
                    value={voiceId}
                    onChange={(v) => onUpdateResource({ 
                         metadata: { ...meta, voiceId: v } 
                    })}
                    options={PRESET_VOICES.map(v => ({ label: v.name, value: v.id, icon: <User size={12}/> }))}
                />
            </PropertySection>
