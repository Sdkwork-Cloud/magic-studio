...
import { MusicTask, GeneratedMusicResult } from '../entities/music.entity';
import { 
...
export const MusicGenerationItem: React.FC<MusicGenerationItemProps> = ({ task, onDelete, onReuse, onToggleFavorite }) => {
    // We display individual results if completed, or a placeholder if pending
        ...
        <div className="flex flex-col gap-3">
            {task.results?.map((result, idx) => (
                <MusicCard 
                    ...
                    result={result} 
                    task={task}
                    onDelete={() => onDelete(task.id)} // Deleting task removes all results usually
    ...
    return (
        <div className="group relative bg-[#18181b] border border-[#27272a] hover:border-[#3f3f46] rounded-xl p-3 flex gap-4 transition-all hover:shadow-lg">
            ...
            <div className="relative w-24 h-24 rounded-lg overflow-hidden bg-black flex-shrink-0 group/cover">
                <img src={result.coverUrl} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" alt="Cover" />
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center">
                <div className="flex justify-between items-start">
                    <h3 className="font-bold text-gray-200 text-base truncate pr-4">{result.title}</h3>
                ...
                <div className="text-xs text-gray-500 mt-1 line-clamp-1 font-mono">
                    {result.style || task.config.prompt}
                </div>
