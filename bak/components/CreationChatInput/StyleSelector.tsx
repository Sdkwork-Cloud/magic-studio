...
export interface StyleOption {
    ...
    prompt_zh?: string;  // Chinese Prompt
    
    // Structured Assets Container
    assets?: {
        scene?: StyleAsset;     // 16:9 Main
        portrait?: StyleAsset;  // 9:16 Character
        sheet?: StyleAsset;     // 16:9 Concept
        video?: StyleAsset;     // Preview Video
    };
    // Legacy support
    previewColor?: string;
}
...
export const StyleSelector: React.FC<StyleSelectorProps> = ({ 
    ...
    // Track hovered item for preview, fallback to selected value
    ...
    return (
        <>
            ...
            <Popover
                ...
                offset={12}
            >
                {/* --- LEFT COLUMN: SELECTOR (Wider for 4 cols) --- */}
                    ...
                    <div className="px-4 py-3 border-b border-[#27272a] bg-[#141414] space-y-3 z-10">
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
                                <Sparkles size={12} className="text-orange-500" /> Style Library
                            </span>
                ...
                {/* --- RIGHT COLUMN: PREVIEW & DETAILS (Flex Fill) --- */}
                        ...
                        <div className="flex justify-between items-start gap-6">
                            <div>
                                <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                                    {displayOption.label}
                                    {displayOption.isCustom && <span className="text-[9px] bg-indigo-500 text-white px-1.5 py-0.5 rounded font-bold uppercase tracking-wider align-middle relative -top-0.5">Custom</span>}
                                </h2>
                                <p className="text-sm text-gray-400 mt-2 leading-relaxed max-w-md">
                                    {displayOption.description || "Applies a specific visual aesthetic to generated content."}
                                </p>
                            </div>
                            {/* Tags */}
                            {displayOption.usage && (
                        ...
                        {(displayOption.prompt || displayOption.prompt_zh) && (
                            <div className="mt-5 p-3 bg-[#131315] border border-[#27272a] rounded-lg group relative">
                                <div className="flex justify-between items-center mb-2">
                                    <div className="flex items-center gap-3">
                                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-1.5">
                                            <Sparkles size={10} className="text-purple-400" /> Style Prompt
                                        </span>
                                ...
                                <div className="text-xs text-gray-300 font-mono leading-relaxed bg-[#0a0a0a] p-2 rounded border border-[#1f1f22] select-all line-clamp-3 hover:line-clamp-none transition-all">
                                    {getCurrentPrompt() || <span className="text-gray-600 italic">No translation available</span>}
                                </div>
                            </div>
                        )}
                    </div>

                    {/* Bento Grid Visuals - Expanded */}
                    <div className="p-6 pt-2 flex-1">
                        <div className="grid grid-cols-3 gap-4 h-[380px]">
                            
                            {/* 1. Main Scene (Top Row, Spans 3 cols) */}
                            <div className="col-span-3 row-span-4 relative bg-[#121214] rounded-xl overflow-hidden border border-[#27272a] group shadow-lg">
                                 <div className="absolute top-3 left-3 z-10 bg-black/70 backdrop-blur-md px-2.5 py-1 rounded-lg text-[10px] font-bold text-gray-200 border border-white/10 flex items-center gap-1.5 shadow-sm">
                                    <ImageIcon size={12} className="text-indigo-400" /> SCENE CONCEPT
                                 </div>
                                 {displayOption.assets?.scene?.url ? (
                                    ...
                                    <EmptyState icon={<ImageIcon size={32} />} label="No Scene Preview" />
                                 )}
                            </div>

                            {/* 2. Portrait (Bottom Left, 1 col) */}
                            <div className="col-span-1 row-span-3 relative bg-[#121214] rounded-xl overflow-hidden border border-[#27272a] group shadow-lg">
                                <div className="absolute top-3 left-3 z-10 bg-black/70 backdrop-blur-md px-2.5 py-1 rounded-lg text-[10px] font-bold text-gray-200 border border-white/10 flex items-center gap-1.5 shadow-sm">
                                    <User size={12} className="text-blue-400" /> AVATAR
                                 </div>
                                 {displayOption.assets?.portrait?.url ? (
                                    <img src={displayOption.assets.portrait.url} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Portrait" />
                                     ...
                                     // Fallback: Crop center of main image
                                    ...
                                    <EmptyState icon={<User size={24} />} label="No Portrait" />
                                 )}
                            </div>

                            {/* 3. Character Sheet (Bottom Right, 2 cols) */}
                            <div className="col-span-2 row-span-3 relative bg-[#121214] rounded-xl overflow-hidden border border-[#27272a] group shadow-lg">
                                 <div className="absolute top-3 left-3 z-10 bg-black/70 backdrop-blur-md px-2.5 py-1 rounded-lg text-[10px] font-bold text-gray-200 border border-white/10 flex items-center gap-1.5 shadow-sm">
                                    <ScanFace size={12} className="text-green-400" /> CHARACTER SHEET
                                 </div>
                                 {displayOption.assets?.sheet?.url ? (
                                    ...
                                    <EmptyState icon={<LayoutTemplate size={24} />} label="No Sheet Data" />
                                 )}
                            </div>
