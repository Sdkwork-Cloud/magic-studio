...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  return createPortal(
    <PaymentModalContent key={sessionKey} {...props} />,
    document.body,
  );
};
...
const PaymentModalContent: React.FC<Omit<PaymentModalProps, 'isOpen'>> = ({
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      await subscribe(planId, purchaseCycle, selectedMethod);
      setPhase('success');
      setMessage(
        t(
          'vip.payment.success',
      ...
      >
        ...
        <div className="space-y-5 px-6 py-5">
              ...
              <button
                ...
                className={`rounded-lg border px-4 py-3 text-sm transition ${
                  selectedMethod === 'balance'
                    ? 'border-[#f59e0b] bg-[#f59e0b1a] text-[#fbbf24]'
          ...
          <div className="rounded-lg border border-[#2a2a2a] bg-[#131315] px-3 py-2.5 text-xs text-[#9ca3af]">
            {t(
              'vip.payment.hint',
              'This action uses the canonical Magic Studio server VIP purchase API and host-owned settlement orchestration.',
            )}
          </div>
