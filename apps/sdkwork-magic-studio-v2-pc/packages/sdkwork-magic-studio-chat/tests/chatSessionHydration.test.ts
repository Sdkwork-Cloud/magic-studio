...
describe('hydrateActiveChatSession', () => {
  it('hydrates a session from the provided metadata list instead of relying on store state', async () => {
    ...
    expect(hydrated).toMatchObject({
      id: null,
      title: 'First',
      messages: [{ id: null, uuid: 'msg-1', content: 'hello' }],
    });
  });
  ...
  it('hydrates sessions by uuid key with id fallback', async () => {
    ...
    const hydrated = await hydrateActiveChatSession('session-uuid-1', sessions, loadTranscript);

    expect(loadTranscript).toHaveBeenCalledWith('session-uuid-1');
    expect(hydrated).toMatchObject({
      id: 'session-db-1',
      uuid: 'session-uuid-1',
      title: 'First',
