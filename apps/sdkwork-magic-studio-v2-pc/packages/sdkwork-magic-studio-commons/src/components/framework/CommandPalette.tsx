...
export const CommandPalette = forwardRef<CommandPaletteHandle, CommandPaletteProps>(
  (
    ...
    const setOpen = (nextOpen: boolean) => {
      ...
      setActiveIndex(nextOpen && indexedCommands.length > 0 ? 0 : -1);
    ...
    const setQuery = (nextQuery: string) => {
      ...
      const nextFilteredCount = commands
        .filter((item) => item.visible !== false)
        .filter((item) => commandMatches(item, normalizedQuery))
        .slice(0, maxVisibleCommands).length;
      setActiveIndex(nextFilteredCount > 0 ? 0 : -1);
    ...
    const indexedCommands = groupedCommands.flatMap((group) => group.commands);
    ...
    useImperativeHandle(
      ...
      [activeIndex, activeQuery, indexedCommands, moveSelection]
    );
