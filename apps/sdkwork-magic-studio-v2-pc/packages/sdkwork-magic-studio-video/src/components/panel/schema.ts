...
export const VIDEO_PANEL_MODE_SCHEMAS: VideoPanelModeSchema[] = [
    ...
    {
        mode: 'extend',
        viewKey: 'video_extend',
        generationRule: {
            allOf: [{ key: 'target_video' }]
        }
    },
    {
...
export const createVideoPanelRuntimeState = (
    ...
    const referenceImages = (config.referenceImages || []).filter(
        (item): item is NonNullable<typeof item> => hasVideoInputResourceReference(item)
    );
    ...
    const hasLipSyncAudioDriver = hasVideoInputResourceReference(config.driverAudio);
    const hasLipSyncTtsDriver = hasPrompt;
    return {
        mode: config.mode,
        isGenerating,
        hasPrompt,
        hasStartFrame: hasVideoInputResourceReference(config.image),
        hasEndFrame: hasVideoInputResourceReference(config.lastFrame),
        hasTargetVideo: hasVideoInputResourceReference(config.targetVideo),
        referenceCount: referenceImages.length,
        lipSyncSourceType,
        lipSyncDriverType,
        hasLipSyncSourceVideo: hasVideoInputResourceReference(config.targetVideo),
        hasLipSyncSourceImage: hasVideoInputResourceReference(config.targetImage),
...
export const getModeTransitionPatch = (
    ...
    const firstReference = (config.referenceImages || []).find((item) =>
        hasVideoInputResourceReference(item)
    );
    const startFrame = hasVideoInputResourceReference(config.image)
        ? config.image
        : undefined;

    if (mode === 'subject_ref' && !hasVideoInputResourceReference(config.image) && firstReference) {
        return { image: firstReference };
    }
    ...
    if (
        ...
        return { referenceImages: [startFrame] };
    }
