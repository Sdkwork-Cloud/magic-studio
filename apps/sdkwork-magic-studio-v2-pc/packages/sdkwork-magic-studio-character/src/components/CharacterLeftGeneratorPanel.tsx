...
import { IMAGE_PROVIDERS } from '@sdkwork/magic-studio-image/constants';
...
import { ChooseVoiceSpeaker } from '@sdkwork/magic-studio-voicespeaker/picker';
...
} from '../utils/characterAvatarAsset';
...
export const CharacterLeftGeneratorPanel: React.FC = () => {
    ...
    const createCapability = useRuntimeMagicStudioExecutionOperationCapability(
        ...
        'create',
        {
            feature: 'CharacterLeftGeneratorPanel'
        }
    );
    ...
    const [imageProviders, setImageProviders] = useState<ModelProvider[]>([]);
    ...
    const createDisabledReason = createCapability.disabledReason;
    const promptEnhanceDisabledReason = promptEnhanceCapability.disabledReason;
    const canGenerate =
        createCapability.ready && (!!config.description?.trim() || !!config.name?.trim());
    ...
    const handleEnhance = async (text: string): Promise<string> => {
        ...
        try {
            return await imageService.enhancePrompt(text);
        } catch (error) {
    ...
    const handleAvatarChange = (asset: Asset | null) => {
        setConfig(toCharacterAvatarAssetFields(asset));
    };
    ...
    return (
        <>
            <div className="flex-none bg-[#09090b] border-b border-[#27272a] z-30">
                <div className="px-4 h-14 flex items-center justify-between gap-3">
                    ...
                    <div className="flex-1 max-w-[220px] min-w-[170px]">
                        <ImageModelSelector
                            value={config.model || imageModelIds[0] || ''}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-[#09090b]">
                <div className="space-y-2">
                    ...
                    <div className="relative h-[clamp(220px,34vh,300px)] w-full rounded-xl border border-[#27272a] bg-[#101013] p-3">
                        <ChooseAsset
                            value={toCharacterAvatarChooseAssetValue(config)}
                ...
                <div className="space-y-2">
                    ...
                    <PromptTextInput
                        ...
                        onEnhance={promptEnhanceCapability.ready ? handleEnhance : undefined}
                    />

                    <p className="text-[10px] text-gray-600">
                        Keep it specific: appearance, outfit, expression, and scene style.
                    </p>
                    {promptEnhanceDisabledReason && (
            ...
            <div className="p-3 border-t border-[#27272a] bg-[#09090b] z-30">
                ...
                {!isGenerating && createDisabledReason && (
