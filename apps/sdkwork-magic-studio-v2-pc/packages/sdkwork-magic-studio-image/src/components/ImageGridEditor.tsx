...
;
import { SettingToggle } from '@sdkwork/magic-studio-settings';
...
class GridProcessor {
    static async split(
        ...
        const cells: GeneratedImageResult[] = [];
        ...
        for (let y = 0; y < rows; y++) {
            for (let x = 0; x < cols; x++) {
                ...
                cells.push(createGeneratedImageResult({
    ...
    static async merge(
        ...
        const loadedImages = await Promise.all(cells.map(cell => {
            const src = resolveGeneratedImageResultUrl(cell.source);
...
interface ImageGridEditorProps {
    ...
    onUpscale?: (image: GeneratedImageResult) => void;
    rows?: number;
    cols?: number;
}
...
export const ImageGridEditor: React.FC<ImageGridEditorProps> = ({ 
    ...
    // --- State ---
    const [currentImage, setCurrentImage] = useState<GeneratedImageResult>(image);
    const currentImageUrl = resolveGeneratedImageResultUrl(currentImage);
    const { url: resolvedImageUrl } = useAssetUrl(currentImageUrl || undefined);
    
    const [rows, setRows] = useState(initialRows);
    const [cols, setCols] = useState(initialCols);
    const [gridCells, setGridCells] = useState<ImageGridCell[]>([]);
    ...
    const handleReplaceSource = async () => {
        try {
            ...
            if (files.length > 0) {
                ...
                setCurrentImage(createGeneratedImageResult({
                    url,
                }));
            }
        } catch (e) {
    ...
    const handleSplit = async () => {
        ...
        try {
            ...
            setGridCells(cells.map((cell) => createImageGridCell({ source: cell })));

            // 2. Auto-Save to Assets if enabled
            if (autoSaveTiles) {
                const savePromises = cells.map(async (cell, idx) => {
                    const dataUrl = resolveGeneratedImageResultUrl(cell);
                    if (!dataUrl) {
                        return;
                    }
                ...
                await runtime.dialog.notify('Tiles Saved', `${cells.length} images saved to Assets library.`);
            }
    ...
    const handleReplaceCell = async (index: number) => {
        try {
            ...
            if (files.length > 0) {
                ...
                setGridCells(prev => {
                    const next = [...prev];
                    const previousCell = next[index];
                    next[index] = createImageGridCell({
                        id: previousCell?.id ?? null,
                        uuid: previousCell?.uuid,
                            ...
                            {originalImage ? (
                                <div className="absolute inset-0 w-full h-full flex items-center justify-center p-2">
                                    <div className="relative w-full h-full">
                                        <img 
                                            src={resolvedImageUrl || currentImageUrl || undefined} 
                    ...
                    {gridCells.length > 0 ? (
                        ...
                        >
                            {gridCells.map((cell, idx) => {
                                ...
                                const src = resolveGeneratedImageResultUrl(source);
                                    ...
                                    <div className={`absolute inset-0 bg-black/60 flex items-center justify-center gap-2 transition-opacity duration-200 ${hoveredCell === idx ? 'opacity-100' : 'opacity-0'}`}>
                                        ...
                                        {src && (
                                            <>
                                                {source && onUpscale && (
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); onUpscale(source); }}
