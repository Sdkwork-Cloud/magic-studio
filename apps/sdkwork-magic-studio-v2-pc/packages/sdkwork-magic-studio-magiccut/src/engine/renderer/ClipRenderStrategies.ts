...
export class VideoClipStrategy implements IClipRenderStrategy {
    ...
    getTexture(ctx: RenderContext, clip: CutClip, resource: AnyMediaResource, _time: number, _isPlaying: boolean): WebGLTexture | null {
        ...
        const video = resourceManager.getVideoElement(clipKey, url);
        ...
        const cachedInfo = textureCache.getInfo(clipKey);
        let tex = cachedInfo?.texture;
        const currentVideoTime = video.currentTime;
        const lastTime = this.lastUploadTime.get(clipKey);
        ...
        if (needsUpload) {
            ...
            try {
                ...
                textureCache.set(clipKey, tex!, videoW, videoH);
                this.lastUploadTime.set(clipKey, currentVideoTime);
            } catch (_e) {
                return emptyTexture;
            }
        }
...
export class ImageClipStrategy implements IClipRenderStrategy {
    ...
    getTexture(ctx: RenderContext, _clip: CutClip, resource: AnyMediaResource, _time: number, _isPlaying: boolean): WebGLTexture | null {
        ...
        let tex = textureCache.get(resourceKey);
        
        if (!tex) {
            const url = resourceManager.resolveResourceUrl(resource);
            if (!url) return emptyTexture;
            
            const img = resourceManager.getImageElement(resourceKey, url);
            if (img.complete && img.naturalWidth > 0) {
                ...
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
                gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
                textureCache.set(resourceKey, tex, img.naturalWidth, img.naturalHeight);
            }
        }
...
export class TextClipStrategy implements IClipRenderStrategy {
    ...
    getTexture(ctx: RenderContext, clip: CutClip, resource: AnyMediaResource, _time: number, _isPlaying: boolean): WebGLTexture | null {
        ...
        const hash = `${clipKey}:${textContent}:${JSON.stringify(style)}`;
        const lastHash = this.hashCache.get(clipKey);
        let tex = textureCache.get(clipKey);
        ...
        if (lastHash !== hash || !tex) {
            ...
            textureCache.set(clipKey, tex, textCanvas.width, textCanvas.height);
            this.hashCache.set(clipKey, hash);
        }
