...
import { getRobustResourceUrl, requiresCanonicalAssetUrlResolution } from '../../utils/resourceUtils';
...
export class ResourceManager {
    ...
    public resolveResourceUrl(resource: AnyMediaResource): string | null {
        ...
        const url = getRobustResourceUrl(resource);
        const resolutionCacheKey = assetId || resourceKey;

        if (!url && !assetId) {
            return null;
        }

        if (assetId || requiresCanonicalAssetUrlResolution(url)) {
            const cached = this.resolvedAssetUrls.get(resolutionCacheKey);
            if (cached) {
                return cached;
            }

            if (!this.pendingAssetResolves.has(resolutionCacheKey)) {
                const task = resolveAssetUrlByAssetIdFirst(resource).then((resolved) => {
                    if (resolved) {
                        this.resolvedAssetUrls.set(resolutionCacheKey, resolved);
                    ...
                    this.pendingAssetResolves.delete(resolutionCacheKey);
                });
                this.pendingAssetResolves.set(resolutionCacheKey, task.then(() => {}));
            }
