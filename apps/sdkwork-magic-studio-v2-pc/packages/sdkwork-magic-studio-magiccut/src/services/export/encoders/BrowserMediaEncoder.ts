    ...
    start(): void {
        ...
        if (this.config?.exportAudio && this.audioBuffer && this.audioCtx && this.audioDest) {
             ...
             if (this.audioCtx.state === 'suspended') {
                 this.audioCtx.resume().catch((_e) => {
                     // Resume may fail until the browser grants audio playback.
                 });
             }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.audioSource) {
                try {
                    ...
                    // Source may already be stopped during teardown.
                }
            }
    ...
    dispose() {
        if (this.audioCtx) {
            try {
                ...
                // Context close is best-effort during disposal.
            }
        }
        if (this.stream) {
