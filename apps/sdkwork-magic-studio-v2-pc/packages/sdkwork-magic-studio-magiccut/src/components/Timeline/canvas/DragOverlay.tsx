...
export const DragOverlay: React.FC<DragOverlayProps> = ({
    ...
    const resolveClipResource = (clip: CutClip | null | undefined): AnyMediaResource | undefined => (
        clip ? getResource(resolveEntityKey(clip.resource)) : undefined
    );
    ...
    const semanticPreview = (() => {
        ...
        if (preview.kind === 'effect') {
            ...
            const resource = resolveClipResource(clip);
        ...
        return {
            ...
            fromLabel: resolveClipResource(fromClip)?.name || 'Outgoing clip',
            toLabel: resolveClipResource(toClip)?.name || 'Incoming clip'
        };
    })();
    ...
    const ghostData = (() => {
        ...
        const isValid = interaction.validDrop && !interaction.hasCollision;
        ...
        // Case B: Moving Existing Clip
            ...
            resource = resolveClipResource(clipsMap[interaction.clipId]);
            operationLabel = "Move";
        }
        ...
        <>
            ...
            <div ref={ghostRef} className="absolute z-[40] pointer-events-none transition-all duration-75 ease-out rounded-md overflow-hidden" style={{ display: 'none' }}>
                <MagicCutClip
                    ...
                    resourceSource={ghostData.resource}
                ...
                const linkedResourceUrl = getRobustResourceUrl(resource);
