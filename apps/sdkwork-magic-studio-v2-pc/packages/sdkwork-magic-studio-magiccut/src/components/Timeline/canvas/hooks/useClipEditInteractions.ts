        ...
        rafId.current = requestAnimationFrame(() => {
            ...
            switch (interaction.type) {
                ...
                case 'slip-trim': {
                    ...
                    updateClip(clipKey, { offset: newOffset });
                    break;
                }
                ...
                case 'trim-start': {
                    ...
                    if (newDuration > 0.1 && newOffset >= 0) {
                        const linkedUpdates = ClipLinkService.calculateLinkedTrim(
                            clipKey, 'start', newStart, state, linkedSelectionEnabled
                        );
                ...
                case 'trim-end': {
                    const newDuration = initialState.duration + deltaTime;
                    if (newDuration > 0.1) {
                        updateClip(clipKey, { duration: newDuration });
                    }
                    break;
                }
                ...
                case 'move': {
                    const linkedUpdates = ClipLinkService.calculateLinkedMovement(
                        clipKey, deltaTime, state, linkedSelectionEnabled
                    );
