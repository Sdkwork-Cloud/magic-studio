...
export const SkimmableAssetCard: React.FC<SkimmableAssetCardProps> = React.memo(({ 
    ...
    useEffect(() => {
        ...
        const resolve = async () => {
            ...
            try {
                ...
                // B. Resolve Thumbnail
                const thumbPath = resolveMagicCutThumbnailReference(item.metadata as any);
                let thumb = null;
                ...
                console.warn("[AssetCard] Failed to resolve asset:", itemKey, e);
    ...
    }, [itemKey, item.path, item.url, isEffect, isVideo, isText, (item.metadata as any)?.thumbnailPath, (item.metadata as any)?.thumbnailUrl]);
            ...
            <div className={`absolute top-1 right-1 z-30 transition-opacity duration-200 ${isFavorite ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                <button 
                    onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        onToggleFavorite?.(itemKey, resolveNextFavoriteState(item.isFavorite));
                    }}
