...
import { DEFAULT_PAGE_SIZE } from '@sdkwork/magic-studio-types/pagination';
...
import type { AnyAsset, Asset } from '@sdkwork/magic-studio-assets/entities';
...
const resolveAssetClientKey = (
    asset: Pick<AnyAsset, 'id' | 'uuid'>
): string => resolveEntityKey(asset);

const resolvePersistentAssetId = (
    asset: Pick<AnyAsset, 'id'>
): string | null => hasPersistentEntityId(asset.id) ? asset.id : null;

const resolveProjectAssetReferenceId = (
    asset: AnyAsset
): string | null => {
    const assetId =
    ...
    return assetId || resolvePersistentAssetId(asset);
};
...
const TemplateCategoryView: React.FC = () => {
    ...
    useEffect(() => {
        const timeoutId = window.setTimeout(() => {
            setTemplateKeyword(searchQuery.trim());
        }, 250);
        ...
        <div className="h-full flex flex-col bg-[#050505] border-r border-white/5 min-w-[320px]">
            ...
            <div className="flex-1 overflow-y-auto p-2 custom-scrollbar bg-[#050505]">
                {templates.length === 0 && !loading ? (
                    ...
                    <TemplateResourceGrid templates={templates} onLoadTemplate={setTemplateToLoad} />
                )}
...
const AssetCategoryView: React.FC<AssetCategoryViewProps> = ({ category, viewMode, onViewModeChange }) => {
    ...
    const queryLocalAssetCenterPage = useCallback(async (pageNum: number) => {
        ...
        const localResult = await assetCenterService.query({
            ...
            scope: {
                ...workspaceScope,
                projectId: workspaceScope.projectId || projectKey,
                domain: 'magiccut'
            },
    ...
    }, [category, debouncedQuery, projectKey]);
        ...
        const persistedCatalogAssets = remoteContent.length === 0
            ...
            : await Promise.all(
                remoteContent.map(async (item) => {
                    try {
                        const persistentId = resolvePersistentAssetId(item);
                        if (!persistentId) {
                            return null;
                        }
                        const persisted = await assetCenterService.findById(persistentId);
    ...
    const assets = useMemo(() => {
        const merged = mergeAssetCollections(localAssets, remoteAssets)
            .filter((asset) => !hiddenAssetIds[resolveAssetClientKey(asset)]);

        return merged.map((asset) => {
            const assetKey = resolveAssetClientKey(asset);
            if (favoriteOverrides[assetKey] === undefined) return asset;
            return {
                ...asset,
                isFavorite: favoriteOverrides[assetKey]
            };
        });
    ...
    const handleToggleFavorite = useCallback(async (assetKey: string, isFavorite: boolean) => {
        const asset = assets.find((item) => resolveAssetClientKey(item) === assetKey);
        if (!asset) {
            return;
        }

        const mutation = beginFavoriteMutation(favoriteMutationVersionsRef.current, assetKey);
        favoriteMutationVersionsRef.current = mutation.activeMutations;
        setFavoriteOverrides(prev => ({ ...prev, [assetKey]: isFavorite }));

        try {
            const scope = buildMagicCutImportScope(readWorkspaceScope(), projectKey);
            const persistedAssetId = resolvePersistentAssetId(asset);
            let persistedAsset = persistedAssetId
                ? await assetCenterService.setFavorite(persistedAssetId, isFavorite)
                : null;

            if (!persistedAsset) {
                const registeredAsset = await assetCenterService.registerExistingAsset(
                    buildFavoriteRegistrationInput(asset, scope)
                );
                persistedAsset = await assetCenterService.setFavorite(registeredAsset.id, isFavorite);
            }
            ...
            if (!nextAsset) {
                throw new Error(`Favorite update failed: asset ${assetKey} was not available in the local catalog.`);
            }

            if (!isCurrentFavoriteMutation(favoriteMutationVersionsRef.current, assetKey, mutation.requestId)) {
                return;
            }

            setRemoteAssets(prev => syncFavoriteInAssetCollection(prev, assetKey, !!nextAsset.isFavorite).map((candidate) => (
                resolveAssetClientKey(candidate) === assetKey
                    ? {
                        ...candidate,
                        id: nextAsset.id,
                        uuid: nextAsset.uuid,
                        updatedAt: nextAsset.updatedAt,
                        metadata: nextAsset.metadata,
                    }
                    : candidate
            )));
            updateResource(assetKey, {
                id: nextAsset.id,
                uuid: nextAsset.uuid,
            ...
            setFavoriteOverrides(prev => clearFavoriteOverride(prev, assetKey));
        } catch (error) {
            if (!isCurrentFavoriteMutation(favoriteMutationVersionsRef.current, assetKey, mutation.requestId)) {
                return;
            }
    ...
    }, [assets, projectKey, t, updateResource]);
    ...
    const handleDeleteAsset = useCallback(async (asset: AnyAsset) => {
        ...
        if (protectedDeleteMessage) {
            await runtime.dialog.notify(t('resources.notifications.deleteUnavailableTitle'), protectedDeleteMessage);
            return;
        }

        const assetKey = resolveAssetClientKey(asset);
        const persistentAssetId = resolvePersistentAssetId(asset);
        const projectAssetReferenceId = resolveProjectAssetReferenceId(asset);

        if (projectAssetReferenceId && isAssetInUse(projectAssetReferenceId)) {
            await runtime.dialog.notify(
        ...
        const confirmed = await runtime.dialog.confirm(
        ...
        if (confirmed) {
            const previewCleanup = resolveDeletedAssetPreviewCleanup({
                assetId: assetKey,
                skimmingResourceId: skimmingResource ? resolveEntityKey(skimmingResource) : undefined,
                isCoordinatorPreviewingAsset: playerPreviewService.isPreviewingResource(assetKey)
            });

            if (persistentAssetId) {
                const deletionResult = await assetService.deleteById(persistentAssetId);
            ...
            if (previewCleanup.clearCoordinatorPreview) {
                playerPreviewService.clearPreviewForResource(assetKey);
            }
            ...
            if (projectAssetReferenceId) {
                removeAssetFromProjectState(projectAssetReferenceId);
            }
            setRemoteAssets(prev => prev.filter((candidate) => resolveAssetClientKey(candidate) !== assetKey));
            setHiddenAssetIds(prev => ({ ...prev, [assetKey]: true }));
        }
    ...
    const handleDragStart = useCallback((e: React.DragEvent, item: AnyAsset) => {
        ...
        e.dataTransfer.setData('text/plain', resolveAssetClientKey(item));
