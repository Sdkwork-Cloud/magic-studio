...
export const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose }) => {
    ...
    const [renderCapabilities, setRenderCapabilities] = useState<Awaited<ReturnType<typeof magicCutRenderService.readCapabilities>> | null>(null);
    ...
    useEffect(() => {
        const checkPlatform = async () => {
            const isNative = isDesktopShellRuntimeKind(runtime.system.kind());
    ...
    useEffect(() => {
        ...
        void magicCutRenderService
            .readCapabilities()
            .then((capabilities) => {
                if (!active) return;

                setRenderCapabilities(capabilities);
                const presentation = resolveExportRuntimePresentation(capabilities, t);
    ...
    const coverImageSource = useMemo(() => {
        ...
        for (const trackRef of activeTimeline.tracks) {
            const track = state.tracks[resolveEntityKey(trackRef)];
            if (track && (track.trackType === 'video' || track.isMain) && track.clips.length > 0) {
                const sortedClips = track.clips
                    .map((ref) => state.clips[resolveEntityKey(ref)])
                    .filter((clip): clip is NonNullable<typeof clip> => Boolean(clip))
                    .sort((a, b) => a.start - b.start);

                if (sortedClips.length > 0) {
                    const firstClip = sortedClips[0];
                    const resource = getResource(resolveEntityKey(firstClip.resource));
                    if (!resource) continue;

                    const thumbnailReference = resolveMagicCutThumbnailReference(resource.metadata as {
    ...
    const { url: coverImage } = useAssetUrl(coverImageSource, {
        resolver: resolveAssetUrlByAssetIdFirst
    });

    const audioOnlyFormatOption = useMemo(
        () => resolveAudioOnlyExportCard(renderCapabilities, t),
    ...
    const exportBlockedReason = useMemo(() => {
        ...
        if (isAudioOnly) {
            if (format !== 'wav') {
                return te('validation.audioOnlyWav');
            }

            return audioOnlyFormatOption.available ? null : audioOnlyFormatOption.description;
        }
        ...
        return null;
    }, [audioEnabled, audioOnlyFormatOption, format, isAudioOnly, runtimeLoading, runtimeError, runtimePresentation, selectedFormatOption, te, videoEnabled]);
    ...
    const handleBrowsePath = async () => {
        try {
            const selected = await runtime.fileSystem.selectDir();
    ...
    const handleExport = async () => {
        ...
        if (!safeFileName) {
            await runtime.dialog.notify(te('failed'), te('validation.missingFileName'));
            return;
        }

        if (exportBlockedReason) {
            await runtime.dialog.notify(te('failed'), exportBlockedReason);
            return;
        }
        ...
        if (exportValidationError) {
            await runtime.dialog.notify(te('failed'), exportValidationError);
            return;
        }

        if (smartHdr && runtimePresentation && !runtimePresentation.smartHdrSupported) {
            await runtime.dialog.notify(te('failed'), runtimePresentation.smartHdrReason);
            return;
        }

        if (inPoint !== null && outPoint !== null && outPoint <= inPoint) {
            await runtime.dialog.notify(te('failed'), te('validation.invalidRange'));
            return;
        }
        ...
        try {
            if (!isAudioOnly) {
                throw new Error(
                    runtimePresentation?.blockingReason || 'Video export is not available in the canonical server render pipeline.',
                );
            }

            await magicCutRenderService.exportAudioMixdown(
                {
                    project,
                    state,
                    timelineId: resolveEntityKey(activeTimeline),
                    fileName: safeFileName,
                    destinationPath: isDesktop ? exportPath : undefined,
                    startTimeSeconds: inPoint,
                    endTimeSeconds: outPoint,
            ...
            if (error.message !== 'Export cancelled') {
                ...
                await runtime.dialog.notify(te('failed'), error.message || te('validation.unknownError'));
            }
        } finally {
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-black/80 via-black/30 to-transparent">
                                <div className="text-[10px] uppercase tracking-[0.22em] text-white/55 font-semibold">
                                    {isAudioOnly
                                        ...
                                        : activeFormatOption?.route === 'server-render'
                                        ? 'Server Render'
...
const FormatCard: React.FC<{
    ...
    const { te } = useMagicCutTranslation();
    const routeLabel = option.route === 'server-render'
        ? 'Server Render'
