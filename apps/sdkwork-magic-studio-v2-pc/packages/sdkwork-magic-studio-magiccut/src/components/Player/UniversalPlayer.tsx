...
import React, { useRef, useState, useMemo, useLayoutEffect, forwardRef, useImperativeHandle, useContext, createContext, useEffect, useCallback } from 'react';
...
export const UniversalPlayer = forwardRef<UniversalPlayerHandle, UniversalPlayerProps>(({
    ...
    const hasPreview = !!previewResource;
    ...
    }, [data, previewResource, previewResourceTime, isPlaying, hiddenClipIds]);

    // --- Dynamic Override Logic (Direct Store Access) ---
    const getRenderOverride = useCallback((): RenderOverrideClip | null => {
        ...
        if (interaction.type !== 'idle' && interaction.clipId) {
            ...
            const resource = clip ? dataRef.current.resources[resolveEntityKey(clip.resource)] : undefined;
    ...
    const renderFrame = useCallback((time: number, forcePlayingState?: boolean, previewTimeOverride?: number) => {
                    ...
                    {hasPreview && (
                        <div className="absolute top-2 left-2 z-30 pointer-events-none">
                            <div className="bg-red-500/90 text-white text-[10px] px-2 py-1 rounded font-bold uppercase shadow-lg border border-white/20">
                                {tp('previewLabel', { name: previewResource?.name })}
                            </div>
                        </div>
                    )}

                    {(!data.timeline || data.timeline.tracks.length === 0) && !hasPreview && !dragOperation && (
