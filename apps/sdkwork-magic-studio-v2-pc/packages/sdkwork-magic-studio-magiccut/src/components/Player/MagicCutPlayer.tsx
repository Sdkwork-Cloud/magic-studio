...
import { createCutLayerRef } from '@sdkwork/magic-studio-types/magiccut';
...
import { createMagicCutPreviewLayer } from '../../domain/preview/previewIdentity';
...
export const MagicCutPlayer: React.FC = React.memo(() => {
    ...
    const renderData: RenderData = useMemo(() => {
        ...
        if (currentDrag && currentDrag.payload) {
            resources = {
                ...state.resources,
                [resolveEntityKey(currentDrag.payload)]: currentDrag.payload
            };
        }
        ...
        if (previewEffect && activeTimeline) {
            ...
            const sortedTracks = activeTimeline.tracks
                .map((ref) => state.tracks[resolveEntityKey(ref)])
                .filter((track): track is CutTrack => Boolean(track) && track.visible !== false && track.trackType === 'video')
            ...
            for (const track of sortedTracks) {
                const found = track.clips
                    ...
                    .find((clip): clip is CutClip => Boolean(clip) && currTime >= clip.start && currTime < clip.start + clip.duration);
                if (found) {
                    targetClip = found;
                    break;
                }
            ...
            if (targetClip) {
                clips = { ...clips };
                layers = { ...layers };
                const tempLayer = createMagicCutPreviewLayer({
                    previewEffectId: resolveEntityKey(previewEffect),
                    targetClip,
                    order: targetClip.layers.length
                });
                layers[tempLayer.uuid] = tempLayer;
                clips[resolveEntityKey(targetClip)] = {
                    ...targetClip,
                    layers: [...targetClip.layers, createCutLayerRef(tempLayer)]
                };
            }
        }
    ...
    const handleStageClick = (e: React.MouseEvent, point: { x: number, y: number, time: number }) => {
        ...
        const tracks = activeTimeline.tracks
            .map((ref) => state.tracks[resolveEntityKey(ref)])
            .filter((track): track is CutTrack => Boolean(track) && track.visible !== false && !track.locked);
        ...
        for (const candidate of candidates) {
            ...
            if (isPointInRotatedRect(projX, projY, tf.x, tf.y, tf.width * (tf.scale || 1), tf.height * (tf.scale || 1), tf.rotation || 0)) {
                hitClipId = resolveEntityKey(clip);
                break;
            }
        }
                ...
                >
                    ...
                    >
                        ...
                        {!skimmingResource && selectedClipId && isEditingText && selectedClip && (
                            <TextOverlayEditor
                                clip={selectedClip}
                                onChange={(txt) => updateClip(resolveEntityKey(selectedClip), { content: txt })}
