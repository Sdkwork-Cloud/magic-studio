...
export const AudioSettingsPanel: React.FC<AudioSettingsPanelProps> = ({ clip, onUpdate }) => {
    ...
    const equalizerSeed = `${clipKey}:${eqSettings.enabled ? '1' : '0'}`;
    const [equalizerState, setEqualizerState] = useState<{ seed: string; open: boolean }>({
        seed: equalizerSeed,
        open: eqSettings.enabled
    });
    ...
    const effectiveEqualizerOpen =
        equalizerState.seed === equalizerSeed ? equalizerState.open : eqSettings.enabled;
    ...
    const toggleEqualizerPanel = () => {
        const nextOpen = !effectiveEqualizerOpen;
        setEqualizerState({ seed: equalizerSeed, open: nextOpen });
    ...
    const closeEqualizer = () => {
        setEqualizerState({ seed: equalizerSeed, open: false });
