...
import { Trash2, Palette, ChevronRight, Eye, EyeOff } from 'lucide-react';
...
export const EffectsListPanel: React.FC<EffectsListPanelProps> = ({ layers, onUpdateLayers }) => {
    ...
    const toggleEffect = (id: string) => {
        onUpdateLayers(layers.map((layer) => resolveEntityKey(layer) === id ? { ...layer, enabled: !layer.enabled } : layer));
    };

    const removeLayer = (id: string) => {
        onUpdateLayers(layers.filter((layer) => resolveEntityKey(layer) !== id));
    };

    const updateLayerParams = (id: string, params: Record<string, any>) => {
        onUpdateLayers(layers.map((layer) => resolveEntityKey(layer) === id ? { ...layer, params: { ...layer.params, ...params } } : layer));
    };
            ...
            {layers.length === 0 ? (
                ...
                <div className="space-y-2">
                    {layers.map((layer) => {
                         ...
                         const layerKey = resolveEntityKey(layer);
                         const isExpanded = expandedId === layerKey;

                         return (
                            <div key={layerKey} className={`bg-[#18181b] border ${isExpanded ? 'border-blue-500/30' : 'border-[#27272a]'} rounded-lg transition-colors overflow-hidden`}>
                                {/* Header */}
                                <div 
                                    className="flex justify-between items-center p-2 cursor-pointer hover:bg-[#1e1e20]"
                                    onClick={() => setExpandedId(isExpanded ? null : layerKey)}
                                >
                                    <div className="flex items-center gap-2">
                                        <button onClick={(e) => { e.stopPropagation(); toggleEffect(layerKey); }} className={`text-gray-500 hover:text-white transition-colors ${!layer.enabled ? 'opacity-50' : ''}`}>
                                    ...
                                    <div className="p-3 border-t border-[#27272a] bg-[#141416] space-y-3 animate-in slide-in-from-top-1 fade-in duration-150">
                                        {def ? (
                                            <EffectParamsEditor 
                                                definition={def} 
                                                values={layer.params} 
                                                onChange={(newParams) => updateLayerParams(layerKey, newParams)} 
                                            />
                                        ) : (
