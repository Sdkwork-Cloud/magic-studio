        ...
        const resolveThumbnail = async () => {
            ...
            try {
                const nextThumbnail = thumbnailSource
                    ? await resolveAssetUrlByAssetIdFirst(thumbnailSource)
                    : !isAudio && !isEffect && !isText
                        ? await resolveAssetUrlByAssetIdFirst(item)
                        : null;

                if (active) {
                    setResolvedThumbnail(nextThumbnail || null);
                }
            } catch {
                if (active) {
                    setResolvedThumbnail(null);
