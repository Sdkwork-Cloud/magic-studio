...
describe('mergeAssetCollections', () => {
  ...
  it('deduplicates runtime assets by uuid before falling back to persisted ids', () => {
    const localAssets = [
      {
        ...createResource(null, MediaResourceType.VIDEO),
    ...
    const remoteAssets = [
      {
        ...createResource('video-db-1', MediaResourceType.VIDEO),
        uuid: 'video-runtime-uuid',
    ...
    expect(resolveEntityKey(merged[0] as any)).toBe('video-runtime-uuid');
    expect(merged[0]?.name).toBe('Runtime Draft');
  });
});
