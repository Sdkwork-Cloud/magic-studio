...
describe('resolveGeneratedVoiceUpdates', () => {
  it('resets offset and retimes the clip to the generated source duration at the current playback speed', () => {
    ...
    expect(result.resourceUpdates).toMatchObject({
      url: 'https://cdn.example.com/generated.wav',
      path: 'https://cdn.example.com/generated.wav',
      duration: 12,
      metadata: expect.objectContaining({
        ...
        pitch: 1.1,
        speakerId: 'Nova',
        generatedVoiceUrl: 'https://cdn.example.com/generated.wav',
      }),
    });
  });

  it('rejects generated voice results without a playable delivery url', () => {
    expect(() =>
      resolveGeneratedVoiceUpdates({
        resource: {
          id: 'voice-resource',
    ...
    ).toThrow('Voice generation returned no playable result.');
  });
});
