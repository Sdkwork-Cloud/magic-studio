...
This document summarizes the comprehensive enhancements made to the `@sdkwork/magic-studio-assets` package to create a perfect, unified asset management center that serves as the foundation for all business modules in the Magic Studio ecosystem.
...
### Basic Setup
```typescript
import { initializeAssetServices } from '@sdkwork/magic-studio-assets';
...
```typescript
import { getAssetService, CharacterAsset } from '@sdkwork/magic-studio-assets';
...
```typescript
import { getAssetService, SfxAsset } from '@sdkwork/magic-studio-assets';
...
- �?Support for Characters and SFX
