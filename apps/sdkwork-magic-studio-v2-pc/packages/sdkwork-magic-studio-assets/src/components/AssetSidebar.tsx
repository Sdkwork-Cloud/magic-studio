...
import { buildFrameworkStyle } from '@sdkwork/magic-studio-commons/framework/tokens';
import type { AssetType } from '../entities';
import { useAssetStore } from '../store/assetStore';
import { ASSET_CENTER_CATEGORIES } from '../asset-center/domain/assetCategory.domain';
...
const readStoredSectionState = (domain: string): SectionState => {
...
export const AssetSidebar: React.FC<AssetSidebarProps> = ({
  ...
  const [sectionsByDomain, setSectionsByDomain] = useState<Record<string, SectionState>>(() => ({
    [domain]: readStoredSectionState(domain)
  }));
  const sections = useMemo(
    () => sectionsByDomain[domain] ?? readStoredSectionState(domain),
    [domain, sectionsByDomain]
  );
  ...
  const toggleSection = useCallback((key: SectionKey) => {
    setSectionsByDomain((prev) => {
      const current = prev[domain] ?? readStoredSectionState(domain);
      return {
        ...prev,
        [domain]: {
          ...current,
