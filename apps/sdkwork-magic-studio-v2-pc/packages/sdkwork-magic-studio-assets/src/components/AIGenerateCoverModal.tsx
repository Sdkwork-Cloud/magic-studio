...
interface AIGenerateCoverModalProps {
    contextText: string;
    onClose: () => void;
    onSuccess: (asset: PersistedGenerationOutcomeAsset) => void;
}
...
export const AIGenerateCoverModal: React.FC<AIGenerateCoverModalProps> = ({ contextText, onClose, onSuccess }) => {
    ...
    const [generatedImage, setGeneratedImage] = useState<PersistedGenerationOutcomeAsset | null>(null);
    const [error, setError] = useState<string | null>(null);
    const { url: generatedImagePreviewUrl } = useAssetUrl(generatedImage?.path || generatedImage?.url || null);
    ...
    const analyzeContent = async () => {
        ...
        setError(null);
        try {
            const suggestions = await suggestAssetCoverPrompts({
                context: contextText || t('assetCenter.coverGenerator.defaults.context'),
                count: 3,
            });
    ...
    const generateImage = async (prompt: string) => {
        ...
        setError(null);
        try {
            const persisted = await generateAssetCoverImage({
                prompt,
            });
            if (!persisted.path && !persisted.url) {
                throw new Error(t('assetCenter.coverGenerator.errors.generate'));
            }
            setGeneratedImage(persisted);
            setStep('preview');
        } catch (e: any) {
                    ...
                    {step === 'selection' && (
                        <div className="flex-1 flex flex-col">
                            ...
                            <div className="mt-4 flex justify-between items-center border-t border-[#333] pt-4">
                                ...
                                <span className="text-xs text-gray-600">{t('assetCenter.coverGenerator.poweredBy', { model: 'SDKWork AI' })}</span>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="flex-1 flex flex-col h-full">
                             <div className="flex-1 bg-[#111] rounded-xl border border-[#333] overflow-hidden relative group mb-6 flex items-center justify-center min-h-[250px]">
                                 {generatedImagePreviewUrl ? (
                                     <img src={generatedImagePreviewUrl} className="max-w-full max-h-[350px] object-contain shadow-2xl" alt={t('assetCenter.coverGenerator.preview.imageAlt')} />
                                 ) : (
