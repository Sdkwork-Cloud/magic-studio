...
} from './assetSelectionIdentity';
import { createDocumentSelectionAsset } from './documentSelectionAsset';
import type { ChooseAssetModalContentProps } from './ChooseAssetModal.types';
import { persistChooseAssetModalSelectionProjectReferences } from './chooseAssetModalProjectReference';

const FRAMEWORK_STYLE = buildFrameworkStyle();

const ChooseAssetModalPanel: React.FC<ChooseAssetModalContentProps> = ({
    onClose,
    onConfirm,
    accepts: _accepts,
    multiple: _multiple,
    ...
    const selectedAssetIds = useMemo(() => selectedAssets.map((item) => resolveAssetSelectionKey(item)), [selectedAssets]);
    ...
    const handleGridSelect = (asset: Asset) => {
        if (_multiple) {
            setSelectedAssets((prev) => toggleSelectedAssets(prev, asset));
            return;
        }
    ...
    const handleConfirm = useCallback(async () => {
        if (tab === 'document' && docSelection) {
            onConfirm([createDocumentSelectionAsset(docSelection)]);
            onClose();
            return;
        }

        const confirmedAssets = _multiple
            ? selectedAssets
        ...
        onConfirm(confirmedAssets);
        onClose();
    }, [docSelection, domain, onClose, onConfirm, projectReference, selectedAsset, selectedAssets, tab, _multiple]);

    const handleDelete = async (asset: Asset) => {
        if (await runtime.dialog.confirm(`Are you sure you want to delete "${asset.name}"?`, 'Delete Asset', 'warning')) {
                                        ...
                                        <div className="h-full bg-[#111] overflow-y-auto p-0 relative">
                                            <div className="absolute inset-0">
                                                <AssetGrid
                                                    ...
                                                    selectedAssetIds={_multiple ? selectedAssetIds : (selectedAsset ? [resolveAssetSelectionKey(selectedAsset)] : [])}
...
export const ChooseAssetModalContent: React.FC<ChooseAssetModalContentProps> = (props) => (
    <AssetStoreProvider
        initialAllowedTypes={props.accepts}
        domain={props.domain}
        projectReference={props.projectReference}
    >
        <ChooseAssetModalPanel {...props} />
    </AssetStoreProvider>
);

export default ChooseAssetModalContent;
