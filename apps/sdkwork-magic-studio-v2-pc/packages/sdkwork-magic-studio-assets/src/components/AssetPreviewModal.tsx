...
export const AssetPreviewModal: React.FC<AssetPreviewModalProps> = ({ asset, onClose }) => {
  ...
  const hasRenderableUrl = !!previewUrl && isRenderableAssetUrl(previewUrl);
  const previewTypeSource = previewUrl || asset.path;
  const isImage = asset.type === 'image' || /\.(png|jpg|jpeg|webp|gif|svg)$/i.test(previewTypeSource);
  const isVideo = asset.type === 'video' || /\.(mp4|mov|webm|avi|mkv|m4v)$/i.test(previewTypeSource);
