...
export interface MagicStudioServerClientErrorOptions {
  status: number;
  /** Numeric SDKWork result code (e.g. 40401 for NOT_FOUND). */
  code?: number;
  detail?: string;
  retryable?: boolean;
...
export class MagicStudioServerClientError extends Error {
  ...
  constructor(message: string, options: MagicStudioServerClientErrorOptions) {
    ...
    this.requestId = options.requestId ?? options.traceId;
    this.traceId = options.traceId ?? options.requestId;
...
async function readJson<T>(
  ...
  if (!response.ok) {
    // Handle standard SDKWork problem+json (SdkWorkProblemDetail) format
    const problem =
      payload && typeof payload === 'object' && 'code' in payload && 'traceId' in payload
        ? (payload as import('@sdkwork/magic-studio-host-types').SdkWorkProblemDetail)
        : null;
    // Handle legacy error envelope format for backward compatibility
    const legacyProblem =
      payload && typeof payload === 'object' && 'error' in payload
        ? (payload as { error: { code?: string; message?: string; detail?: string; retryable?: boolean; fieldErrors?: Record<string, string> }; requestId?: string; timestamp?: string })
        : null;

    const message =
      problem?.detail
      || problem?.title
      || legacyProblem?.error?.message
      || (typeof payload === 'string' && payload.trim())
      || `HTTP ${response.status} for ${url}`;

    throw new MagicStudioServerClientError(message, {
      status: problem?.status ?? response.status,
      code: problem?.code ?? (legacyProblem?.error?.code ? Number(legacyProblem.error.code) || undefined : undefined),
      detail: problem?.detail ?? legacyProblem?.error?.detail,
      retryable: legacyProblem?.error?.retryable,
      traceId: problem?.traceId ?? legacyProblem?.requestId,
      timestamp: legacyProblem?.timestamp,
      fieldErrors: legacyProblem?.error?.fieldErrors,
    });
  }
