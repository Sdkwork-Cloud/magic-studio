...
use super::service_utils::{normalize_optional_text, normalize_text, current_time_millis, current_timestamp, require_non_empty_text};
...
impl FileBackedCreationHistoryService {
    ...
    fn persist_document(&self, document: &CreationHistoryDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_string_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize creation history document: {error}"),
            )
        })?;
...
impl CreationHistoryService for FileBackedCreationHistoryService {
    ...
    fn set_history_entry_favorite(
        ...
        let canonical_entry_id = imported_entry
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("creation history entry {normalized_entry_id} was not found"),
                )
            })?;
...
fn parse_generation_product(value: &str) -> ServerResult<GenerationProduct> {
    match value {
        ...
        _ => Err(ServerError::bad_request(format!("unsupported creation history product {value}"),
        )),
    }
}
