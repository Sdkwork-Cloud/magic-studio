...
impl FileBackedCreationBatchService {
    ...
    fn persist_document(&self, document: &CreationBatchDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_string_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize creation batches: {error}"),
            )
        })?;
    ...
    fn find_batch_index(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("creation batch {batch_key} was not found"),
                )
            })
    }
    ...
    fn normalize_batch_items_from_input(
        ...
        for item in items {
            ...
            if !seen.insert(item_id.clone()) {
                return Err(ServerError::bad_request(format!("duplicate creation batch item id {item_id}"),
                ));
            }
    ...
    fn read_linked_task(
        ...
        match task_family {
            ...
            _ => Err(ServerError::bad_request("request validation failed")),
        }
    }
}
...
impl CreationBatchService for FileBackedCreationBatchService {
    ...
    fn delete_batch(&self, batch_key: &str) -> ServerResult<()> {
        ...
        if document.batches.len() == before_len {
            return Err(ServerError::not_found(format!("creation batch {batch_key} was not found"),
            ));
        }
    ...
    fn update_batch_item_status(
        ...
        let linked_task =
            match (input.task_family.as_deref(), input.task_id.as_deref()) {
                (None, None) => None,
                (Some(_), None) | (None, Some(_)) => return Err(ServerError::bad_request("request validation failed")),
...
fn find_batch_item_index(batch: &CreationBatchRecord, item_key: &str) -> ServerResult<usize> {
    ...
    batch
        ...
        .ok_or_else(|| {
            ServerError::not_found(format!("creation batch item {item_key} was not found"),
            )
        })
}
