...
impl PolicyValidationResult {
    ...
    pub fn ensure_allowed(self) -> ServerResult<()> {
        if self.allowed {
            return Ok(());
        }

        Err(ServerError::forbidden(self.reason
                .unwrap_or_else(|| "operation blocked by policy".to_string()),).with_tag(self.code.unwrap_or_else(|| "POLICY_DENIED".to_string())))
    }
}
...
impl PolicyService {
    ...
    ) -> ServerResult<PolicyValidationResult> {
    ...
    pub fn validate_command(&self, name: String) -> PolicyValidationResult {
...
fn to_absolute_path(input: &str) -> ServerResult<PathBuf> {
    ...
    let current_dir = env::current_dir().map_err(|error| {
        ServerError::internal(format!("failed to resolve current dir: {error}"),
        )
    })?;
