...
impl FileBackedMagicCutService {
    ...
    fn read_document(path: &Path, error_prefix: &str) -> ServerResult<Value> {
        let contents = fs::read_to_string(path).map_err(|error| {
            ServerError::internal(format!("failed to read {}: {error}", path.display()),).with_tag(format!("{error_prefix}_READ_FAILED"))
        })?;

        serde_json::from_str::<Value>(&contents).map_err(|error| {
            ServerError::internal(format!("failed to parse {}: {error}", path.display()),).with_tag(format!("{error_prefix}_PARSE_FAILED"))
        })
    }
    ...
    fn write_document(path: &Path, document: &Value, error_prefix: &str) -> ServerResult<()> {
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize document {}: {error}", path.display()),).with_tag(format!("{error_prefix}_SERIALIZE_FAILED"))
        })?;

        fs::write(path, contents).map_err(|error| {
            ServerError::internal(format!("failed to write {}: {error}", path.display()),).with_tag(format!("{error_prefix}_WRITE_FAILED"))
        })
    }
    ...
    fn read_document_by_key(
        ...
        let path = self
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("{entity_name} {trimmed_key} was not found"),
                )
            })?;
        let document = Self::read_document(&path, "MAGICCUT_DOCUMENT")?;
        Ok((path, document))
...
impl MagicCutService for FileBackedMagicCutService {
    fn list_projects(&self, query: MagicCutListQuery) -> ServerResult<MagicCutListResult> {
        let _guard = self.lock.lock().map_err(|_| {
            ServerError::internal("failed to acquire magiccut lock")
        })?;
    ...
    fn delete_project(&self, project_key: &str) -> ServerResult<bool> {
        ...
        let path = self
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("magiccut project {project_key} was not found"),
                )
            })?;

        fs::remove_file(path).map_err(|error| {
            ServerError::internal(format!("failed to delete magiccut project {project_key}: {error}"),
            )
        })?;
        Ok(true)
    }
    ...
    fn delete_template(&self, template_key: &str) -> ServerResult<bool> {
        ...
        let path = self
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("magiccut template {template_key} was not found"),
                )
            })?;

        fs::remove_file(path).map_err(|error| {
            ServerError::internal(format!("failed to delete magiccut template {template_key}: {error}"),
            )
        })?;
        Ok(true)
    }
...
fn require_object(value: Value, code: &str) -> ServerResult<Map<String, Value>> {
    match value {
        ...
        _ => Err(ServerError::bad_request("request validation failed").with_tag(code)),
    }
}
