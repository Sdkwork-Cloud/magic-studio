...
impl FileBackedTradeCommerceService {
    ...
    fn persist_to_disk(&self, document: &TradeCommerceDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize trade commerce registry: {error}"),
            )
        })?;
    ...
    fn read_order_record<'a>(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("trade order {trimmed} was not found"),
                )
            })
    }
    ...
    fn read_payment_record<'a>(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("trade payment {trimmed} was not found"),
                )
            })
    }
...
fn normalize_wallet_payment_amount(
    ...
    if let Some(amount) = value {
        if amount < 0 {
            return Err(ServerError::bad_request(format!("{field_name} must be greater than or equal to zero"),
            ));
        }
    }
    Ok(value)
...
fn settle_payment_success(
    ...
    let payment_index = document
        ...
        .ok_or_else(|| {
            ServerError::not_found(format!("trade payment {payment_uuid} was not found"),
            )
        })?;
