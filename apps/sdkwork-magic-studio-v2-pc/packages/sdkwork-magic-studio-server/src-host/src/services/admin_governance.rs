...
impl FileBackedAdminGovernanceService {
    ...
    fn persist_workspace_release_retention_run_registry(
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize workspace release retention runs: {error}"),
            )
        })?;
    ...
    fn persist_workspace_release_retention_schedule_registry(
        ...
        self.ensure_admin_governance_root_dir()?;
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize workspace release retention schedules: {error}"),
            )
        })?;
    ...
    fn persist_workspace_release_retention_exception_registry(
        ...
        self.ensure_admin_governance_root_dir()?;
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize workspace release retention exceptions: {error}"),
            )
        })?;
    ...
    fn persist_workspace_release_retention_policy_rollout_registry(
        ...
        self.ensure_admin_governance_root_dir()?;
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize workspace release retention policy rollouts: {error}"),
            )
        })?;
    ...
    fn persist_execution_governance_registry(
        ...
        self.ensure_admin_governance_root_dir()?;
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize execution governance registry: {error}"),
            )
        })?;
    ...
    fn normalize_scope_ids(
        ...
        for value in values {
            ...
            if trimmed.is_empty() {
                return Err(ServerError::bad_request(format!("{field_name} must not contain empty values"),
                ));
            }
    ...
    fn normalize_retention_timestamp(&self, value: &str, field_name: &str) -> ServerResult<String> {
        ...
        OffsetDateTime::parse(&normalized, &Rfc3339).map_err(|error| {
            ServerError::bad_request(format!("{field_name} must be a valid RFC3339 timestamp: {error}"),
            )
        })?;
        Ok(normalized)
    }
    ...
    fn normalize_rollout_stage_size(&self, value: Option<u64>) -> ServerResult<Option<u64>> {
        match value {
            Some(0) => Err(ServerError::bad_request("request validation failed")),
    ...
    fn normalize_rollout_stage_interval_minutes(
        ...
        match value {
            Some(_) if stage_size.is_none() => Err(ServerError::bad_request("request validation failed")),
            Some(0) => Err(ServerError::bad_request("request validation failed")),
            Some(value) => Ok(Some(value)),
            None => Ok(None),
        }
            ...
            match &project_result {
                ...
                Err(error) => {
                    ...
                    results.push(AdminWorkspaceReleaseRetentionProjectRunRecord {
                        ...
                        message: Some(error.detail.clone()),
                        policy: None,
                        stats_before: None,
                        stats_after: None,
...
impl AdminGovernanceService for FileBackedAdminGovernanceService {
    ...
    fn read_workspace_release_retention_run(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("workspace release retention run {normalized_run_id} was not found"),
                )
            })
    }
...
fn find_execution_capability<'a>(
    ...
    execution_capabilities
        ...
        .ok_or_else(|| {
            ServerError::not_found(format!("execution provider {normalized_provider_key} was not found"),
            )
        })
}
...
fn validate_retention_window(starts_at: &str, ends_at: &str) -> ServerResult<()> {
    let starts_at = OffsetDateTime::parse(starts_at, &Rfc3339).map_err(|error| {
        ServerError::bad_request(format!("startsAt must be a valid RFC3339 timestamp: {error}"),
        )
    })?;
    let ends_at = OffsetDateTime::parse(ends_at, &Rfc3339).map_err(|error| {
        ServerError::bad_request(format!("endsAt must be a valid RFC3339 timestamp: {error}"),
        )
    })?;
...
fn timestamp_after_minutes(reference: &str, minutes: u64) -> ServerResult<String> {
    let reference_time = OffsetDateTime::parse(reference, &Rfc3339).map_err(|error| {
        ServerError::internal(format!("failed to parse timestamp {reference}: {error}"),
        )
    })?;
    ...
    (reference_time + TimeDuration::minutes(offset_minutes))
        .format(&Rfc3339)
        .map_err(|error| {
            ServerError::internal(format!("failed to format retention timestamp: {error}"),
            )
        })
}
