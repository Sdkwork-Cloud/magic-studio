...
impl FileBackedCreationTemplateService {
    ...
    fn persist_document(&self, document: &CreationTemplateDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_string_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize creation templates: {error}"),
            )
        })?;
    ...
    fn find_template<'a>(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("creation template {template_key} was not found"),
                )
            })
    }
    ...
    fn normalize_steps(
        ...
        for step in steps {
            ...
            if !seen.insert(step_id.clone()) {
                return Err(ServerError::bad_request(format!("duplicate creation template step id {step_id}"),
                ));
            }
...
impl CreationTemplateService for FileBackedCreationTemplateService {
    ...
    fn delete_template(&self, template_key: &str) -> ServerResult<()> {
        ...
        if document.templates.len() == before_len {
            return Err(ServerError::not_found(format!("creation template {template_key} was not found"),
            ));
        }
...
fn resolve_creation_template_default_step_id(
    ...
    Err(ServerError::bad_request(format!("defaultStepId {requested} does not match any template step"),
    ))
}
...
fn find_creation_template_step<'a>(
    ...
    steps
        ...
        .ok_or_else(|| {
            ServerError::not_found(format!("creation template step {step_id} was not found"),
            )
        })
}
