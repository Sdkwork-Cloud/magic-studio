...
use sdkwork_utils_rust::id;

use crate::response::{ServerError, ServerResult};

use super::app_storage::AppStoragePaths;

use super::service_utils::{normalize_optional_text, current_timestamp};
...
impl FileBackedDriveService {
    ...
    fn acquire_lock(&self) -> ServerResult<MutexGuard<'_, ()>> {
        self.lock.lock().map_err(|_| {
            ServerError::internal("drive registry lock was poisoned")
        })
    }
    ...
    fn load_from_disk(&self) -> ServerResult<DriveRegistryDocument> {
        ...
        let mut document =
            serde_json::from_str::<DriveRegistryDocument>(&contents).map_err(|error| {
                ServerError::internal(format!("failed to parse drive registry {}: {error}", path.display()),
                )
            })?;
    ...
    fn persist_to_disk(&self, document: &DriveRegistryDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize drive registry: {error}"),
            )
        })?;
    ...
    fn build_item_path_inner(
        ...
        if !visited.insert(item_id.to_string()) {
            return Err(ServerError::internal(format!("detected a drive parent cycle at item {item_id}"),
            ));
        }
    ...
    fn read_item<'a>(
        ...
        items
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("drive item {item_key} was not found"),
                )
            })
    }
    ...
    fn read_parent_folder<'a>(
        ...
        if parent.kind != DriveItemKind::Folder {
            return Err(ServerError::bad_request(format!("{field_name} must reference a folder"),
            ));
        }
    ...
    fn resolve_parent_from_id_and_path(
        ...
        let by_id = match parent_id {
            Some(parent_id) => Some(
                self.read_parent_folder(
                    items,
                    &require_non_empty_text(parent_id, field_name)?,
                    required_status,
                    field_name,
                )?
                .id
        ...
        let by_path = match parent_path {
            Some(parent_path) => {
                let normalized = normalize_virtual_path(parent_path, field_name,
                )?;
                if normalized == "/" {
                    Some(None)
                } else {
                    let item_id = self
                        ...
                        .ok_or_else(|| {
                            ServerError::not_found(format!("{field_name} path {normalized} was not found"),
                            )
                        })?;
        ...
        if let (Some(by_id), Some(by_path)) = (by_id.clone(), by_path.clone()) {
            if Some(by_id.clone()) != by_path {
                return Err(ServerError::bad_request(format!("{field_name} and parentPath reference different folders"),
                ));
            }
        }
    ...
    fn ensure_name_available(
        ...
        for candidate in items {
            ...
            if candidate.name.eq_ignore_ascii_case(name) {
                return Err(ServerError::conflict(format!("an item named {name} already exists in the target folder"),
                ));
            }
        }
        Ok(())
    ...
    fn decode_request_content(
        ...
        match encoding.unwrap_or(DriveContentEncoding::Utf8) {
            ...
            DriveContentEncoding::Base64 => encoding::base64_decode(content).ok_or_else(|| {
                ServerError::bad_request(format!("{field_name} must be valid base64"),
                )
            }),
        }
    }
    ...
    fn encode_content_response(
        ...
        match preferred_encoding {
            Some(DriveContentEncoding::Base64) => {
                (DriveContentEncoding::Base64, encoding::base64_encode(&bytes))
            }
            ...
            None => match std::str::from_utf8(bytes) {
                ...
                Err(_) => (DriveContentEncoding::Base64, encoding::base64_encode(&bytes)),
            },
        }
    }
    ...
    fn read_file_bytes(&self, item_id: &str) -> ServerResult<Vec<u8>> {
        fs::read(self.drive_file_path(item_id)).map_err(|error| {
            ServerError::internal(format!("failed to read drive file bytes for {item_id}: {error}"),
            )
        })
    }
    ...
    fn write_file_bytes(&self, item_id: &str, bytes: &[u8]) -> ServerResult<()> {
        ...
        fs::write(self.drive_file_path(item_id), bytes).map_err(|error| {
            ServerError::internal(format!("failed to write drive file bytes for {item_id}: {error}"),
            )
        })
    }
    ...
    fn normalize_item_selection(
        ...
        for item_id in item_ids {
            let item_id = require_non_empty_text(item_id, "itemIds[]")?;
...
impl DriveService for FileBackedDriveService {
    ...
    fn import_file(&self, input: DriveImportFileRequest) -> ServerResult<DriveItemRecord> {
        ...
        let metadata = fs::metadata(source_path_ref).map_err(|error| {
            ServerError::bad_request(format!("sourcePath {source_path} could not be read: {error}"),
            )
        })?;
        ...
        fs::copy(source_path_ref, self.drive_file_path(&item_id)).map_err(|error| {
            ServerError::internal(format!("failed to import drive file from {source_path}: {error}"),
            )
        })?;
    ...
    fn rename_item(&self, input: DriveRenameRequest) -> ServerResult<DriveItemRecord> {
        let _guard = self.acquire_lock()?;
        let mut document = self.load_from_disk()?;
        let item_key = require_non_empty_text(&input.item_id, "itemId")?;
        let name = require_drive_name(&input.name, "name")?;
        let now = current_timestamp();
        let parent_id;
...
fn require_non_empty_text(value: &str, field_name: &str) -> ServerResult<String> {
...
fn require_drive_name(value: &str, field_name: &str) -> ServerResult<String> {
    let name = require_non_empty_text(value, field_name)?;
    if name == "." || name == ".." {
        return Err(ServerError::bad_request(format!("{field_name} contains an invalid drive segment"),
        ));
    }
    if name.chars().any(is_invalid_drive_name_char) {
        return Err(ServerError::bad_request(format!("{field_name} contains characters not allowed in drive names"),
        ));
    }
    Ok(name)
}
...
fn normalize_virtual_path(value: &str, field_name: &str) -> ServerResult<String> {
    let trimmed = value.trim();
    if trimmed.is_empty() {
        return Err(ServerError::bad_request(format!(
            "{field_name} must not be empty"
...
fn next_entity_id(prefix: &str) -> String {
    ...
    format!("{prefix}-{}-{sequence}", id::uuid())
}
