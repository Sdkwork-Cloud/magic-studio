...
use super::service_utils::{normalize_optional_text, current_timestamp, to_client_entity_uuid, require_non_empty_text, normalize_text};
...
impl FileBackedVoiceService {
    ...
    fn persist_to_disk(&self, document: &VoiceRegistryDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_string_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize voice registry: {error}"),
            )
        })?;
    ...
    fn create_clone_task_record(
        ...
        let preview_text = input.preview_text.and_then(normalize_text);
        ...
        Ok(VoiceCloneTaskRecord {
            ...
            status: GenerationTaskStatus::Failed,
            language,
            model: input.model.and_then(normalize_text),
            provider: "magic-studio-server".to_string(),
            remote_job_id: None,
            progress: Some(0.0),
            idempotency_key: input.idempotency_key.and_then(normalize_text),
            sample_audio,
            sample_audio_url,
            preview_text,
...
impl VoiceService for FileBackedVoiceService {
    ...
    fn read_voice_speaker(&self, speaker_id: &str) -> ServerResult<VoiceSpeakerRecord> {
        ...
        self.resolve_market_voices()
            ...
            .find(|voice| matches_speaker_key(voice, &normalized_speaker_id))
            .ok_or_else(|| {
                ServerError::not_found(format!("voice speaker {normalized_speaker_id} was not found"),
                )
            })
    }
    ...
    fn create_custom_voice(
        ...
        let mut voice = VoiceSpeakerRecord {
            ...
            provider_voice_id: input.provider_voice_id.and_then(normalize_text),
            preview_url: input.preview_url.and_then(normalize_text),
            preview_text: input.preview_text.and_then(normalize_text),
            avatar_url: input.avatar_url.and_then(normalize_text),
            description: input.description.and_then(normalize_text),
    ...
    fn update_custom_voice(
        ...
        let voice = self
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("custom voice speaker {normalized_speaker_id} was not found"),
                )
            })?;
    ...
    fn read_clone_task(&self, task_id: &str) -> ServerResult<VoiceCloneTaskRecord> {
        ...
        self.read_clone_task_from_document(&document, &normalized_task_id)
            .cloned()
            .ok_or_else(|| {
                ServerError::not_found(format!("voice clone task {normalized_task_id} was not found"),
                )
            })
    }
    ...
    fn list_clone_tasks(&self, query: VoiceCloneTaskListQuery) -> ServerResult<VoiceCloneTaskPage> {
        ...
        let speaker_id_filter = query.speaker_id.and_then(normalize_text);
    ...
    fn read_speech_task(&self, task_id: &str) -> ServerResult<GenerationTaskRecord> {
        ...
        self.read_speech_task_from_document(&document, &normalized_task_id)
            .cloned()
            .ok_or_else(|| {
                ServerError::not_found(format!("voice speech task {normalized_task_id} was not found"),
                )
            })
    }
    ...
    fn upsert_speech_task(
        ...
        let remote_job_id = remote_job_id.and_then(normalize_text).or_else(|| {
            existing
                .as_ref()
        ...
        let description = description.and_then(normalize_text).or_else(|| {
            existing
                .as_ref()
                .and_then(|task| task_parameter_string(task, "description"))
        });
        let voice_mode = mode.and_then(normalize_text).or_else(|| {
            existing
                .as_ref()
                .and_then(|task| task_parameter_string(task, "mode"))
        });
        let input_method = input_method.and_then(normalize_text).or_else(|| {
            existing
                .as_ref()
        ...
        let updated_at = updated_at
            .and_then(normalize_text)
            .or_else(|| completed_at.clone().and_then(normalize_text))
            .or_else(|| cancelled_at.clone().and_then(normalize_text))
...
fn normalize_voice_record(voice: &mut VoiceSpeakerRecord, source: VoiceSpeakerSource) {
    ...
    voice.provider_voice_id = voice
        ...
        .clone()
        .and_then(normalize_text);
    voice.preview_url = voice.preview_url.clone().and_then(normalize_text);
    voice.preview_text = voice.preview_text.clone().and_then(normalize_text);
    voice.avatar_url = voice.avatar_url.clone().and_then(normalize_text);
    voice.description = voice.description.clone().and_then(normalize_text);
...
fn normalize_clone_task_record(task: &mut VoiceCloneTaskRecord) {
    task.speaker_name = task.speaker_name.clone().and_then(normalize_text);
    task.language = normalize_non_empty_with_fallback(task.language.clone(), "en-US");
    task.model = task.model.clone().and_then(normalize_text);
    task.provider = normalize_non_empty_with_fallback(task.provider.clone(), "magic-studio-server");
    task.remote_job_id = task.remote_job_id.clone().and_then(normalize_text);
    ...
    task.sample_audio_url = task
        ...
        .clone()
        .and_then(normalize_text);
    task.preview_text = task.preview_text.clone().and_then(normalize_text);
    task.preview_audio_url = task
        .preview_audio_url
        .clone()
        .and_then(normalize_text);
    task.error_code = task.error_code.clone().and_then(normalize_text);
    task.error_message = task.error_message.clone().and_then(normalize_text);
    task.provider_payload = normalize_json_map(task.provider_payload.clone());
    task.completed_at = task.completed_at.clone().and_then(normalize_text);
    task.cancelled_at = task.cancelled_at.clone().and_then(normalize_text);
}
...
fn normalize_voice_generation_artifact(
    ...
    Ok(GenerationArtifactRecord {
        ...
        asset_id: artifact.asset_id.and_then(normalize_text),
        asset_uuid: artifact.asset_uuid.and_then(normalize_text),
        ...
        primary_resource_uuid: artifact
            ...
            .and_then(normalize_text),
        resource_view_id: artifact.resource_view_id.and_then(normalize_text),
        resource_view_uuid: artifact
            .resource_view_uuid
            .and_then(normalize_text),
        url,
        poster_url: artifact.poster_url.and_then(normalize_text),
        mime_type: artifact.mime_type.and_then(normalize_text),
...
fn normalize_voice_task_completed_at(
    ...
    match status {
        ...
        | GenerationTaskStatus::Cancelled => input
            .and_then(normalize_text)
            .or_else(|| existing.and_then(normalize_text))
