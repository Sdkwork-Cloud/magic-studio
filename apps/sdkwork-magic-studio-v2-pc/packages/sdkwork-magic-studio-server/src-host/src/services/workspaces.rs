...
impl FileBackedWorkspaceService {
    ...
    fn persist_to_disk(&self, document: &WorkspaceRegistryDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize workspace registry: {error}"),
            )
        })?;
    ...
    fn persist_project_git_sync_registry(
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize project git sync registry: {error}"),
            )
        })?;
    ...
    fn persist_project_release_registry(
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize project release registry: {error}"),
            )
        })?;
    ...
    fn persist_project_release_retention_policy(
        ...
        let contents = serde_json::to_vec_pretty(&document).map_err(|error| {
            ServerError::internal(format!("failed to serialize project release retention policy: {error}"),
            )
        })?;
    ...
    fn find_project_git_sync_record(
        ...
        registry
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("project git sync {normalized_sync_key} was not found"),
                )
            })
    }
    ...
    fn find_project_release_record(
        ...
        registry
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("project release {normalized_release_key} was not found"),
                )
            })
    }
    ...
    fn resolve_project_release_prune_candidates(
        ...
        for release_key in release_ids {
            ...
            let Some(record) = records.iter().find(|record| {
                ...
                return Err(ServerError::not_found(format!("project release {normalized_release_key} was not found"),
                ));
            };
        ...
        if policy.enabled {
            ...
            let applied_at =
                OffsetDateTime::parse(&applied_at_text, &Rfc3339).map_err(|error| {
                    ServerError::internal(format!("failed to parse retention apply timestamp: {error}"),
                    )
                })?;
    ...
    fn read_workspace_record<'a>(
        ...
        workspaces
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("workspace {normalized_key} was not found"),
                )
            })
    }
    ...
    fn read_project_record<'a>(
        ...
        workspace
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("project {normalized_key} was not found"),
                )
            })
    }
...
impl WorkspaceService for FileBackedWorkspaceService {
    ...
    fn delete_workspace(&self, workspace_key: &str) -> ServerResult<bool> {
        ...
        let Some(index) = document
            ...
            return Err(ServerError::not_found(format!("workspace {normalized_key} was not found"),
            ));
        };
    ...
    fn update_project(
        ...
        let Some(workspace_index) = document
            ...
            return Err(ServerError::not_found(format!("workspace {normalized_workspace_key} was not found"),
            ));
        };
        ...
        let response = {
            ...
            let Some(project_index) = workspace
                ...
                return Err(ServerError::not_found(format!("project {normalized_project_key} was not found"),
                ));
            };
...
fn execute_project_git_sync(
    ...
    if !project_dir.exists() {
        return Err(ServerError::not_found(format!("project directory {} was not found", project_dir.display()),
        ));
    }
...
fn create_project_release_archive(source_root: &Path, artifact_path: &Path) -> ServerResult<usize> {
    ...
    for (name, path) in file_entries {
        archive.start_file(&name, options).map_err(|error| {
            ServerError::internal(format!("failed to create archive entry {name}: {error}"),
            )
        })?;
        ...
        archive.write_all(&buffer).map_err(|error| {
            ServerError::internal(format!("failed to write project release archive entry {name}: {error}"),
            )
        })?;
        file_count += 1;
    }
