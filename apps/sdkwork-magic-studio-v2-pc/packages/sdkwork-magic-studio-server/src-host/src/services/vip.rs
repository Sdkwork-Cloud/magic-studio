...
impl FileBackedVipService {
    ...
    fn load_from_disk(&self) -> ServerResult<VipDocument> {
        ...
        match fs::read_to_string(path) {
            Ok(contents) => {
                let mut document =
                    serde_json::from_str::<VipDocument>(&contents).map_err(|error| {
                        ServerError::internal(format!("failed to parse vip registry {}: {error}", path.display()),
                        )
                    })?;
            ...
            Err(error) => Err(ServerError::internal(format!("failed to read vip registry {}: {error}", path.display()),
            )),
        }
    }
    ...
    fn persist_to_disk(&self, document: &VipDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_string_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize vip registry: {error}"),
            )
        })?;
    ...
    fn lock_document(&self) -> ServerResult<MutexGuard<'_, ()>> {
        self.lock.lock().map_err(|_| {
            ServerError::internal("vip registry lock is poisoned")
        })
    }
    ...
    fn plan_by_tier<'a>(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("vip plan for tier {} was not found", tier_as_str(tier)),
                )
            })
    }
    ...
    fn read_subscription_record_mut<'a>(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("vip subscription {trimmed} was not found"),
                )
            })
    }
}
