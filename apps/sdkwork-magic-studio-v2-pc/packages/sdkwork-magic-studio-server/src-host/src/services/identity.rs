...
impl FileBackedIdentityService {
    ...
    fn persist_to_disk(&self, document: &IdentityRegistryDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(format!("failed to serialize identity registry: {error}"),
            )
        })?;
    ...
    fn require_user<'a>(
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("identity user {user_id} was not found"),
                )
            })
    }
    ...
    fn decode_totp_secret(secret_base32: &str) -> ServerResult<Vec<u8>> {
        BASE32_NOPAD
            ...
            .map_err(|error| {
                ServerError::internal(format!("failed to decode TOTP secret: {error}"),
                )
            })
    }
    ...
    fn generate_totp_for_counter(secret_base32: &str, counter: u64) -> ServerResult<String> {
        ...
        let mut mac = HmacSha1::new_from_slice(&secret).map_err(|error| {
            ServerError::internal(format!("failed to initialize TOTP HMAC: {error}"),
            )
        })?;
    ...
    fn consume_enabled_two_factor_code(
        ...
        Err(ServerError::forbidden("access was forbidden"))
    }
    ...
    fn ensure_unique_user_fields(
        ...
        for user in &document.users {
            ...
            if user.username.eq_ignore_ascii_case(username) {
                return Err(ServerError::conflict(format!("username {username} already exists"),
                ));
            }
            ...
            if let Some(email) = email {
                if user
                    ...
                    .unwrap_or(false)
                {
                    return Err(ServerError::conflict(format!("email {email} already exists"),
                    ));
                }
            }
            ...
            if let Some(phone) = phone {
                if user.phone.as_deref() == Some(phone) {
                    return Err(ServerError::conflict(format!("phone {phone} already exists"),
                    ));
                }
            }
        }
    ...
    fn resolve_reset_target<'a>(
        ...
        Err(ServerError::not_found(format!("identity account {account} was not found"),
        ))
    }
}
...
impl IdentityService for FileBackedIdentityService {
    ...
    fn login(&self, input: AuthLoginRequest) -> ServerResult<AuthSessionRecord> {
        ...
        let user_id = {
            let user = self
                ...
                .ok_or_else(|| {
                    ServerError::not_found(format!("identity account {username} was not found"),
                    )
                })?;
    ...
    fn login_with_phone(&self, input: AuthPhoneLoginRequest) -> ServerResult<AuthSessionRecord> {
        ...
        let user_id = self
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("identity phone account {phone} was not found"),
                )
            })?
            .id
            .clone();
    ...
    fn read_qr_code_status(&self, qr_key: &str) -> ServerResult<AuthQrCodeStatusResultRecord> {
        ...
        let qr_code = document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("auth qr code {qr_key} was not found"),
                )
            })?;
    ...
    fn update_user_address(
        ...
        let index = user
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("user address {address_id} was not found"),
                )
            })?;
        let now = current_timestamp();
    ...
    fn unbind_user_platform(&self, platform: &str) -> ServerResult<()> {
        ...
        let index = user
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("user binding {platform} was not found"),
                )
            })?;
    ...
    fn revoke_user_session(&self, session_id: &str) -> ServerResult<()> {
        ...
        let session = document
            ...
            .ok_or_else(|| {
                ServerError::not_found(format!("user session {session_id} was not found"),
                )
            })?;
    ...
    fn revoke_user_device(&self, device_id: &str) -> ServerResult<()> {
        ...
        if !affected {
            return Err(ServerError::not_found(format!("trusted device {device_id} was not found"),
            ));
        }
...
fn session_required_error() -> ServerError {
    ServerError::forbidden("access was forbidden")
}
