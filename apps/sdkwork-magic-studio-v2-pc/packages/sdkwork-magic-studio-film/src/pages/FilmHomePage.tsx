...
import { ModelSelector, AspectRatioSelector, GalleryCard } from '@sdkwork/magic-studio-commons';
import type { Resolution } from '@sdkwork/magic-studio-commons/components';
import { GenerationPreview } from '@sdkwork/magic-studio-generation-history';
...
import {
    buildFilmHomeAttachment,
    type FilmHomeAttachment
} from '../utils/filmHomeAttachment';
...
const classifyFilmHomeFile = (
    ...
    if (SCRIPT_EXTS.has(ext)) {
        return {
            ...
            importType: ['txt', 'md', 'markdown', 'rtf', 'fountain'].includes(ext) ? 'text' : 'file',
            ext
        };
    }
...
const FilmHomePageContent: React.FC = () => {
    ...
    // Hero Input State
    const activeTab = 'short_drama' as PortalTab;
    ...
    }, [activeModel, activeTab, capabilitySnapshot, currentProviders]);
    ...
    const effectiveActiveStyle = useMemo(
        () => resolvePreferredSelectionId(activeStyleOptions, activeStyle, ''),
    ...
    const effectiveAspectRatio = useMemo(
        () => resolvePreferredSelectionValue(activeAspectRatioOptions, aspectRatio, '16:9') as FilmAspectRatio,
        ...
        return (
            <div className="flex items-center gap-2 whitespace-nowrap">
                ...
                <AspectRatioSelector 
                    ...
                    resolution={effectiveResolution as Resolution}
                             ...
                             <div className="absolute bottom-full left-0 mb-2 w-32 bg-[#18181b] border border-[#27272a] rounded-xl shadow-xl p-1 z-50 animate-in fade-in zoom-in-95 duration-75">
                                {activeDurationOptions.map(d => (
                                    <button 
                                        ...
                                        className={`w-full text-left px-3 py-1.5 text-xs rounded hover:bg-[#27272a] ${effectiveDuration === d.value ? 'text-blue-400' : 'text-gray-400'}`}
                                    >
                                        {d.label}
                                    </button>
                                ))}
