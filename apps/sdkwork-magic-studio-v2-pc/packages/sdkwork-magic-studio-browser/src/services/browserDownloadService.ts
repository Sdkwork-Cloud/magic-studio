...
import { settingsService } from '@sdkwork/magic-studio-settings';
import { assetBusinessService, ASSET_CATEGORIES } from '@sdkwork/magic-studio-assets/services';
...
class BrowserDownloadService {
    ...
    private async executeDownload(item: DownloadItem) {
        ...
        const configuredDownloadDir = settings?.browser?.downloadPath?.trim();
        const targetDir = configuredDownloadDir || await runtime.system.path('downloads');
        try {
        ...
        const response = await runtime.network.request(item.url, { method: 'GET' });
