...
import { isRenderableAssetUrl, resolveRuntimeMagicStudioAssetUrl } from '../../storage';
...
class AspectRatioService {
    private async resolveRenderableSource(source: string): Promise<string> {
        ...
        try {
            return await resolveRuntimeMagicStudioAssetUrl(getPlatformRuntime(), source);
        } catch (e) {
            logger.warn('[AspectRatioService] Failed to resolve renderable source', e);
            return source;
        }
    }
    ...
    public async getVideoMetadata(source: string | File): Promise<MediaDimensions> {
        const url = source instanceof File
            ...
            : await this.resolveRenderableSource(source);
