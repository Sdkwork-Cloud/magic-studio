import { isRenderableAssetUrl } from '../../storage';

// Stub types - to be replaced with actual @sdkwork/magic-studio-assets types
...
class MediaAnalysisService {
    ...
    public async analyze(url: string, type: AssetType): Promise<MediaAnalysisResult> {
        if (!isRenderableAssetUrl(url)) {
            console.warn('[MediaAnalysis] Received a non-renderable media source. Resolve it before analysis.');
            return { metadata: {} };
        }
            ...
            video.onloadedmetadata = async () => {
                ...
                for (const time of candidates) {
                    try {
                        ...
                        // Ignore seek errors and try the next candidate frame.
                    }
                }
