...
type LocalStorageEntityLike = EntityIdentityLike & {
 ...
 */
export class LocalStorageService<T extends LocalStorageEntityLike> {
    ...
    async findById(id: string): Promise<ServiceResult<T | null>> {
        await this.ensureInitialized();
        const item = this.cache?.find(i => matchesEntityKey(i, id)) || null;
        return Result.success(item);
    }

    async existsById(id: string): Promise<boolean> {
        await this.ensureInitialized();
        return this.cache?.some(i => matchesEntityKey(i, id)) || false;
    }

    async findAllById(ids: string[]): Promise<ServiceResult<T[]>> {
        await this.ensureInitialized();
        const items = this.cache?.filter(i => ids.some(id => matchesEntityKey(i, id))) || [];
    ...
    async save(entity: Partial<T>): Promise<ServiceResult<T>> {
        ...
        const createFallbackIdentityKey = createUuid();
        const hasExplicitId = Object.prototype.hasOwnProperty.call(entity, 'id');
        const hasExplicitUuid = Object.prototype.hasOwnProperty.call(entity, 'uuid');
        let entityKey: string;
        try {
            entityKey = resolveEntityKey(entity);
        } catch {
            entityKey = createFallbackIdentityKey;
        }

        const now = Date.now();
        const existingIndex = this.cache!.findIndex(
            i => entityKeysEqual(i, entity) || matchesEntityKey(i, entityKey)
        );
        let savedEntity: T;

        if (existingIndex >= 0) {
            // Update
            const current = this.cache![existingIndex];
            const updateFallbackIdentityKey = createUuid();
            const explicitId = hasExplicitId ? (entity.id ?? null) : undefined;
            const explicitUuid = hasExplicitUuid ? (entity.uuid ?? null) : undefined;
            savedEntity = {
                ...current,
                ...entity,
                id: typeof explicitId !== 'undefined'
                    ? explicitId
                    : current.id ?? null,
                uuid: explicitUuid
                    ?? current.uuid
                    ?? (explicitId ? deriveClientEntityUuidFromId(explicitId) : undefined)
                    ?? (current.id ? deriveClientEntityUuidFromId(current.id) : undefined)
                    ?? updateFallbackIdentityKey,
                updatedAt: now
            } as unknown as T;
            ...
            savedEntity = {
                ...entity,
                id: typeof explicitId !== 'undefined' ? explicitId : null,
                uuid: explicitUuid
                    ?? (explicitId ? deriveClientEntityUuidFromId(explicitId) : undefined)
                    ?? createFallbackIdentityKey,
    ...
    async deleteById(id: string): Promise<ServiceResult<void>> {
        ...
        this.cache = this.cache!.filter(i => !matchesEntityKey(i, id));
    ...
    async deleteAll(ids: string[]): Promise<ServiceResult<void>> {
        await this.ensureInitialized();
        this.cache = this.cache!.filter(i => !ids.some(id => matchesEntityKey(i, id)));
        await this.persist();
        return Result.success(undefined);
    }
