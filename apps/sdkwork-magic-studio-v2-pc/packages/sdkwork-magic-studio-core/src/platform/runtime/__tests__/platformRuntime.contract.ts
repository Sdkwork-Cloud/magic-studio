...
import { createPlatformRuntime } from '../createPlatformRuntime.ts';
import { isBrowserHostedRuntimeKind } from '../kinds.ts';
...
const run = async (): Promise<void> => {
  ...
  const runtimeTypesSource = fs.readFileSync(new URL('../types.ts', import.meta.url), 'utf8');

  assert(
    /PlatformBridgeCapability/.test(runtimeTypesSource),
    'runtime types should export PlatformBridgeCapability',
  );
  assert(
    /PlatformShellCommandName/.test(runtimeTypesSource),
    'runtime types should export PlatformShellCommandName',
  );
  assert(
    /PlatformShellEventName/.test(runtimeTypesSource),
    'runtime types should export PlatformShellEventName',
  );
  assert(
    !/PlatformNativeBridgeCapability/.test(runtimeTypesSource),
    'runtime types should not export PlatformNativeBridgeCapability',
  );
  assert(isBrowserHostedRuntimeKind('web') === true, 'web should be classified as browser-hosted');
  assert(isBrowserHostedRuntimeKind('server') === true, 'server should be classified as browser-hosted');
  assert(isBrowserHostedRuntimeKind('desktop') === false, 'desktop should not be classified as browser-hosted');
  assert(runtime.bridge.available() === false, 'bridge should be unavailable without invoke adapter');

  let invokeMessage = '';
  try {
    await runtime.bridge.invoke('noop' as never);
  } catch (error) {
    invokeMessage = error instanceof Error ? error.message : String(error);
  }
  assert(
    invokeMessage === '[PlatformRuntime] Bridge invoke is not available on this platform',
    'runtime bridge invoke error should use neutral bridge wording',
  );
  ...
  assert(await runtime.window.isMaximized() === false, 'window maximized probe should proxy platform state');
