...
import type { PlatformShellCommandName, PlatformShellEventName } from './types';
...
export const createPlatformRuntime = (api: PlatformAPI): PlatformRuntime => {
  const bridgeAdapter = api as PlatformAPI & PlatformBridgeAdapter;
  const runtime: PlatformRuntime = {
    raw: api,
    bridge: {
      available: (): boolean => typeof bridgeAdapter.invoke === 'function',
      invoke: async <T = unknown>(
        command: PlatformShellCommandName,
        payload?: Record<string, unknown>,
      ): Promise<T> => {
        if (typeof bridgeAdapter.invoke !== 'function') {
          throw new Error('[PlatformRuntime] Bridge invoke is not available on this platform');
        }
        return bridgeAdapter.invoke<T>(command, payload);
      },
      listen: async <T = unknown>(
        event: PlatformShellEventName,
        callback: (payload: T) => void
      ): Promise<() => void> => {
        if (typeof bridgeAdapter.listen === 'function') {
          return bridgeAdapter.listen<T>(event, callback);
        }
        return () => {};
      }
    },
    ...
    window: {
      ...
      isMaximized: async (): Promise<boolean> => api.isWindowMaximized(),
