...
export interface PlatformVideoToolkit {
  ...
  transcode(inputPath: string, outputPath: string, options?: ToolkitVideoTranscodeOptions): Promise<ToolkitMediaCommandResult>;
  trim(inputPath: string, outputPath: string, options: ToolkitVideoTrimOptions): Promise<ToolkitMediaCommandResult>;
  concat(inputPaths: string[], outputPath: string, options?: ToolkitVideoConcatOptions): Promise<ToolkitMediaCommandResult>;
  extractAudio(inputPath: string, outputPath: string, overwrite?: boolean): Promise<ToolkitMediaCommandResult>;
  createThumbnail(inputPath: string, outputPath: string, options?: ToolkitFrameCaptureOptions): Promise<ToolkitMediaCommandResult>;
}

export interface PlatformAudioToolkit {
  getMetadata(source: ToolkitMediaSource): Promise<ToolkitAudioMetadata>;
  convert(inputPath: string, outputPath: string, options?: ToolkitAudioConvertOptions): Promise<ToolkitMediaCommandResult>;
  normalize(inputPath: string, outputPath: string, options?: ToolkitAudioNormalizeOptions): Promise<ToolkitMediaCommandResult>;
  mix(inputs: ToolkitAudioMixInput[], outputPath: string, options?: ToolkitAudioMixOptions): Promise<ToolkitMediaCommandResult>;
}
...
export interface PlatformMediaToolkit {
...
export interface PlatformToolKit {
  ...
  readonly media: PlatformMediaToolkit;
