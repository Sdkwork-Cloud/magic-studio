...
import type { AnyMediaResource } from '@sdkwork/magic-studio-assets/entities';
import {
    CanvasElement,
    CanvasMediaResource,
    resolveCanvasMediaResourceUrl,
} from '../entities';
import { Popover, AspectRatioSelector, findByIdOrFirst } from '@sdkwork/magic-studio-commons';
import {
    assetService,
    importAssetBySdk,
...
import { ImageModelSelector } from '@sdkwork/magic-studio-image/selectors';
import { VideoModelSelector } from '@sdkwork/magic-studio-video/selectors';
import { generateCanvasNodeMedia } from '../services/canvasGenerationService';
import { textRenderer, TextStyle } from '@sdkwork/magic-studio-magiccut';
import { VideoNode } from './nodes/VideoNode';
import { getCanvasViewportBounds } from '../utils/canvasContainer';
import { toCanvasImportedAssetResource } from '../utils/canvasImportedAssetResource';
...
} from '../utils/canvasReferenceImageAttachment';
...
type CanvasAssetResource = CanvasMediaResource & AnyMediaResource;

const isCanvasAssetResource = (
    resource: CanvasMediaResource | null | undefined,
): resource is CanvasAssetResource => Boolean(resource);

const resolveCanvasAttachmentUrl = (resource: CanvasAssetResource): string =>
    resolveCanvasMediaResourceUrl(resource) || '';
...
export const CanvasNode = React.memo(forwardRef<HTMLDivElement, CanvasNodeProps>(({ 
    ...
    const { type, x, y, width, height, resource, data, selected, color, style } = element;
    const elementKey = resolveCanvasEntityKey(element);
    const { updateElement, viewport, selectedIds, activeBoard } = useCanvasStore();

    const [_isResizing] = useState(false);
    ...
    useEffect(() => {
        ...
        let isMounted = true;
        const resolve = async () => {
            const url = await assetService.resolveAssetUrl({
                ...resource,
                ...
                if (files.length > 0) {
                    const newImages = (await Promise.all(files.map((file) => (
                        importCanvasReferenceImageFile({
                            ...
                            boardUuid: activeBoard?.uuid ?? undefined,
                                elementId: elementKey,
                                source: 'canvas-reference-image-upload',
                            })
                        )))).filter((resource): resource is CanvasAssetResource =>
                            isCanvasAssetResource(resource) && resolveCanvasAttachmentUrl(resource).length > 0
                        );
                    const currentRefs = data?.referenceImages || [];
                    const updatedRefs = [...currentRefs, ...newImages].slice(0, 5);
                    updateElement(elementKey, { data: { ...data, referenceImages: updatedRefs } }, false);
                }
        } catch (e) {
    ...
    const handleRemoveAttachment = (attId: string) => {
         const newRefs = removeCanvasReferenceImageByAttachmentId(
            data?.referenceImages,
            attId
         );
         updateElement(elementKey, { data: { ...data, referenceImages: newRefs } }, false);
    };

    const attachments: InputAttachment[] = (data?.referenceImages || [])
        .filter(isCanvasAssetResource)
        .map((resource, i) => createInputAttachment({
            id: resolveCanvasReferenceImageAttachmentId(resource, i),
            name: `Ref ${i+1}`,
            type: 'image',
            url: resolveCanvasAttachmentUrl(resource)
        }));
    ...
    const handleGenerate = async () => {
        ...
        try {
             ...
             updateElement(elementKey, { data: { ...data, status: 'generating', error: undefined } }, false);
             const newResource = await generateCanvasNodeMedia({
             ...
             if (!newResource) throw new Error("Generation did not return a persisted resource");
            ...
            updateElement(elementKey, { data: { ...data, status: 'failed', error: e.message } });
        } finally {
    ...
    const handleInteractionStart = (e: React.MouseEvent) => {
        ...
        if (!_isResizing && !isEditing) {
            ...
            e.stopPropagation(); 
            if (e.button === 0) onMouseDown(e, elementKey);
        }
    };
        ...
        try {
            ...
            if (files.length > 0) {
                ...
                const resolvedUrl =
                    ...
                    uploaded.path ||
                    '';
                const newResource = toCanvasImportedAssetResource({
                    uploaded,
                    fallbackType: type as 'image' | 'video',
                    resolvedUrl,
                });
                ...
                <div className="min-w-[120px]">
                    {type === 'video' ? (
                        <VideoModelSelector 
                            ...
                            onChange={(m: string) => updateElement(elementKey, { data: { ...data, model: m } })} 
                ...
                <div className="flex items-center gap-1">
                    {type === 'video' ? (
                        <VideoModeDropdown 
                            ...
                            onChange={(m: string) => updateElement(elementKey, { data: { ...data, videoMode: m } })} 
                        />
                    ) : (
                        <AspectRatioSelector 
                            value={(data?.aspectRatio as any) || '1:1'}
                            onChange={(r: string) => updateElement(elementKey, { data: { ...data, aspectRatio: r as any } })}
                            resolution={(data?.resolution as any) || '2k'}
                            onResolutionChange={(r: string) => updateElement(elementKey, { data: { ...data, resolution: r as any } })}
        ...
        >
            {lod === 'detail' && (
                <>
                    ...
                    <div onMouseDown={(e) => { e.stopPropagation(); onConnectStart(e, elementKey, 'in'); }} className={`absolute left-0 top-1/2 -translate-x-1/2 w-3 h-3 bg-white border border-blue-500 rounded-full cursor-crosshair transition-all z-50 shadow-sm hover:scale-150 ${selected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />
                    <div onMouseDown={(e) => { e.stopPropagation(); onConnectStart(e, elementKey, 'out'); }} className={`absolute right-0 top-1/2 translate-x-1/2 w-3 h-3 bg-white border-2 border-blue-500 rounded-full cursor-crosshair transition-all z-50 shadow-sm hover:scale-150 ${selected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />
                        ...
                        >
                            <div style={{ width: `${chatInputWidth}px` }}>
                                <CreationChatInput 
                                    variant="compact"
                                    value={data?.prompt || ''}
                                    onChange={(val: string) => updateElement(elementKey, { data: { ...data, prompt: val } }, false)}
