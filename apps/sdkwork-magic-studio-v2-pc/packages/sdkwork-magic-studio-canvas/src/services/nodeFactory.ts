...
import { textRenderer } from '@sdkwork/magic-studio-magiccut';
...
class TextNodeStrategy implements INodeCreationStrategy {
    ...
    createResource(_id: string, content: string = 'Double click to edit') {
        const identity = createLocalCanvasEntityIdentity();
        return {
            ...identity,
...
class StickyNoteStrategy implements INodeCreationStrategy {
    ...
    createResource(_id: string, content: string = 'New Idea') {
        const identity = createLocalCanvasEntityIdentity();
         return {
            ...identity,
            type: MediaResourceType.TEXT,
...
export class NodeFactory {
    ...
    static create(options: CreateNodeOptions): CanvasElement {
        ...
        let resource: NodeResource = strategy.createResource(identity.uuid, options.content);
        
        // Migrate legacy content option if provided
        if (options.content) {
             if (type === 'image' || type === 'video') {
                if (!resource) resource = strategy.createResource(identity.uuid); // Ensure resource exists
