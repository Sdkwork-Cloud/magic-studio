...
import { CanvasBoard, CanvasElement, Viewport, SnapLine } from '../entities';
import { canvasBusinessService } from '../services/canvasBusinessService';
import { collectAncestorGroupIds, fitGroupsToChildren } from './canvasGroupGeometry';
import {
    createLocalCanvasEntityIdentity,
    matchesCanvasEntityKey,
...
const findElementByKey = (
    ...
    return elements.find((element) => matchesCanvasEntityKey(element, key));
};

const normalizeElementRelations = (draft: CanvasElement[], removedIds: Set<string>): void => {
    for (let i = draft.length - 1; i >= 0; i -= 1) {
        if (removedIds.has(resolveCanvasEntityKey(draft[i]))) {
            draft.splice(i, 1);
        }
    }

    const existingIds = new Set(draft.map((element) => resolveCanvasEntityKey(element)));
    ...
    for (let i = draft.length - 1; i >= 0; i -= 1) {
        ...
        if (element.type === 'group' && element.groupChildren) {
            ...
            element.groupChildren.forEach((childId) => {
                if (childId === resolveCanvasEntityKey(element)) return;
...
const cloneElementWithMappedIds = (
    ...
    const sourceKey = resolveCanvasEntityKey(element);
    const cloneIdentity = createLocalCanvasEntityIdentity();
    const cloneUuid = idMap.get(sourceKey) ?? cloneIdentity.uuid;
    ...
    if (clone.resource) {
        const clonedResourceIdentity = createLocalCanvasEntityIdentity();
        clone.resource = { ...clone.resource, ...clonedResourceIdentity };
    }
    ...
    if (clone.type === 'group' && clone.groupChildren) {
        ...
        ).filter((childId) => childId !== cloneUuid);
    }
...
const collectCanvasIntegrityIssues = (elements: CanvasElement[]): string[] => {
    ...
    elements.forEach((element) => {
        ...
        if (elementById.has(elementKey)) {
            issues.push(`Duplicate element key: ${elementKey}`);
            return;
        }
        elementById.set(elementKey, element);
        if (element.type === 'group') {
            groupById.set(elementKey, element);
        }
    });
    ...
    elements.forEach((element) => {
        ...
        if (element.type === 'connector') {
            ...
            const to = element.data?.connection?.to;
            if (!from || !to) {
                issues.push(`Connector ${elementKey} missing from/to endpoint.`);
                return;
            }
            if (from === to) {
                issues.push(`Connector ${elementKey} has self-loop endpoint ${from}.`);
            }
            ...
            if (!isConnectableElement(fromElement) || !isConnectableElement(toElement)) {
                issues.push(`Connector ${elementKey} points to invalid endpoint(s): ${from} -> ${to}.`);
            }
        ...
        if (element.groupId) {
            ...
            if (!parent) {
                issues.push(`Element ${elementKey} references missing group ${element.groupId}.`);
            } else if (!parent.groupChildren?.includes(elementKey)) {
                issues.push(`Element ${elementKey} groupId ${element.groupId} missing in parent's children.`);
            }
        }
        ...
        if (element.type === 'group' && element.groupChildren) {
            ...
            element.groupChildren.forEach((childId) => {
                const elementKey = resolveCanvasEntityKey(element);
                if (childId === elementKey) {
                    issues.push(`Group ${elementKey} includes itself as child.`);
                    return;
                }
                if (childDedupe.has(childId)) {
                    issues.push(`Group ${elementKey} has duplicate child ${childId}.`);
                    return;
                }
                childDedupe.add(childId);
                const child = elementById.get(childId);
                if (!child) {
                    issues.push(`Group ${elementKey} references missing child ${childId}.`);
                    return;
                }
                if (child.groupId && child.groupId !== elementKey) {
                    issues.push(`Child ${childId} linked to different group ${child.groupId} (expected ${elementKey}).`);
                }
            });
        }
    });
...
const collectMovableSelectionRoots = (state: {
    ...
    rootIds.forEach((rootId) => {
        const descendantIds = canvasHierarchyService
            ...
            .map((element) => resolveCanvasEntityKey(element));
...
const applyRootTranslationsToDraft = (
    ...
    draft.forEach((element) => {
        const delta = deltaByElement.get(resolveCanvasEntityKey(element));
        if (!delta) return;
        element.x += delta.dx;
        element.y += delta.dy;
    });
...
export const useCanvasStore = create<CanvasState>((set, get) => ({
    ...
    createBoard: async (title) => {
        ...
        if (result.success && result.data) {
             ...
             const boardKey = resolveCanvasEntityKey(newBoard);
             set(state => ({ 
                 boards: [...state.boards, newBoard],
                 activeBoardId: boardKey,
    ...
    deleteBoard: async (id) => {
        ...
        set(state => {
            const nextBoards = state.boards.filter(b => !matchesCanvasEntityKey(b, id));
            return { 
                boards: nextBoards,
    ...
    importBoard: (board) => {
        const boardKey = resolveCanvasEntityKey(board);
        set(state => ({
    ...
    addElement: (element) => {
        ...
        set(state => ({
            elements: nextElements,
            selectedIds: new Set([elementKey]),
    ...
    updateElement: (id, updates, isFinal = true) => {
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(referenceElements, draft => {
            const idx = draft.findIndex(el => matchesCanvasEntityKey(el, id));
            if (idx !== -1) {
    ...
    updateElementsPosition: (updates, isFinal, groupIdsToFit = []) => {
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(referenceElements, draft => {
            ...
            draft.forEach(el => {
                const upd = map.get(resolveCanvasEntityKey(el));
                if (upd) {
                    el.x = upd.x;
                    el.y = upd.y;
                }
    ...
    deleteElement: (id) => {
        ...
        const existingIds = new Set(nextElements.map(el => resolveCanvasEntityKey(el)));
    ...
    selectElement: (id, multi = false, _deep = false) => {
        set(state => {
            if (!id) {
    ...
    selectAll: () => set(state => ({ selectedIds: new Set(state.elements.map(e => resolveCanvasEntityKey(e))) })),
    ...
    addConnection: (fromId, toId) => {
        ...
        const fromElement = findElementByKey(elements, fromId);
        const toElement = findElementByKey(elements, toId);
        ...
        const connectionIdentity = createLocalCanvasEntityIdentity();
    ...
    bringToFront: () => {
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(state.elements, draft => {
            const selected = draft.filter(e => state.selectedIds.has(resolveCanvasEntityKey(e)));
            const others = draft.filter(e => !state.selectedIds.has(resolveCanvasEntityKey(e)));
            draft.length = 0;
    ...
    duplicateSelected: () => {
        ...
        selectedElements.forEach((element) => {
            idMap.set(resolveCanvasEntityKey(element), createLocalCanvasEntityIdentity().uuid);
        });
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(state.elements, draft => {
            selectedElements.forEach((element) => {
                ...
                if (!clone) return;
                draft.push(clone);
                createdIds.push(resolveCanvasEntityKey(clone));
            });
        });
    ...
    groupSelected: () => {
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(state.elements, draft => {
            const selectedEls = draft.filter(e => rootIdSet.has(resolveCanvasEntityKey(e)));
            ...
            const padding = 20;
            const groupIdentity = createLocalCanvasEntityIdentity();
            ...
            draft.forEach(el => {
                if (rootIdSet.has(resolveCanvasEntityKey(el))) {
                    el.groupId = group.uuid;
                }
            });
        ...
        const newGroupId = nextElements
            .find(el => el.type === 'group' && !get().elements.some(e => matchesCanvasEntityKey(e, resolveCanvasEntityKey(el))))
            ?.uuid;
    ...
    ungroupSelected: () => {
        const state = get();
        const groups = Array.from(state.selectedIds).map(id => findElementByKey(state.elements, id)).filter(e => e && e.type === 'group');
        if (groups.length === 0) return;
        
        const groupIds = new Set(groups.map(g => resolveCanvasEntityKey(g!)));
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(state.elements, draft => {
            ...
            for (let i = draft.length - 1; i >= 0; i--) {
                if (groupIds.has(resolveCanvasEntityKey(draft[i]))) {
                    draft.splice(i, 1);
                } else {
                    const parentGroupId = draft[i].groupId;
                    if (childrenIds.includes(resolveCanvasEntityKey(draft[i])) && parentGroupId && groupIds.has(parentGroupId)) {
    ...
    fitGroupToChildren: (groupId: string) => {
        const state = get();
        const group = findElementByKey(state.elements, groupId);
    ...
    alignSelected: (alignment) => {
        const state = get();
        const { rootIds, descendantIdsByRoot } = collectMovableSelectionRoots(state);
        const selected = state.elements.filter(e => rootIds.includes(resolveCanvasEntityKey(e)));
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(state.elements, draft => {
            ...
            selected.forEach((element) => {
                ...
                deltaByRoot.set(resolveCanvasEntityKey(element), { dx, dy });
            });
            applyRootTranslationsToDraft(draft, descendantIdsByRoot, deltaByRoot);
            applyAncestorGroupFit(draft, state.elements, rootIds);
        });
        ...
        const [nextElements, patches, inversePatches] = produceWithPatches(state.elements, draft => {
            ...
            if (direction === 'horizontal') {
                ...
                sorted.forEach((el, idx) => {
                    if (idx === 0) {
                        nextPositionMap.set(resolveCanvasEntityKey(el), first.x);
                        return;
                    }
                    currentX += sorted[idx - 1].width + gap;
                    nextPositionMap.set(resolveCanvasEntityKey(el), currentX);
                });
            } else {
                ...
                sorted.forEach((el, idx) => {
                    if (idx === 0) {
                        nextPositionMap.set(resolveCanvasEntityKey(el), first.y);
                        return;
                    }
                    currentY += sorted[idx - 1].height + gap;
                    nextPositionMap.set(resolveCanvasEntityKey(el), currentY);
                });
            }
            ...
            selected.forEach((element) => {
                const elementKey = resolveCanvasEntityKey(element);
                const nextValue = nextPositionMap.get(elementKey);
