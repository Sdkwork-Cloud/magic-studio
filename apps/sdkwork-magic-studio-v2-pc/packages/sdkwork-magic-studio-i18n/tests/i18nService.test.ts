...
describe('i18nService locale handling', () => {
  afterEach(async () => {
    ...
    await i18nService.setLocale(DEFAULT_LOCALE, { persist: false });
  });
  ...
  it('persists locale changes and syncs document language', async () => {
  ...
  it('warms base locale resources before exposing runtime copy', async () => {
    await i18nService.initialize({
      requestedLocale: 'zh-CN',
      persist: false,
    });

    expect(i18nService.t('appShell.loading_route')).toBe('路由加载中...');
  });

  it('notifies subscribers when package resources change and falls back to en-US package strings', async () => {
    const listener = vi.fn();
    const unsubscribe = i18nService.subscribe(listener);
    ...
    await i18nService.setLocale('zh-CN', { persist: false });
    listener.mockClear();
