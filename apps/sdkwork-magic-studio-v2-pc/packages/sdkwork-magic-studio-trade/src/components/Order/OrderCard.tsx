...
import { cn, formatDate } from '@sdkwork/magic-studio-commons';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {order.status === OrderStatusEnum.REFUNDING && (
          <div className="text-xs text-orange-400">{t('market.order.status.refunding')}</div>
        )}
        {order.status === OrderStatusEnum.REFUNDED && (
          <div className="text-xs text-red-400">{t('market.order.status.refunded')}</div>
        )}
      </div>
    </div>
  );
