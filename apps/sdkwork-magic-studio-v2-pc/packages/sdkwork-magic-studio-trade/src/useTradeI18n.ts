import { formatCurrency, useTranslation } from '@sdkwork/magic-studio-i18n';
import type {
  OrderStatus,
  OrderType,
  PaymentMethod,
...
export const useTradeI18n = () => {
  ...
  const orderStatusLabel = (status: OrderStatus): string => {
    const keyMap: Record<OrderStatus, string> = {
      ...
      [OrderStatusEnum.REFUNDING]: 'market.order.status.refunding',
  ...
  const orderTypeLabel = (type: OrderType): string => {
    const keyMap: Record<OrderType, string> = {
      [OrderTypeEnum.GOODS]: 'market.order.type_label.goods',
      [OrderTypeEnum.VIRTUAL]: 'market.order.type_label.virtual',
      [OrderTypeEnum.MEMBER]: 'market.order.type_label.member',
      [OrderTypeEnum.POINTS]: 'market.order.type_label.points',
      [OrderTypeEnum.IM_GROUP]: 'market.order.type_label.im_group',
      [OrderTypeEnum.BOOKING]: 'market.order.type_label.booking',
      [OrderTypeEnum.SERVICE]: 'market.order.type_label.service',
