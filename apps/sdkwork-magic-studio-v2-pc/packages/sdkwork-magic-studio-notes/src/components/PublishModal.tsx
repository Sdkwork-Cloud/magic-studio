...
import { MediaAccountConfig, useSettingsStore } from '@sdkwork/magic-studio-settings';
import { useTranslation } from '@sdkwork/magic-studio-i18n';
import { useAuthStore } from '@sdkwork/magic-studio-auth';
...
import {
  ...
  PopoverTrigger,
  Textarea,
} from '@sdkwork/magic-studio-commons/ui';
import { createUuid } from '@sdkwork/magic-studio-types/entity';
import type { Asset } from '@sdkwork/magic-studio-types/assets';
import type { ArticlePayload, Note, PublishTarget } from '@sdkwork/magic-studio-types/notes';
...
export const PublishModal: React.FC<PublishModalProps> = ({ note, onClose }) => {
  ...
  const clearActiveArticleCover = () => {
    updateActiveArticle({
      coverImage: undefined,
      coverAssetId: null,
      coverAssetUuid: null,
      coverPrimaryResourceId: null,
      coverPrimaryResourceUuid: null,
      coverResourceViewId: null,
      coverResourceViewUuid: null,
    });
  };

  const handleCoverAssetChange = async (asset: Asset | null): Promise<void> => {
    if (!asset) {
      clearActiveArticleCover();
      return;
    }

    let resolvedUrl: string | null = null;
    try {
      resolvedUrl = await resolveAssetUrlByAssetIdFirst(asset);
    } catch (error) {
      console.warn('Failed to resolve publish cover asset URL', error);
    }

    updateActiveArticle(
      toArticleCoverAssetFields(
        createNoteImageAssetReferenceFromAsset(asset, resolvedUrl)
      )
    );
  };

  useEffect(() => {
    const extractedCoverReference =
      extractFirstHtmlImageAssetReference(note.content || '') ||
      createEmptyNoteImageAssetReference();
    const defaultCover = note.metadata?.coverImage || extractedCoverReference.url || undefined;
    const initialCoverFields = toArticleCoverAssetFields({
      ...extractedCoverReference,
      url: extractedCoverReference.url || defaultCover || null,
    });
            ...
            <div className="space-y-8 px-6 py-4">
              <div className="space-y-2">
                ...
                <ChooseAsset
                  ...
                  label="Select Cover"
                  onChange={(asset) => { void handleCoverAssetChange(asset); }}
                  value={toNoteImageChooseAssetValue({
                    url: activeArticle.coverImage ?? null,
                    assetId: activeArticle.coverAssetId ?? null,
                    assetUuid: activeArticle.coverAssetUuid ?? null,
                    primaryResourceId: activeArticle.coverPrimaryResourceId ?? null,
                    primaryResourceUuid: activeArticle.coverPrimaryResourceUuid ?? null,
                    resourceViewId: activeArticle.coverResourceViewId ?? null,
                    resourceViewUuid: activeArticle.coverResourceViewUuid ?? null,
                  }, 'Selected Cover')}
                />
