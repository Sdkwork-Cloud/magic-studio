...
import { FileAttachmentAttrs, getNodeAttrs } from './nodeAttrs';
import { toNoteAssetUrlSource } from './mediaAssetAttrs';

export const FileAttachmentNode: React.FC<NodeViewProps> = (props) => {
  const { node } = props;
  const attrs = getNodeAttrs<FileAttachmentAttrs>(node);
  const { name = 'file', size = 0, src = '' } = attrs;
  const { url: resolvedHref } = useAssetUrl(toNoteAssetUrlSource({ ...attrs, src }));
