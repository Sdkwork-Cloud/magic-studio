...
import type { AppAuthRegisterInput } from '../../services/appAuthService';
...
interface RegisterFlowProps {
  ...
  onSubmit: (payload: AppAuthRegisterInput) => Promise<void>;
}
...
export function RegisterFlow({
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      startCooldown();
      sdkToast.success(t('auth.toasts.codeSent'));
    } catch (error) {
      sdkToast.error(readErrorMessage(error, t('auth.errors.sendCodeFailed')));
    } finally {
      setIsSendingCode(false);
    }
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      sdkToast.success(t('auth.toasts.registerSuccess'));
    } catch (error) {
      sdkToast.error(readErrorMessage(error, t('auth.errors.registrationFailed')));
    } finally {
