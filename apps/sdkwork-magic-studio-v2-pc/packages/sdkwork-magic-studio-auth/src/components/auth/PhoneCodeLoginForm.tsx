...
import { looksLikePhoneNumber, readErrorMessage } from './authConfig';
import { useActionCooldown } from './useActionCooldown';
import { VerificationCodeField } from './VerificationCodeField';

interface PhoneCodeLoginFormProps {
  initialPhone?: string;
  onSubmit: (payload: { code: string; phone: string }) => Promise<void>;
}
...
export function PhoneCodeLoginForm({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!isPhoneValid) {
      sdkToast.error(t('auth.errors.invalidPhone'));
      return;
    }
