...
export const PPTExplorer: React.FC = () => {
    const { presentations, activePresentationKey, selectPresentation, createPresentation, deletePresentation } = useChatPPTStore();
    ...
    return (
        <div className="h-full flex flex-col bg-[#1e1e1e] border-r border-[#27272a]">
             ...
             <div className="flex-1 overflow-y-auto p-2 space-y-1">
                 {filtered.map(ppt => {
                     const presentationKey = resolveEntityKey(ppt);
                     const isActive = matchesEntityKey(ppt, activePresentationKey);

                     return (
                     <div 
                        key={presentationKey}
                        onClick={() => selectPresentation(presentationKey)}
                        className={`
                            group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-colors
                            ${isActive ? 'bg-[#094771] text-white' : 'text-gray-400 hover:bg-[#2a2a2c] hover:text-gray-200'}
                        `}
                     >
                         ...
                         <button 
                            onClick={(e) => { e.stopPropagation(); if(confirm('Delete presentation?')) deletePresentation(presentationKey); }}
