...
export const DriveStoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    if (filterType !== 'all') {
      filtered = filtered.filter(item => {
        ...
        switch (filterType) {
          case 'image':
            return (
              mime.startsWith('image/') || /\.(png|jpg|jpeg|gif|svg|webp|bmp|ico|tiff)$/.test(name)
            );
          case 'video':
          ...
          case 'code':
            return /\.(ts|tsx|js|jsx|json|html|css|py|rs|go|java|c|cpp|h|xml|yaml|yml|sh|bat)$/.test(
              name
            );
          case 'font':
      ...
      for (const file of files) {
        if (file.path && hasNativeImport) {
          ...
          if (file.data.length > 0) {
            const uploadResult = await driveBusinessService.uploadFile(
              currentPath,
              file.name,
              file.data
            );
  ...
  const downloadItems = async (ids: string[]) => {
    const selectedItems = items.filter(item => ids.includes(item.id) && item.type === 'file');
    if (selectedItems.length === 0) {
      return;
    }
    const downloadResult = await driveBusinessService.downloadItems(selectedItems);
    ensureResultSuccess(downloadResult, 'Download items');
  };
