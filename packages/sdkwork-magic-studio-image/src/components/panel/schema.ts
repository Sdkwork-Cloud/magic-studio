...
export const IMAGE_PANEL_SCHEMA: ImagePanelSchema = {
    ...
    generationRule: {
        anyOf: [{ key: 'prompt' }, { key: 'reference_image' }]
    }
};
...
export const getImageOutputConfigPatch = (
        ...
        configReferenceImages.length > 0
            ? configReferenceImages
                : fallbackReferenceImage
                    ? [fallbackReferenceImage]
                    : [];
    const normalizedReferenceImages: typeof mergedReferenceImages = [];
    const seenReferenceKeys = new Set<string>();
    mergedReferenceImages.forEach((item) => {
        const key = resolveImageInputResourceKey(item) || resolveImageInputResourceReference(item);
        if (!key || seenReferenceKeys.has(key)) {
            return;
        }
        seenReferenceKeys.add(key);
        normalizedReferenceImages.push(item);
    });
    const boundedReferenceImages = normalizedReferenceImages.slice(0, outputPolicy.maxReferenceImages);
    const normalizedReferenceImage = boundedReferenceImages[0];
    ...
    if (
        boundedReferenceImages.length !== configReferenceImages.length ||
        boundedReferenceImages.some((item, index) => configReferenceImages[index] !== item)
    ) {
        patch.referenceImages = boundedReferenceImages;
    }
