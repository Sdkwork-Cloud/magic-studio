...
import { toImageInputSelectableAsset } from '../../utils/imageInputResource';
...
interface ImageReferenceSectionProps {
    ...
    onChangeReferences: (value: ImageInputResourceRef[]) => void;
            ...
            const resolved = await Promise.all(
                referenceImages.map(async (source) => {
                    const url = await resolveAssetUrlByAssetIdFirst(
                        toImageInputSelectableAsset(source) || (source as any)
                    );
                    if (url) {
                        return url;
                    }
                    const directUrl = resolveImageInputResourceUrl(source);
                    return directUrl && isRenderableUrl(directUrl) ? directUrl : '';
                })
            );
            if (!cancelled) {
        ...
        <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-3">
                {referenceImages.map((_, index) => (
                    <div key={`${resolveImageInputResourceKey(referenceImages[index]) || resolveImageInputResourceReference(referenceImages[index]) || index}-${index}`} className="relative aspect-video rounded-lg overflow-hidden border border-[#27272a] group bg-[#121214]">
                        {resolvedReferenceImages[index] ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center gap-1 px-3 text-center">
                                ...
                                <div className="max-w-full truncate text-[9px] text-gray-500">
                                    {resolveImageInputResourceReference(referenceImages[index]) || 'Preview unavailable'}
                                </div>
                            </div>
                        )}
                        <button
