...
import { formatLocaleTime } from '@sdkwork/magic-studio-commons'
import React, { useState } from 'react';
import { matchesEntityKey, resolveEntityKey } from '@sdkwork/magic-studio-types/entity';
import { resolveGeneratedImageResultUrl, type ImageTask } from '../entities';
...
export const GalleryGrid: React.FC = () => {
    ...
    const previewTask = previewId
        ? history.find((task) => matchesEntityKey(task, previewId)) || null
        : null;

    const handleDownload = (task: ImageTask) => {
        const url = resolveGeneratedImageResultUrl(task.results?.[0]);
        if (url) {
            const a = document.createElement('a');
            a.href = url;
            a.download = `generated-${resolveEntityKey(task)}.png`;
            a.click();
        }
    };
    ...
    return (
        <div className="flex-1 bg-[#111] overflow-y-auto p-6 scroll-smooth">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                {history.map(task => {
                    const imageUrl = resolveGeneratedImageResultUrl(task.results?.[0]);
                    
                    return (
                        <div 
                            key={resolveEntityKey(task)} 
                                    ...
                                    <>
                                        ...
                                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3">
                                            <div className="self-end">
                                                <button 
                                                    onClick={() => deleteTask(resolveEntityKey(task))}
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button 
                                                    onClick={() => setPreviewId(resolveEntityKey(task))}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md animate-in fade-in duration-200" onClick={onClose}>
            <div className="max-w-[90vw] max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="mt-4 flex justify-center gap-4">
                     ...
                     <button className="bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-full backdrop-blur text-sm flex items-center gap-2" onClick={() => {
                         void runtime.clipboard.copy(task.config.prompt);
                     }}>
