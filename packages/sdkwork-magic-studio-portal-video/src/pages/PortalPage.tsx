...
import { resolveLocalizedText, useTranslation } from '@sdkwork/magic-studio-i18n';
import { useFilmStore, FilmStoreProvider } from '@sdkwork/magic-studio-film';
import {
    inlineDataService,
...
import { ModelSelector, AspectRatioSelector, Popover, findByIdOrFirst } from '@sdkwork/magic-studio-commons';
...
import { resolvePortalGenMode } from '../utils/portalGenerationSelection';
...
const PortalContentInner: React.FC<PortalContentInnerProps> = ({ navigate }) => {
    ...
    const currentProviders = useMemo(() => {
        if (!activeCapabilitySnapshot) {
            return [];
        }
        return toCreationModelProviders(activeCapabilitySnapshot);
    ...
    const activeCapabilityOptions = useMemo(() => {
        return resolveCreationEntryCapabilityOptions(
            activeCapabilitySnapshot || { target: activeTab, channels: [], styleOptions: [] },
            resolvePreferredModelId(currentProviders, activeModel),
        );
    }, [activeCapabilitySnapshot, activeModel, activeTab, currentProviders]);
    ...
    const effectiveActiveStyle = useMemo(() => {
        if (activeTab !== 'short_drama') {
            return '';
        }
        return resolvePreferredSelectionId(activeStyleOptions, activeStyle, '');
    }, [activeStyle, activeStyleOptions, activeTab]);
    const effectiveDuration = useMemo(
        () => resolvePreferredSelectionValue(activeDurationOptions, duration, '5s'),
        [activeDurationOptions, duration]
    );
    const effectiveResolution = useMemo(
        () => resolvePreferredSelectionValue(activeResolutionOptions, resolution, '2k'),
        [activeResolutionOptions, resolution]
    );
    const effectiveAspectRatio = useMemo(
        () => resolvePreferredSelectionValue(activeAspectRatioOptions, aspectRatio, '16:9') as PortalAspectRatio,
        [activeAspectRatioOptions, aspectRatio]
    );
    const effectiveGenMode = useMemo(
        () => resolvePortalGenMode({ activeTab, requestedMode: genMode, attachments }),
        [activeTab, attachments, genMode]
    );
    ...
    const handleGenerate = async () => {
        ...
        const persistPortalLaunchContext = (target: PortalTab) => {
            return savePortalLaunchSession({
                target,
                prompt,
                genMode: effectiveGenMode,
                model: effectiveActiveModel,
                styleId: effectiveActiveStyle,
                aspectRatio: toOptionalString(effectiveAspectRatio),
                resolution: toOptionalString(effectiveResolution),
                duration: toOptionalString(effectiveDuration),
                attachments: attachments.map((attachment) => ({
                    id: attachment.assetId ?? null,
                    uuid: attachment.uuid,
                    name: attachment.name,
                    type: attachment.type,
                    assetId: attachment.assetId ?? null,
                    assetUuid: attachment.assetUuid ?? null,
        ...
        try {
            if (activeTab === 'short_drama') {
                 await clearPortalLaunchSession();
                 ...
                 await persistPortalLaunchContext('video');
                 navigate(ROUTES.VIDEO); 
            } else if (activeTab === 'image') {
                 await persistPortalLaunchContext('image');
                 navigate(ROUTES.IMAGE);
            } else if (activeTab === 'speech') {
                 await persistPortalLaunchContext('speech');
                 navigate(ROUTES.AUDIO);
            } else if (activeTab === 'human') {
                 await persistPortalLaunchContext('human');
                 navigate(ROUTES.CHARACTER);
            } else if (activeTab === 'music') {
                 await persistPortalLaunchContext('music');
    ...
    const handleRemoveAttachment = (id: string) => {
        setAttachments(prev => removeInputAttachmentByKey(prev, id));
    };
    ...
    const getPlaceholder = () => {
        ...
        if (activeTab === 'video') {
            if (effectiveGenMode === 'start_end') return 'Upload frames and describe camera movement...';
    ...
    const heroFooterControls = (
        <FooterControls
            ...
            activeStyle={effectiveActiveStyle}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 p-8">
                    <div className="max-w-[1600px] mx-auto flex flex-col items-center">
                        ...
                        <div className="w-full mt-20 mb-8 text-center space-y-8">
                            ...
                            <div className="max-w-[1500px] mx-auto w-full" ref={heroSectionRef}>
                                <CreationChatInput 
                                    value={prompt}
                                    onChange={setPrompt}
                    ...
                    {isExpanded && (
                        <div className="fixed inset-0 z-[100] flex flex-col justify-end pointer-events-none">
                            ...
                            <div className="w-full max-w-[1500px] mx-auto px-4 pb-12 relative z-10 pointer-events-auto animate-in slide-in-from-bottom-10 fade-in duration-300">
                                <button
