import { Button, Asset } from '@sdkwork/react-commons';
...
import { ChooseAsset, resolveAssetUrlByAssetIdFirst } from '@sdkwork/react-assets';
    ...
    const handleAssetChange = async (asset: Asset | null): Promise<void> => {
        ...
        try {
            const resolved = await resolveAssetUrlByAssetIdFirst({
                ...
                assetId: asset.id,
                path: asset.path,
                url: (asset as { url?: string; remoteUrl?: string }).url || (asset as { remoteUrl?: string }).remoteUrl
            });
            if (resolved) {
        ...
        const fallback = asset.path || asset.id;
        setImageUrl(fallback && isRenderableUrl(fallback) ? fallback : '');
    };
