...
import { 
    AlertCircle, File, X, Trash2, FolderOpen, 
...
export const DownloadsPanel: React.FC = () => {
    ...
    const handleOpen = (path?: string) => {
        if (path) void runtime.shell.openExternal(`file://${path}`); // Attempt to open file
    };

    const handleShowInFolder = (path?: string) => {
        if (path) void runtime.shell.showItemInFolder(path);
    };
