...
export const CharacterLeftGeneratorPanel: React.FC = () => {
    ...
    const config = characterStore?.config || { model: 'gemini-3-flash-image', voiceId: 'male' };
    ...
    const toggleVoicePreview = (_url?: string) => { // eslint-disable-line @typescript-eslint/no-unused-vars
        if (!_url) return;
        ...
        if (isPlayingVoice) {
            ...
            } else if (audioRef.current.src !== _url) {
                audioRef.current.src = _url;
            }
    ...
    const activeModel = CHARACTER_MODELS.find((m: any) => m.id === config.model) || CHARACTER_MODELS[0];

    // Voice helpers
    const activeVoice = PRESET_VOICES.find((v: any) => v.id === config.voiceId);
    const _selectedVoiceForChooser = activeVoice ? { // eslint-disable-line @typescript-eslint/no-unused-vars
        id: activeVoice.id,
    ...
    const [activeTab, _setActiveTab] = useState<'avatar' | 'threeView' | 'grid'>('avatar'); // eslint-disable-line @typescript-eslint/no-unused-vars
