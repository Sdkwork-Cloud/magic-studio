...
import { Confirm, useConfirm } from '@sdkwork/magic-studio-commons';
import { logger } from '@sdkwork/magic-studio-commons/utils/logger';
import type { TreeItem, NoteType } from '@sdkwork/magic-studio-types/notes';
import { 
...
export const NoteSidebar: React.FC = () => {
    ...
    const resolveFolderSelectionKey = useCallback((folderKey: string | null | undefined): string | null => {
        if (!folderKey) {
            return null;
        }
        const folder = findNoteSidebarEntityByKey(folders, folderKey);
        return folder ? resolveNoteSidebarKey(folder) : folderKey;
    }, [folders]);
    ...
    const selectedTrashedNotes = useMemo(() => {
        return filteredTrashedNotes.filter((note) => trashSelection.has(resolveNoteSidebarKey(note)));
    ...
    useEffect(() => {
        setTrashSelection((prev) => {
            ...
            const next = reconcileNoteSidebarSelectionKeys(filteredTrashedNotes, prev);
    ...
    const getVisibleItems = useCallback(() => {
        ...
        const traverse = (items: TreeItem[]) => {
            for (const item of items) {
                ...
                visible.push(item);
                if (item.kind === 'folder' && isNoteSidebarFolderExpanded(item, expandedFolders) && item.children) {
    ...
    const handleDelete = async (item: TreeItem) => {
        ...
        if (confirmed) {
            clearOperationError();
            const deleted = await deleteItem(resolveNoteSidebarKey(item), item.kind);
            if (deleted && matchesNoteSidebarKey(item, selectedId)) {
                setSelectedId(null);
            }
        }
        setContextMenu(null);
    ...
    const toggleSelectAllVisibleTrash = () => {
        setTrashSelection((prev) => {
            ...
            if (allVisibleTrashedSelected) {
                filteredTrashedNotes.forEach((note) => next.delete(resolveNoteSidebarKey(note)));
            } else {
                filteredTrashedNotes.forEach((note) => next.add(resolveNoteSidebarKey(note)));
            }
            return next;
        });
    };
    ...
    const handleBatchRestore = async () => {
        ...
        clearOperationError();
        const selectedIds = selectedTrashedNotes.map((note) => resolveNoteSidebarKey(note));
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        ...
        if (['ArrowDown', 'ArrowUp'].includes(e.key)) {
            ...
            const currentIndex = visible.findIndex((item) => matchesNoteSidebarKey(item, selectedId));
            
            if (e.key === 'ArrowDown') {
                const next = visible[currentIndex + 1];
                if (next) setSelectedId(resolveNoteSidebarKey(next));
            } else if (e.key === 'ArrowUp') {
                const prev = visible[currentIndex - 1];
                if (prev) setSelectedId(resolveNoteSidebarKey(prev));
            }
        } 
            ...
            if (selectedId) {
                const visible = getVisibleItems();
                const item = findNoteSidebarEntityByKey(visible, selectedId);
                if (item && item.kind === 'folder' && !isNoteSidebarFolderExpanded(item, expandedFolders)) {
                    toggleFolderExpand(resolveNoteSidebarKey(item));
                }
            }
        }
            ...
            if (selectedId) {
                ...
                const item = findNoteSidebarEntityByKey(visible, selectedId);
                if (item) {
                    if (item.kind === 'folder' && isNoteSidebarFolderExpanded(item, expandedFolders)) {
                        toggleFolderExpand(resolveNoteSidebarKey(item));
                    } else if (item.parentId) {
                        setSelectedId(resolveFolderSelectionKey(item.parentId));
                    }
                }
            }
        }
            ...
            if (selectedId) {
                ...
                const item = findNoteSidebarEntityByKey(visible, selectedId);
                if (item) {
                    if (item.kind === 'note') setActiveNoteId(resolveNoteSidebarKey(item));
                    else toggleFolderExpand(resolveNoteSidebarKey(item));
                }
            }
        }
        else if (e.key === 'F2') {
    ...
    const getTargetParentId = (): string | null => {
        ...
        const folder = findNoteSidebarEntityByKey(folders, selectedId);
        if (folder) return resolveNoteSidebarKey(folder);

        const note = findNoteSidebarEntityByKey(notes, selectedId);
        if (note) return resolveFolderSelectionKey(note.parentId);
    ...
    const handleRenameCommit = async (id: string, newName: string) => {
        const item = findNoteSidebarEntityByKey(notes, id) || findNoteSidebarEntityByKey(folders, id);
        if (!item) return;
        ...
        if ('title' in item) { 
             ...
             if (matchesNoteSidebarKey(item, selectedId)) setSelectedId(newId);
        }
        setRenamingId(null);
        sidebarRef.current?.focus();
    };
    ...
    const handleItemClick = (e: React.MouseEvent, item: TreeItem) => {
        ...
        setSelectedId(resolveNoteSidebarKey(item));
        
        if (item.kind === 'note') {
            setActiveNoteId(resolveNoteSidebarKey(item));
        }
    };

    const handleItemToggle = (e: React.MouseEvent, item: TreeItem) => {
        e.stopPropagation();
        if (item.kind === 'folder') toggleFolderExpand(resolveNoteSidebarKey(item));
    };
    ...
    const handleDragStart = (e: React.DragEvent, item: TreeItem) => {
        ...
        const dragPayload: DragPayload = { key: resolveNoteSidebarKey(item), kind: item.kind };
    ...
    const handleDragOver = (e: React.DragEvent, item: TreeItem) => {
        ...
        if (payload && item.kind === 'folder' && payload.key !== resolveNoteSidebarKey(item)) {
            setDragOverId(resolveNoteSidebarKey(item));
    ...
    const handleDrop = (e: React.DragEvent, targetId: string) => {
        ...
        try {
            const payload = parseDragPayload(e);
            if (payload && payload.key !== targetId) {
                void moveItem(payload.key, payload.kind, targetId);
            }
        } catch (err) {
    ...
    const handleRootDrop = (e: React.DragEvent) => {
        ...
        try {
            const payload = parseDragPayload(e);
            if (payload) {
                void moveItem(payload.key, payload.kind, null);
            }
        } catch (err) {
    ...
    const renderTreeNodes = (items: TreeItem[], level = 0, parentId: string | null = null) => {
        ...
        const nodes = nodesToRender.map(item => {
            const isFolder = item.kind === 'folder';
            return (
                <div key={resolveNoteSidebarKey(item)}>
                    <SidebarItemRow 
                        item={item}
                        level={level}
                        isSelected={matchesNoteSidebarKey(item, selectedId)}
                        isExpanded={!!(isFolder && item.isExpanded)}
                        isDragOver={dragOverId === resolveNoteSidebarKey(item)}
                        isRenaming={matchesNoteSidebarKey(item, renamingId)}
                        onRenameCommit={(val) => handleRenameCommit(resolveNoteSidebarKey(item), val)}
                        ...
                        onDrop={(e) => {
                            if (isFolder) {
                                handleDrop(e, resolveNoteSidebarKey(item));
                            }
                        }}
                    ...
                    {isFolder && item.isExpanded && (
                        <div>
                             {renderTreeNodes(item.children || [], level + 1, resolveNoteSidebarKey(item))}
                        </div>
                    )}
                </div>
            );
        ...
        >
                        ...
                        <input 
                            ...
                            onKeyDown={(e) => {
                                ...
                                if (e.key === 'Enter' && searchResults.length > 0) {
                                    ...
                                    setSelectedId(resolveNoteSidebarKey(target));
                                    setActiveNoteId(resolveNoteSidebarKey(target));
                                    setIsSearchOpen(false);
                                }
                            }}
                        ...
                        {filteredTrashedNotes.length === 0 ? (
                            ...
                            <div className="space-y-1">
                                {filteredTrashedNotes.map((note) => (
                                    <div
                                        key={`trash-${resolveNoteSidebarKey(note)}`}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            toggleTrashSelection(resolveNoteSidebarKey(note));
                                        }}
                                        className={`px-3 py-2 rounded-lg border transition-colors cursor-pointer ${
                                            trashSelection.has(resolveNoteSidebarKey(note))
                                    ...
                                    >
                                        <div className="flex items-start gap-2">
                                            ...
                                            >
                                                {trashSelection.has(resolveNoteSidebarKey(note)) && <span className="h-2 w-2 rounded-[2px] bg-blue-300" />}
                                            </button>
                                        ...
                                        <div className="mt-2 flex items-center justify-between gap-2">
                                            ...
                                            <div className="flex items-center gap-1">
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        void handleRestoreSingle(resolveNoteSidebarKey(note));
                                                    }}
                                                ...
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        void handleDeleteSinglePermanently(resolveNoteSidebarKey(note));
                                                    }}
                        ...
                        {searchResults.length === 0 ? (
                            ...
                            <div className="space-y-1 pb-6">
                                {searchResults.map((note) => (
                                    <button
                                        key={`search-${resolveNoteSidebarKey(note)}`}
                                        onClick={() => {
                                            setSelectedId(resolveNoteSidebarKey(note));
                                            setActiveNoteId(resolveNoteSidebarKey(note));
                                            setIsSearchOpen(false);
                                        }}
                                        className={`w-full text-left px-3 py-2 rounded-lg border transition-colors ${
                                            matchesNoteSidebarKey(note, selectedId)
                                ...
                                {showFavorites && (
                                    <div className="mt-0.5">
                                        {favoriteNotes.map(note => (
                                            <SidebarItemRow 
                                                key={`fav-${resolveNoteSidebarKey(note)}`}
                                                item={{ ...note, kind: 'note' } as TreeItem}
                                                level={0}
                                                isSelected={matchesNoteSidebarKey(note, selectedId)}
                                                isExpanded={false}
                                                isDragOver={false}
                                                isRenaming={false} 
                                                onRenameCommit={() => {}}
                                ...
                                {showRecent && (
                                    <div className="mt-0.5">
                                        {recentNotes.map(note => (
                                            <SidebarItemRow 
                                                key={`recent-${resolveNoteSidebarKey(note)}`}
                                                item={{ ...note, kind: 'note' } as TreeItem}
                                                level={0}
                                                isSelected={matchesNoteSidebarKey(note, selectedId)}
                                                isExpanded={false}
            ...
            {contextMenu && (
                <ContextMenu 
                    ...
                    onRename={() => startRename(resolveNoteSidebarKey(contextMenu.item))}
                    onDelete={() => handleDelete(contextMenu.item)}
                    onCreateNote={() => { 
                        const targetParent = contextMenu.item.kind === 'folder'
                            ? resolveNoteSidebarKey(contextMenu.item)
                            : resolveFolderSelectionKey(contextMenu.item.parentId);
                    ...
                    onToggleFavorite={() => { toggleFavorite(resolveNoteSidebarKey(contextMenu.item)); setContextMenu(null); }}
                    t={t}
                />
            )}
