...
import { ChooseAsset, resolveAssetUrlByAssetIdFirst } from '@sdkwork/magic-studio-assets';
import {
    createEmptyNoteImageAssetReference,
    createNoteImageAssetReferenceFromAsset,
    toMiniProgramImageAssetFields,
    toNoteImageChooseAssetValue
} from '../utils/noteAssetReference';

export interface MiniProgramInsertPayload {
    appid: string;
    path: string;
    type: DisplayMode;
    title: string;
...
interface MiniProgramModalProps {
    onClose: () => void;
    onInsert: (data: MiniProgramInsertPayload) => void;
}
...
export const MiniProgramModal: React.FC<MiniProgramModalProps> = ({ onClose, onInsert }) => {
    ...
    const [imageReference, setImageReference] = useState(createEmptyNoteImageAssetReference());
    const [textContent, setTextContent] = useState('');

    const handleAssetChange = async (asset: Asset | null): Promise<void> => {
        if (!asset) {
            setImageReference(createEmptyNoteImageAssetReference());
            return;
        }

        let resolved: string | null = null;
        try {
            resolved = await resolveAssetUrlByAssetIdFirst(asset);
        } catch (error) {
            console.warn('Failed to resolve mini-program cover asset URL', error);
        }

        const reference = createNoteImageAssetReferenceFromAsset(asset, resolved);
        const fallbackUrl = reference.url && isRenderableUrl(reference.url) ? reference.url : null;
        setImageReference({
            ...reference,
            url: fallbackUrl,
        });
    ...
    const handleConfirm = () => {
        ...
        onInsert({
            ...
            ...toMiniProgramImageAssetFields(imageReference),
            ...
            >
                <div className="flex-none px-6 py-4 border-b border-gray-200 dark:border-[#333] flex justify-between items-center bg-gray-50 dark:bg-[#252526]">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-gray-900 dark:text-white font-bold text-base">Insert Mini Program</h3>
                            <p className="text-xs text-gray-500 dark:text-gray-400">Insert a WeChat mini program card/link</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-6 overflow-y-auto border-r border-gray-200 dark:border-[#333]">
                        <div className="space-y-6 max-w-lg mx-auto">
                            <div className="space-y-4">
                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Basic</h4>
                                <div>
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Display Mode</h4>
                            ...
                            <div className="space-y-4">
                                {mode === 'text' && (
                                    <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                                        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Link Text</label>
                                        <input
                                            type="text"
                                ...
                                {mode === 'card' && (
                                    <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                                        <div>
                                            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Card Title</label>
                                            <input
                                                type="text"
                                                className="w-full bg-white dark:bg-[#111] border border-gray-200 dark:border-[#333] rounded-lg px-3 py-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-green-500 transition-colors"
                                                placeholder="Mini program title"
                                                value={title}
                                                onChange={(event) => setTitle(event.target.value)}
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Cover Image</label>
                                            <div className="h-48">
                                                <ChooseAsset
                                                    value={toNoteImageChooseAssetValue(imageReference, 'Mini Program Cover')}
                                                    onChange={handleAssetChange}
                                                    accepts={['image']}
                                                    domain="notes"
                                                    label="Select or upload cover (5:4)"
                                ...
                                {mode === 'image' && (
                                    <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                                        <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Image</label>
                                        <div className="h-48">
                                                <ChooseAsset
                                                value={toNoteImageChooseAssetValue(imageReference, 'Mini Program Image')}
                                                onChange={handleAssetChange}
                                                accepts={['image']}
                                                domain="notes"
            ...
            <div className="p-3">
                ...
                <div className="relative w-full aspect-[5/4] bg-[#f5f5f5] dark:bg-[#111] overflow-hidden rounded-[2px]">
                    ...
                    <div className="absolute bottom-1 left-2 flex items-center gap-1 opacity-80">
                        ...
                        <span className="text-[8px] text-white text-shadow-sm">Mini Program</span>
                    </div>
                </div>
            </div>
