...
import { NoteImageExtension } from './editor/NoteImageExtension';
...
import { filePicker } from '@sdkwork/magic-studio-editor/file-picker';

import { genAIService } from '@sdkwork/magic-studio-core/ai';
import { markdownUtils } from '@sdkwork/magic-studio-commons'; 
import type { AssetType } from '@sdkwork/magic-studio-types/assets';
import type { NoteType } from '@sdkwork/magic-studio-types/notes';
...
import { ImageGeneratorModal } from '@sdkwork/magic-studio-image';
import { VideoGeneratorModal } from '@sdkwork/magic-studio-video';
import { AudioGeneratorModal } from '@sdkwork/magic-studio-audio';
import type { GenerationResultSelection } from '@sdkwork/magic-studio-assets';
import { resolveGenerationResultDeliveryUrl } from '@sdkwork/magic-studio-generation-history';

import {
    importAssetBySdk,
    resolveAssetPrimaryUrlBySdk
} from '@sdkwork/magic-studio-assets';
import { resolveNotesGeneratedSelectionSource } from '../utils/generatedSelectionAssetResolver';
import { createNoteUploadedAssetAttrs } from '../utils/noteUploadedAssetAttrs';
import {
    createNoteFileAttachmentAttrs,
    createNoteImageAttrs,
    createNoteMediaAttrs,
} from './editor/mediaAssetAttrs';
import { buildNoteEditorBreadcrumbs, resolveNoteEditorKey } from './noteEditorIdentity';

type NoteGeneratedMediaType = 'image' | 'video' | 'audio';
...
const AIDropdown: React.FC<{ onAction: (type: NoteGeneratedMediaType) => void }> = ({ onAction }) => {
    const { t } = useTranslation();
    const [isOpen, setIsOpen] = useState(false);
    const ref = useRef<HTMLDivElement>(null);
    ...
    const handleSelect = (type: NoteGeneratedMediaType) => {
        onAction(type);
        setIsOpen(false);
    };
...
interface EditorToolbarProps {
    ...
    onOpenGenModal: (type: NoteGeneratedMediaType) => void;
    ...
    return (
        <div id="note-editor-toolbar" className="sticky top-0 z-20 w-full bg-[#050505]/90 backdrop-blur-md border-b border-[#1f1f22] mb-6 px-2 py-2 flex flex-wrap items-center gap-1">
            ...
            <ToolbarButton
                ...
                disabled={!((editor.can() as unknown as { undo?: () => boolean }).undo?.() ?? false)}
                icon={Undo}
                title="Undo (Cmd+Z)"
            />
            <ToolbarButton
                onClick={() => (editor.chain().focus() as unknown as { redo: () => { run: () => boolean } }).redo().run()}
                disabled={!((editor.can() as unknown as { redo?: () => boolean }).redo?.() ?? false)}
                icon={Redo}
...
export const UniversalNoteEditor: React.FC<UniversalNoteEditorProps> = ({ 
    ...
    const [genModalType, setGenModalType] = useState<NoteGeneratedMediaType | null>(null);
    ...
    const editor = useEditor({
        ...
        extensions: [
            ...
            Placeholder.configure({
                placeholder: ({ node }: { node: { attrs: { level?: number }; type: { name: string } } }) => {
        ...
        if (onFileUpload) {
            ...
            try {
                ...
                if (files.length > 0) {
                     ...
                     const baseAssetAttrs = createNoteUploadedAssetAttrs(uploaded, src);
                     
                     if (type === 'image') {
                        editor.chain().focus().insertContent({
                            type: 'image',
                            attrs: createNoteImageAttrs(baseAssetAttrs),
                        }).run();
                     }
                     else if (type === 'video') {
                        editor.chain().focus().insertContent({
                            type: 'video',
                            attrs: createNoteMediaAttrs(baseAssetAttrs),
                        }).run();
                     }
                     else if (type === 'audio') {
                        editor.chain().focus().insertContent({
                         ...
                         editor.chain().focus().insertContent({
                             type: 'fileAttachment',
                             attrs: createNoteFileAttachmentAttrs({
                                ...baseAssetAttrs,
                                name: file.name,
                                size,
                             })
    ...
    const handleImageSuccess = async (selection: GenerationResultSelection) => {
        try {
            ...
            if (editor) editor.chain().focus().insertContent({
                type: 'image',
                attrs: createNoteImageAttrs({
                    src: resolved.url,
                    assetId: resolved.assetId,
                    assetUuid: resolved.assetUuid,
                    primaryResourceId: resolved.primaryResourceId,
                    primaryResourceUuid: resolved.primaryResourceUuid,
                    resourceViewId: resolved.resourceViewId,
                    resourceViewUuid: resolved.resourceViewUuid,
                }),
            }).run();
        } catch (e) {
            ...
            if (editor) editor.chain().focus().insertContent({
                type: 'image',
                attrs: createNoteImageAttrs({
                    src: resolveGenerationResultDeliveryUrl(selection),
                    assetId: selection.assetId ?? null,
                    assetUuid: selection.assetUuid ?? null,
                    primaryResourceId: selection.primaryResourceId ?? null,
                    primaryResourceUuid: selection.primaryResourceUuid ?? null,
                    resourceViewId: selection.resourceViewId ?? null,
                    resourceViewUuid: selection.resourceViewUuid ?? null,
                }),
            }).run();
        }
    ...
    const handleVideoSuccess = async (selection: GenerationResultSelection) => {
         try {
             const resolved = await resolveNotesGeneratedSelectionSource({
                 selection,
                 type: 'video',
    ...
    const handleAudioSuccess = async (selection: GenerationResultSelection) => {
         try {
             const resolved = await resolveNotesGeneratedSelectionSource({
                 selection,
                 type: 'audio',
            ...
            <div className="flex-1 flex flex-col min-w-0 h-full">
                ...
                >
                    <div className={maxWidthClasses}>
                        ...
                        {/* Content */}
                        <div className="sdk-rich-prose relative z-0 min-h-[500px]">
                            {editor && (
                                <>
...
export const NoteEditor: React.FC = () => {
    ...
    const breadcrumbs = useMemo(
        () => buildNoteEditorBreadcrumbs(activeNote?.parentId, folders),
    ...
    const handleChange = (content: string) => {
        updateNote(resolveNoteEditorKey(activeNote), { content });
    };

    const handleTitleChange = (title: string) => {
        updateNote(resolveNoteEditorKey(activeNote), { title });
    };
    
    const handleTypeChange = (type: NoteType) => {
        updateNote(resolveNoteEditorKey(activeNote), { type });
    };

    return (
        <>
            <UniversalNoteEditor 
                key={resolveNoteEditorKey(activeNote)}
