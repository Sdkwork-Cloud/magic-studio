...
import { MiniProgramAttrs, getNodeAttrs } from './nodeAttrs';
import { toNoteAssetUrlSource } from './mediaAssetAttrs';
...
export const MiniProgramNode: React.FC<NodeViewProps> = (props) => {
  ...
  const {
    ...
    imageAssetId = null,
    imageAssetUuid = null,
    imagePrimaryResourceId = null,
    imagePrimaryResourceUuid = null,
    imageResourceViewId = null,
    imageResourceViewUuid = null,
    type = 'card',
    text = ''
  } = getNodeAttrs<MiniProgramAttrs>(node);
  const { url: resolvedImage } = useAssetUrl(
    toNoteAssetUrlSource({
      src: image,
      assetId: imageAssetId,
      assetUuid: imageAssetUuid,
      primaryResourceId: imagePrimaryResourceId,
      primaryResourceUuid: imagePrimaryResourceUuid,
      resourceViewId: imageResourceViewId,
      resourceViewUuid: imageResourceViewUuid,
    })
  );
  const previewImage = resolvedImage || image;
      ...
      >
        ...
        {type === 'image' && (
            ...
            <div className="relative group max-w-sm rounded-lg overflow-hidden border border-gray-200 dark:border-[#333] mx-auto">
                {previewImage ? (
                    <img src={previewImage} alt="Mini Program Link" className="w-full h-auto block" />
                ) : (
