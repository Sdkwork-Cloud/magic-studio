...
import { MediaAttrs, getNodeAttrs, getNodeTypeName } from './nodeAttrs';
import { toNoteAssetUrlSource } from './mediaAssetAttrs';

export const MediaNode: React.FC<NodeViewProps> = (props) => {
  const { node, selected } = props;
  const { src = '', controls = true } = getNodeAttrs<MediaAttrs>(node);
  const isVideo = getNodeTypeName(node) === 'video';
  
  const { url: resolvedSrc, loading, error } = useAssetUrl(toNoteAssetUrlSource({ ...getNodeAttrs<MediaAttrs>(node), src }));
