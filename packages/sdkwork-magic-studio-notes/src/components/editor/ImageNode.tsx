...
import { ImageAttrs, getNodeAttrs } from './nodeAttrs';
import { toNoteAssetUrlSource } from './mediaAssetAttrs';

export const ImageNode: React.FC<NodeViewProps> = (props) => {
  const { node, selected } = props;
  const { src = '', alt = '', title = '' } = getNodeAttrs<ImageAttrs>(node);
  
  const { url: resolvedSrc, loading, error } = useAssetUrl(toNoteAssetUrlSource({ ...getNodeAttrs<ImageAttrs>(node), src }));
