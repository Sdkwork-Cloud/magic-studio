...
import { assetService } from '@sdkwork/magic-studio-assets/asset-service';
...
export const FilePreviewModal: React.FC<FilePreviewModalProps> = ({ item, onClose }) => {
    ...
    useEffect(() => {
        ...
        const loadContent = async () => {
            ...
            try {
                ...
                const previewSource = item.previewUrl || resolvedPath;
                const resolvedUrl = previewSource
                    ? await assetService.resolveAssetUrl({ path: previewSource })
                    : '';
                ...
                if (isMedia) {
                    const bytesResult = await driveBusinessService.getFileBytes(item.id);
                    const bytes = getResultDataOrThrow(bytesResult, 'Load file bytes');
                    const mime = item.mimeType || 'application/octet-stream';
                    const blob = new Blob([Uint8Array.from(bytes)] as BlobPart[], { type: mime });
                ...
                console.error('[FilePreview] Load failed', error);
    ...
    }, [item.id, item.mimeType, item.name, t]);
