...
class DriveService {
  ...
  private async ensureCanonicalLibraryRoot(): Promise<string> {
    const runtime = getPlatformRuntime();
    const rootLayout = await resolveRuntimeMagicStudioRootLayout(runtime);

    for (const path of [
      rootLayout.rootDir,
      rootLayout.systemRoot,
      rootLayout.systemLibraryRoot,
      ...listMagicStudioSystemLibraryDirs(rootLayout),
    ]) {
      try {
  ...
  async getDefaultPath(): Promise<string> {
    return this.ensureCanonicalLibraryRoot();
  }
