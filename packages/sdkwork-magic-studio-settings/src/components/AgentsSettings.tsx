...
import { enhanceAgentSystemPrompt } from '../services';
...
const AgentsSettings: React.FC = () => {
    ...
    const handleEnhancePrompt = async (currentText: string): Promise<void> => {
        if (!currentText) return;
        
        setIsEnhancing(true);
        setEnhanceError(null);
        try {
            const response = await enhanceAgentSystemPrompt(currentText);
            if (editingId) {
                updateAgent(editingId, { systemPrompt: response });
            }
        } catch {
            setEnhanceError(t('settings.agents.enhance_failed'));
        } finally {
                        ...
                        <div className="bg-white dark:bg-[#1e1e1e] border border-gray-200 dark:border-[#333] rounded-2xl p-8 sticky top-6 animate-in fade-in slide-in-from-bottom-4 duration-300 shadow-xl shadow-black/5">
                             ...
                             <SettingsSection title={t('settings.sections.parameters')}>
                                <div>
                                    ...
                                    />
                                    {enhanceError ? (
                                        <div className="mt-1.5 text-xs font-medium text-red-500" role="alert">
                                            {enhanceError}
                                        </div>
                                    ) : null}
