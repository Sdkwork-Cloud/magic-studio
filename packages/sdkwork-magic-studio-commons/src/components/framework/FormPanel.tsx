...
const deriveFormOverrides = (
  baseValue: FormPanelValues,
  nextValue: FormPanelValues
): FormPanelValues => {
  const nextOverrides: FormPanelValues = {};
  const keys = new Set([...Object.keys(baseValue), ...Object.keys(nextValue)]);

  keys.forEach((key) => {
    if (nextValue[key] !== baseValue[key]) {
      nextOverrides[key] = nextValue[key];
    }
  });
...
export const FormPanel = forwardRef<FormPanelHandle, FormPanelProps>(
  (
    ...
    const [uncontrolledOverrides, setUncontrolledOverrides] = useState<FormPanelValues>({});
    const fieldRefs = useRef<Record<string, FieldRef>>({});
    const activeValue = value ?? { ...initialValue, ...uncontrolledOverrides };

    const commitValue = (nextValue: FormPanelValues, meta: FormValueChangeMeta): void => {
      if (value === undefined) {
        setUncontrolledOverrides(deriveFormOverrides(initialValue, nextValue));
      }
    ...
    const doReset = () => {
      ...
      if (value === undefined) {
        setUncontrolledOverrides({});
      }
      onValueChange?.(resetValue, { source: 'reset' });
      onReset?.(resetValue);
    };
