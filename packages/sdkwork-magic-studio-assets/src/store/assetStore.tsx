...
import { uploadHelper } from '@sdkwork/magic-studio-core/utils';
import { entityKeysEqual, hasPersistentEntityId } from '@sdkwork/magic-studio-types/entity';
...
} from '../services/assetBusinessService';
import {
  normalizeSpringPageRequest,
} from '../asset-center/application/assetCenterAdapters';
import {
...
} from '../components/chooseAssetProjectReference';
...
export const AssetStoreProvider: React.FC<AssetStoreProviderProps> = ({
  ...
  const importAssets = async () => {
    try {
      ...
      for (const file of files) {
        ...
        const imported = await assetBusinessService.importAssetBySdk(file, detected, { domain });
  ...
  const deleteAsset = async (asset: Asset) => {
    try {
      if (hasPersistentEntityId(asset.id)) {
        await assetBusinessService.deleteAssetBySdk(asset.id);
      }
      setAssets((prev) => prev.filter((item) => !entityKeysEqual(item, asset)));
      setSelectedAsset((current) => (
        current && entityKeysEqual(current, asset) ? null : current
      ));
      if (hasPersistentEntityId(asset.id)) {
        await load(0);
      }
  ...
  const renameAsset = async (asset: Asset, newName: string) => {
    try {
      ...
      setAssets((prev) => prev.map((item) => (
        entityKeysEqual(item, asset)
          ? { ...item, name: newName }
          : item
      )));

      setSelectedAsset((current) => (
        current && entityKeysEqual(current, asset)
