...
} from '../asset-center/domain/assetCategory.domain';
...
import { toAssetFromGeneratedSelection } from './generate/generatedSelectionAsset';
import {
...
export const ChooseAsset: React.FC<ChooseAssetProps> = ({ 
    ...
    aiGenerator: AIGenerator, domain = 'asset-center', projectReference
}) => {
    ...
    const hasSelectedValue = value !== null && typeof value !== 'undefined';
    ...
    const { url: displayUrl, loading } = useAssetUrl(
        typeof value === 'object' && value ? value : assetPath || null,
    ...
    const handleAIConfirm = (result: GenerationResultSelection | GenerationResultSelection[]) => {
        const selection = Array.isArray(result) ? result[0] : result;
        if (!selection) {
            return;
        }

        const asset: Asset = toAssetFromGeneratedSelection(selection);
        onChange(asset);
    ...
    const handleLocalUpload = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            if (files.length > 0) {
                ...
                const resolvedUrl = imported.id
                    ? (await assetBusinessService.resolveAssetPrimaryUrlBySdk(imported.id)) || imported.path || ''
                    : imported.path || '';
    ...
    const renderPreview = () => {
        ...
        const canRenderImagePreview =
            !!finalSrc &&
            isRenderableAssetUrl(finalSrc) &&
            (
                assetType === 'image' ||
                finalSrc.match(/\.(jpeg|jpg|gif|png|webp|svg)$/i) !== null ||
        ...
        let Icon = File;
        if (assetType === 'video' || finalSrc.match(/\.(mp4|mov|webm)$/i)) Icon = FileVideo;
        else if (assetType === 'audio' || finalSrc.match(/\.(mp3|wav)$/i)) Icon = FileAudio;
