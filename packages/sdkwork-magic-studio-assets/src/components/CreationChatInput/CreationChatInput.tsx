...
import { matchesInputAttachmentKey } from './attachmentIdentity';
import { Popover } from '@sdkwork/magic-studio-commons';
...
import { createSuggestionConfigSource } from './suggestion';

// CSS for Tiptap
const EDITOR_STYLES = `
.sdk-creation-chat-input .ProseMirror {
    outline: none;
...
.sdk-creation-chat-input .ProseMirror p {
    margin-bottom: 0;
    line-height: 1.6;
}

.sdk-creation-chat-input .ProseMirror p + p {
    margin-top: 0.5em;
}

.sdk-creation-chat-input .ProseMirror::-webkit-scrollbar {
  width: 4px;
}
.sdk-creation-chat-input .ProseMirror::-webkit-scrollbar-track {
  background: transparent;
}
.sdk-creation-chat-input .ProseMirror::-webkit-scrollbar-thumb {
  background-color: rgba(255, 255, 255, 0.1);
  border-radius: 4px;
}
.sdk-creation-chat-input .ProseMirror::-webkit-scrollbar-thumb:hover {
  background-color: rgba(255, 255, 255, 0.2);
}

/* Mention Chip Styling - Updated for "Label" look */
.sdk-creation-chat-input .mention-chip {
    color: #60a5fa;
...
.sdk-creation-chat-input .mention-chip:hover {
...
export const CreationChatInput: React.FC<CreationChatInputProps> = ({
    ...
    const [suggestionSource] = useState(() => createSuggestionConfigSource(attachments));
    const placeholder = placeholderProp || t('assetCenter.creationInput.placeholder');
    const showPlaceholderOverlay = value.trim().length === 0;

    // Keep ref updated for the suggestion closure
    useEffect(() => {
        attachmentsRef.current = attachments;
        suggestionSource.setAttachments(attachments);
    }, [attachments, suggestionSource]);
    ...
    const editor = useEditor({
        extensions: [
            ...
            Mention.configure({
                ...
                suggestion: suggestionSource.config,
            }),
        ],
        editorProps: {
            attributes: {
                class: `sdk-creation-chat-input__editor prose prose-invert max-w-none focus:outline-none ${textareaClassName}`,
            ...
            handleClick: (_view: any, _pos: number, event: MouseEvent) => {
                ...
                if (target.classList.contains('mention-chip')) {
                    ...
                    if (id) {
                        const index = attachmentsRef.current.findIndex((attachment) =>
                            matchesInputAttachmentKey(attachment, id)
                        );
    ...
    const wrapperClass = `sdk-creation-chat-input relative z-10 group ${className} ${!width ? 'w-full' : ''}`;
                   ...
                   <div className={`flex items-start gap-4 w-full px-5 ${attachments.length > 0 ? 'pb-4 pt-2' : 'py-5'}`}>
                        ...
                        <div className="relative flex-1 min-w-0 cursor-text">
                            {showPlaceholderOverlay && (
                                <div
                                    className="pointer-events-none absolute inset-x-0 top-0 z-10 overflow-hidden text-[16px] leading-[1.6] text-[#71717a]"
                                    data-creation-chat-input-placeholder=""
                                    aria-hidden="true"
                                >
                                    {placeholder}
                                </div>
