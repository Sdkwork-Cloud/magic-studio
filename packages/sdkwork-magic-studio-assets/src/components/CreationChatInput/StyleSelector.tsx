...
import { Popover, findByIdOrFirst } from '@sdkwork/magic-studio-commons';
import { getPlatformRuntime } from '@sdkwork/magic-studio-core/platform';
import { resolveLocalizedText, type LocalizedTextLike, useTranslation } from '@sdkwork/magic-studio-i18n';
...
export const StyleSelector: React.FC<StyleSelectorProps> = ({ 
    ...
    const { url: scenePreviewUrl } = useAssetUrl(resolveStyleAssetReference(displayOption?.assets?.scene));
    const { url: portraitPreviewUrl } = useAssetUrl(resolveStyleAssetReference(displayOption?.assets?.portrait));
    const { url: sheetPreviewUrl } = useAssetUrl(resolveStyleAssetReference(displayOption?.assets?.sheet));
    
    const getLocalizedLabel = useCallback((option?: StyleOption | null) => {
        if (!option) return '';
        return t(`portalVideo.styles.${option.id}.label`, resolveLocalizedText(option.label || option.id, locale));
    }, [locale, t]);

    const getLocalizedDescription = useCallback((option?: StyleOption | null) => {
        if (!option) return '';
    ...
    }, [getLocalizedLabel, options, searchQuery]);
                        ...
                        <div className="grid grid-cols-4 gap-2">
                            {filteredOptions.map((style) => (
                                <StyleOptionCard
                                    key={style.id}
                                    style={style}
                                    isActive={style.id === value}
                                    isHovered={style.id === displayOption?.id}
                                    label={getLocalizedLabel(style)}
                                    onHover={() => setHoveredStyleId(style.id)}
                                    onSelect={() => { onChange(style.id); handleClose(); }}
                                />
                            ))}
                        </div>
...
const StyleOptionCard: React.FC<{
    ...
    onHover: () => void;
    onSelect: () => void;
}> = ({ style, isActive, isHovered, label, onHover, onSelect }) => {
    ...
    return (
        ...
        >
            <div className="w-full aspect-[4/3] rounded bg-black overflow-hidden relative border border-white/5">
                {thumbnail ? (
                    <img src={thumbnail} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" alt="" />
                ) : (
