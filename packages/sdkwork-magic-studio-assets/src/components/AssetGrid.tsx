...
import { resolveAssetSelectionKey } from './assetSelectionIdentity';
...
export const AssetGrid: React.FC<AssetGridProps> = ({
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4 p-6 content-start pb-20">
            {assets.map(asset => (
                <AssetCard
                    key={resolveAssetSelectionKey(asset)}
                    asset={asset}
                    selected={selectedAssetIds.some((key) => matchesEntityKey(asset, key))}
