
import React, { useState, useRef, useEffect } from 'react';
import { MessageSquare, Upload, ChevronDown, Image as ImageIcon, Film, Music, FileAudio, User } from 'lucide-react';
import { GenerateHistory, type GenerationResultSelection, type GenerationTaskRecord } from '@sdkwork/magic-studio-generation-history';
import { type ImportData, type ImportDataType } from './upload/types';
import { UploadImageGenerationModal } from './upload/UploadImageGenerationModal';
import { UploadVideoGenerationModal } from './upload/UploadVideoGenerationModal';
import { UploadMusicGenerationModal } from './upload/UploadMusicGenerationModal';
import { UploadAudioGenerationModal } from './upload/UploadAudioGenerationModal';
import { UploadCharacterGenerationModal } from './upload/UploadCharacterGenerationModal';
import { useTranslation } from '@sdkwork/magic-studio-i18n';

export interface HistoryTab {
    id: string;
    label: string;
}

// Unified Global Tabs Definition with Translation Keys
export const GENERATION_TABS: HistoryTab[] = [
    { id: 'all', label: 'studio.tabs.all' },
    { id: 'image', label: 'studio.tabs.image' },
    { id: 'character', label: 'studio.tabs.character' },
    { id: 'video', label: 'studio.tabs.video' },
    { id: 'audio', label: 'studio.tabs.audio' },
    { id: 'sfx', label: 'studio.tabs.sfx' },
    { id: 'music', label: 'studio.tabs.music' },
    { id: 'voice', label: 'studio.tabs.voice' },
    { id: 'speech', label: 'studio.tabs.speech' },
];

export interface GenerationHistoryListPaneProps<T extends GenerationTaskRecord = GenerationTaskRecord> {
    // Data Source
    tasks: T[];
    
    // Core Actions
    onDelete: (id: string) => void;
    onReuse: (task: T) => void;
    
    // Selection Mode (Open/Closed Principle extension)
    /** Enables selection mode styling and behavior */
    selectionMode?: boolean;
    /** Callback when a generated result is selected */
    onSelect?: (selection: GenerationResultSelection, task: T) => void;
    /** List of currently selected item keys (for multi-select visual state) */
    selectedKeys?: string[];

    // Layout & Appearance
    /** Hides the top toolbar for embedded use cases */
    minimal?: boolean;
    /** Optional styling override */
    className?: string;
    /** Inject a custom header element (e.g. for Modals) */
    customHeader?: React.ReactNode;

    // Tabs Control (Optional)
    tabs?: HistoryTab[]; 
    activeTab?: string;
    onTabChange?: (id: string) => void;
    
    // Feature: Chat Mode
    onChatMode?: () => void;
    
    // Feature: Filtering
    filter?: string; 
    
    // Feature: Favorites
    showFavorites?: boolean;
    onToggleFavorites?: (show: boolean) => void;

    // Feature: Import
    onImport?: (data: ImportData) => void;
    importTypes?: ImportDataType[];
}

type ImportModalType = ImportDataType;

const IMPORT_TYPE_OPTIONS: Record<
    ImportModalType,
    {
        label: string;
        icon: React.ComponentType<{ size?: number; className?: string }>;
        className: string;
    }
> = {
    image: {
        label: 'Import Image',
        icon: ImageIcon,
        className: 'text-purple-400',
    },
    video: {
        label: 'Import Video',
        icon: Film,
        className: 'text-pink-400',
    },
    music: {
        label: 'Import Music',
        icon: Music,
        className: 'text-indigo-400',
    },
    audio: {
        label: 'Import Audio',
        icon: FileAudio,
        className: 'text-cyan-400',
    },
    character: {
        label: 'Import Character',
        icon: User,
        className: 'text-blue-400',
    },
};

export function GenerationHistoryListPane<T extends GenerationTaskRecord = GenerationTaskRecord>({
    tasks,
    onDelete,
    onReuse,
    selectionMode = false,
    onSelect,
    selectedKeys = [],
    minimal = false,
    className = "",
    customHeader,
    tabs = GENERATION_TABS,
    activeTab,
    onTabChange,
    onChatMode,
    filter = 'all',
    showFavorites = false,
    onToggleFavorites,
    onImport,
    importTypes = ['image', 'video', 'music']
}: GenerationHistoryListPaneProps<T>) {
    const { t } = useTranslation();
    const [showImportMenu, setShowImportMenu] = useState(false);
    const [activeModal, setActiveModal] = useState<ImportModalType | null>(null);
    const importMenuRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            if (importMenuRef.current && !importMenuRef.current.contains(e.target as Node)) {
                setShowImportMenu(false);
            }
        };
        document.addEventListener('mousedown', handleClickOutside);
        return () => document.removeEventListener('mousedown', handleClickOutside);
    }, []);

    const handleOpenUpload = (type: ImportModalType) => {
        setActiveModal(type);
        setShowImportMenu(false);
    };

    return (
        <div className={`flex flex-col w-full h-full min-w-0 bg-[#09090b] ${className}`}>
            
            {/* 1. Custom Header Injection (Highest Priority) */}
            {customHeader && (
                <div className="flex-none z-10">
                    {customHeader}
                </div>
            )}

            {/* 2. Standard Header (Conditional) */}
            {!minimal && !customHeader && (
                <div className="h-14 border-b border-[#27272a] flex items-center justify-between px-6 bg-[#18181b] flex-none z-10">
                     <div className="flex items-center gap-6 h-full overflow-x-auto no-scrollbar">
                        {onTabChange && tabs.map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => onTabChange(tab.id)}
                                className={`
                                    relative h-full flex items-center text-sm font-medium transition-colors border-b-2 whitespace-nowrap
                                    ${activeTab === tab.id 
                                        ? 'text-white border-[#4ade80]' 
                                        : 'text-gray-500 border-transparent hover:text-gray-300'
                                    }
                                `}
                            >
                                {t(tab.label)}
                            </button>
                        ))}
                        
                        {!onTabChange && (
                            <span className="text-sm font-bold text-gray-200">History</span>
                        )}
                     </div>
                     
                     <div className="flex items-center gap-3">
                        {onImport && (
                            <div className="relative" ref={importMenuRef}>
                                <button 
                                    onClick={() => setShowImportMenu(!showImportMenu)}
                                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-400 hover:text-white bg-[#27272a] hover:bg-[#333] rounded-lg transition-colors border border-[#333]"
                                >
                                    <Upload size={14} />
                                    <span className="hidden sm:inline">Import</span>
                                    <ChevronDown size={12} className="opacity-50" />
                                </button>
                                
                                {showImportMenu && (
                                    <div className="absolute top-full right-0 mt-1 w-40 bg-[#252526] border border-[#333] rounded-xl shadow-xl py-1 z-50 animate-in fade-in zoom-in-95 duration-75">
                                        {importTypes.map((type) => {
                                            const option = IMPORT_TYPE_OPTIONS[type];
                                            const Icon = option.icon;
                                            return (
                                                <button
                                                    key={type}
                                                    onClick={() => handleOpenUpload(type)}
                                                    className="flex items-center gap-2 w-full px-3 py-2 text-xs text-left text-gray-300 hover:bg-[#333] hover:text-white transition-colors"
                                                >
                                                    <Icon size={14} className={option.className} /> {option.label}
                                                </button>
                                            );
                                        })}
                                    </div>
                                )}
                            </div>
                        )}
    
                        <div className="w-[1px] h-4 bg-[#333]" />
    
                        {onToggleFavorites && (
                            <div className="flex items-center gap-2">
                                 <input 
                                    type="checkbox" 
                                    id="show-favorites" 
                                    checked={showFavorites}
                                    onChange={(e) => onToggleFavorites(e.target.checked)}
                                    className="w-4 h-4 rounded bg-[#27272a] border-gray-600 text-[#4ade80] focus:ring-0 cursor-pointer" 
                                 />
                                 <label htmlFor="show-favorites" className="text-xs text-gray-400 cursor-pointer select-none hover:text-gray-300 hidden sm:inline">
                                     Favorites
                                 </label>
                            </div>
                        )}
    
                        {onChatMode && (
                            <>
                                <div className="w-[1px] h-4 bg-[#333] hidden sm:block" />
                                <button 
                                    onClick={onChatMode}
                                    className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-gray-400 hover:text-white bg-[#27272a] hover:bg-[#333] rounded-lg transition-colors border border-[#333]"
                                >
                                    <MessageSquare size={14} /> <span className="hidden sm:inline">Chat</span>
                                </button>
                            </>
                        )}
                     </div>
                </div>
            )}
            
            {/* 3. The List Content */}
            <div className="flex-1 min-h-0 overflow-hidden relative">
                {tasks.length === 0 ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-600 bg-transparent select-none p-8">
                        <div className="w-20 h-20 rounded-3xl bg-[#18181b] border border-[#27272a] flex items-center justify-center mb-4 shadow-inner">
                            <ImageIcon size={32} className="opacity-20" />
                        </div>
                        <h3 className="text-base font-bold text-gray-300">{t('studio.history.empty_title') || 'No History Yet'}</h3>
                        <p className="text-xs opacity-50 mt-2 max-w-[200px] text-center leading-relaxed">
                            {t('studio.history.empty_description') || 'Start creating to see your generations here.'}
                        </p>
                    </div>
                ) : (
                    <GenerateHistory 
                        tasks={tasks} 
                        onDelete={onDelete} 
                        onReuse={onReuse}
                        onSelect={selectionMode ? onSelect : undefined}
                        selectedKeys={selectedKeys}
                        order="desc" 
                        filter={filter}
                    />
                )}
            </div>

            {/* Modals */}
            {activeModal === 'image' && onImport && (
                <UploadImageGenerationModal onClose={() => setActiveModal(null)} onImport={onImport} />
            )}
            {activeModal === 'video' && onImport && (
                <UploadVideoGenerationModal onClose={() => setActiveModal(null)} onImport={onImport} />
            )}
            {activeModal === 'music' && onImport && (
                <UploadMusicGenerationModal onClose={() => setActiveModal(null)} onImport={onImport} />
            )}
            {activeModal === 'audio' && onImport && (
                <UploadAudioGenerationModal onClose={() => setActiveModal(null)} onImport={onImport} />
            )}
            {activeModal === 'character' && onImport && (
                <UploadCharacterGenerationModal onClose={() => setActiveModal(null)} onImport={onImport} />
            )}
        </div>
    );
}
