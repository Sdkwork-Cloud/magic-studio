...
import { Button, AudioUpload, ImageUpload } from '@sdkwork/magic-studio-commons';
...
import { SettingInput, SettingToggle } from '@sdkwork/magic-studio-settings';
...
interface SourceInput {
    id: string;
    label: string;
    resource: ImportDataResource | null;
    prompt: string;
}
...
export const UploadMusicGenerationModal: React.FC<UploadMusicGenerationModalProps> = ({ onClose, onImport }) => {
    ...
    const handleModeChange = (newMode: GenerationMode) => {
        ...
        if (newMode === 'text') {
            ...
            setSourceInputs([
                { id: 'ref1', label: 'Reference Audio', resource: null, prompt: '' }
            ]);
        }
    };
                ...
                >
                        ...
                        <div className="col-span-12 lg:col-span-4 bg-[#121214] p-8 overflow-y-auto custom-scrollbar flex flex-col h-full">
                            ...
                            <div className="w-full aspect-square mb-4 relative group bg-[#111] rounded-xl overflow-hidden border border-[#27272a] shadow-md">
                                <ImageUpload
                                    ...
                                    onPreview={() => coverResource?.url && setPreviewData({ url: coverResource.url, type: 'image', title: 'Cover Art' })}
                            ...
                            <div className="flex-none flex flex-col relative group">
                                <AudioUpload 
                                    ...
                                    assetUrlResolver={resolveCanonicalUploadAssetUrl}
                                    onChange={(file) => {
                                        setResource(createImportDataResourceFromUpload(file, 'music'));
                                        if (!title) setTitle(file.name.replace(/\.[^/.]+$/, ""));
                                    }}
                                    onRemove={() => setResource(null)}
                                    onPreview={() => resource?.url && setPreviewData({ url: resource.url, type: 'audio', title: title || 'Audio Track' })}
                                    aspectRatio="h-24"
                                    label="Upload Audio Track (MP3/WAV)"
                                    className={`bg-[#1c1c1f] border-[#27272a] shadow-inner hover:border-indigo-500/30 transition-colors ${resource ? 'border-indigo-500/50 bg-indigo-900/10' : ''}`}
                                />
                            </div>
                                            ...
                                            {sourceInputs.map((input) => (
                                                <div key={input.id} className="flex gap-4 p-4 bg-[#202023] rounded-xl border border-[#27272a] group items-start hover:border-[#3f3f46] transition-colors shadow-sm">
                                                    ...
                                                    <div className="w-24 h-24 flex-shrink-0 relative">
                                                        <AudioUpload 
                                                            value={input.resource?.url ?? null}
                                                            assetUrlResolver={resolveCanonicalUploadAssetUrl}
                                                            onChange={(file) => updateSourceInput(input.id, 'resource', createImportDataResourceFromUpload(file, 'audio'))}
                                                            onRemove={() => updateSourceInput(input.id, 'resource', null)}
                                                            onPreview={() => input.resource?.url && setPreviewData({ url: input.resource.url, type: 'audio', title: input.label })}
                                                            aspectRatio="aspect-square"
                                                            label="Ref Audio"
                    ...
                    <div className="flex-none h-20 px-8 bg-[#18181b] border-t border-[#27272a] flex items-center justify-between z-10">
                        <div className="text-xs text-gray-500">
                            {resource ? 'Ready to import' : 'Please upload the audio file'}
                        </div>
