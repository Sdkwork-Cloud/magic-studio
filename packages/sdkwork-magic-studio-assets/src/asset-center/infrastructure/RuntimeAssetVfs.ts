...
import type { AssetStorageMode } from '@sdkwork/magic-studio-types/asset-center';
...
export class RuntimeAssetVfs implements AssetVfsPort {
  getMode(): AssetStorageMode {
    return isDesktopShellRuntimeKind(getPlatformRuntime().system.kind()) ? 'desktop-fs' : 'browser-vfs';
  }

  async getMagicStudioStorageConfig() {
    const homeDir = await getPlatformRuntime().system.path('home');
  ...
  async toAbsolutePath(virtualPath: string): Promise<string> {
    ...
    const storageConfig = await this.getMagicStudioStorageConfig();
    return resolveMagicStudioAssetAbsolutePath(storageConfig, virtualPath);
  }

  async toVirtualPath(absolutePath: string): Promise<string> {
    const storageConfig = await this.getMagicStudioStorageConfig();
    return resolveMagicStudioAssetVirtualPath(storageConfig, absolutePath);
  }
}
