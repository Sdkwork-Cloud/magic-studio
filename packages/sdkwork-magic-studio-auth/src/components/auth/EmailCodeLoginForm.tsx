...
import { looksLikeEmailAddress, readErrorMessage } from './authConfig';
import { useActionCooldown } from './useActionCooldown';
import { VerificationCodeField } from './VerificationCodeField';

interface EmailCodeLoginFormProps {
  initialEmail?: string;
  onSubmit: (payload: { code: string; email: string }) => Promise<void>;
}
...
export function EmailCodeLoginForm({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!isEmailValid) {
      sdkToast.error(t('auth.errors.invalidEmail'));
      return;
    }
