...
interface ForgotPasswordFlowProps {
  ...
  onRequestReset: (payload: AppAuthPasswordResetRequestInput) => Promise<void>;
  onSubmit: (payload: AppAuthPasswordResetInput) => Promise<void>;
}
...
export function ForgotPasswordFlow({
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      startCooldown();
      sdkToast.success(t('auth.toasts.resetCodeSent'));
    } catch (error) {
      sdkToast.error(readErrorMessage(error, t('auth.errors.resetPasswordFailed')));
    } finally {
      setIsSendingCode(false);
    }
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (newPassword !== confirmPassword) {
      sdkToast.error(t('auth.errors.passwordMismatch'));
      return;
    }
    ...
    try {
      ...
      sdkToast.success(t('auth.toasts.passwordResetSuccess'));
    } catch (error) {
      sdkToast.error(readErrorMessage(error, t('auth.errors.resetPasswordFailed')));
    } finally {
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder={method === 'phone' ? t('auth.placeholders.phone') : t('auth.placeholders.email')}
