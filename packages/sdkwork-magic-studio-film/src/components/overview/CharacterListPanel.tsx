...
import { FilmCharacter } from '@sdkwork/magic-studio-types/film';
...
export const CharacterListPanel: React.FC<CharacterListPanelProps> = ({ 
    ...
    const { bindProjectAsset } = useFilmStore();
    ...
    const handleGenerateAll = async () => {
        ...
        for (const char of characters) {
            ...
            try {
                const outcome = await filmService.generateImage(prompt, '3:4');
                const imported = await resolveFilmGeneratedOutcomeAsset(
                    outcome,
                    `${char.name}_avatar`,
                    'image',
                    {
                ...
                await bindProjectAsset({
                    entityType: 'character',
                    entityId: char.uuid,
                    targetField: 'refAssets',
                    mode: 'set-primary',
                    asset: {
