...
import { SettingTextArea, SettingSlider } from '@sdkwork/magic-studio-settings';
import { Button, ModelSelector } from '@sdkwork/magic-studio-commons';
...
import type { GenerationProduct, AssetAtomicMediaResource } from '@sdkwork/magic-studio-types/media';
import { MediaScene, MediaResourceType } from '@sdkwork/magic-studio-types/vocabulary';
...
import { createInputAttachment, type InputAttachment } from '@sdkwork/magic-studio-assets/creation-chat';
import { PromptTextInput, createPromptTextInputCapabilityProps } from '@sdkwork/magic-studio-assets/generation';
import { detectAssetTypeByFilename } from '@sdkwork/magic-studio-assets/asset-center';
import { AIImageGeneratorModal } from '@sdkwork/magic-studio-image/modals';
import {
...
import { enhanceFilmPrompt, filmPreferencesService } from '../services';
import {
...
import { resolveFilmAssetKey } from '../utils/filmAssetIdentity';
...
export const ShotModal: React.FC<ShotModalProps> = ({ isOpen, onClose, onSave, initialData, sceneIndex, characters = [] }) => {
    ...
    useEffect(() => {
        if (isOpen && initialData) {
            ...
            const normalizedAssets: AssetAtomicMediaResource[] = loadedAssets.map((asset, idx) => {
                ...
                const baseId = resolveAssetRecordId(asset) || generateUUID();
                const baseUuid = resolveAssetRecordClientUuid(asset) || baseId;
                const baseAssetUuid = resolveAssetRecordAssetUuid(asset);
                const normalized = buildAtomicAssetResource({
                    assetId: baseId,
                    uuid: baseUuid,
                    assetUuid: baseAssetUuid,
                    type,
                    ...
                    metadata: {
                        ...(asset.metadata || {}),
                        assetId: baseId,
                        ...(baseAssetUuid ? { assetUuid: baseAssetUuid } : {})
                    }
                });
                return {
                    ...asset,
    ...
    const handleEnhancePrompt = async (currentText: string): Promise<string> => {
        ...
        try {
            ...
            const enhanced = await enhanceFilmPrompt(baseText);
    ...
    const inputAttachments = useMemo((): InputAttachment[] => {
        return assets.filter(a => a.url).map((asset, idx) => createInputAttachment({
    ...
    const handleAIModalSuccess = async (result: GenerationResultSelection | GenerationResultSelection[]) => {
        const selection = Array.isArray(result) ? result[0] : result;
        if (!selection) {
            setShowAIModal(false);
            return;
        }

        const finalUrl = resolveGenerationResultDeliveryUrl(selection);
        if (activeAssetIndex !== null) {
            ...
            if (currentAsset) {
                try {
                    ...
                    const imported = await resolveFilmGeneratedSelectionAsset(
                        selection,
    ...
    const renderAssetSlot = (asset: AssetAtomicMediaResource, index: number) => {
        ...
        return (
            <div key={resolveFilmAssetKey(asset)} className="space-y-3 group/slot">
