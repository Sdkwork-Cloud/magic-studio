...
import { Asset } from '@sdkwork/magic-studio-types/assets';
import { enhanceFilmPrompt } from '../services';
...
export const CharacterModal: React.FC<CharacterModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
    ...
    const handleEnhancePrompt = (type: ImageType) => async (text: string) => {
        const enhanced = await enhanceFilmPrompt(text);
    ...
    const handleFileUpload = (type: ImageType) => {
        ...
        input.onchange = async (e) => {
            ...
            try {
                ...
                setImage(type, resolveImportedFilmAssetUrl(imported), imported.assetId, imported.uuid, imported.assetUuid);
            } catch (err) {
    ...
    const handleAssetSelected = async (assets: Asset[]) => {
        if (assets.length > 0 && activeAssetModal) {
            ...
            const selectedUrl = resolveImportedFilmAssetUrl(selected);
            if (selectedUrl) {
                setImage(activeAssetModal, selectedUrl, selected?.assetId, selected?.uuid, selected?.assetUuid);
            }
        }
            ...
            {activeAIModal && (
                <AIImageGeneratorModal 
                    ...
                    onSuccess={async (selection) => {
                        ...
                        setImage(activeAIModal, resolveImportedFilmAssetUrl(imported), imported.assetId, imported.uuid, imported.assetUuid);
