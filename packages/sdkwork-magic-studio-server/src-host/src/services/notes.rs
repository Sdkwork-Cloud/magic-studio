...
impl FileBackedNoteService {
    ...
    fn acquire_lock(&self) -> ServerResult<MutexGuard<'_, ()>> {
        self.lock.lock().map_err(|_| {
            ServerError::internal("APP_NOTES_LOCK_FAILED", "notes registry lock was poisoned")
        })
    }
        ...
        let mut document =
            serde_json::from_str::<NoteRegistryDocument>(&contents).map_err(|error| {
                ServerError::internal(
                    "APP_NOTES_PARSE_FAILED",
                    format!("failed to parse notes registry {}: {error}", path.display()),
                )
            })?;
    ...
    fn persist_to_disk(&self, document: &NoteRegistryDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_vec_pretty(document).map_err(|error| {
            ServerError::internal(
                "APP_NOTES_SERIALIZE_FAILED",
                format!("failed to serialize notes registry: {error}"),
            )
        })?;
        fs::write(self.storage_paths.notes_registry_file(), contents).map_err(|error| {
            ServerError::internal(
                "APP_NOTES_WRITE_FAILED",
                format!(
        ...
        folders
            ...
            .ok_or_else(|| {
                ServerError::not_found(
                    "APP_NOTE_FOLDER_NOT_FOUND",
                    format!("note folder {folder_key} was not found"),
                )
            })
    }
    ...
    fn read_note_record<'a>(
        ...
        notes
            ...
            .ok_or_else(|| {
                ServerError::not_found(
                    "APP_NOTE_NOT_FOUND",
                    format!("note {note_key} was not found"),
                )
            })
    }
        ...
        for folder in folders {
            ...
            if folder.name.eq_ignore_ascii_case(name) {
                return Err(ServerError::conflict(
                    "APP_NOTE_FOLDER_NAME_CONFLICT",
                    format!("a note folder named {name} already exists in the target location"),
                ));
            }
        }
        Ok(())
    ...
    fn ensure_folder_empty(
        ...
        {
            return Err(ServerError::conflict(
                "APP_NOTE_FOLDER_NOT_EMPTY",
                "note folder cannot be deleted while it still contains child folders",
            ));
        }
            ...
            .any(|note| note.parent_id.as_deref() == Some(folder_id))
        {
            return Err(ServerError::conflict(
                "APP_NOTE_FOLDER_NOT_EMPTY",
                "note folder cannot be deleted while it still contains notes",
            ));
        }
...
impl NoteService for FileBackedNoteService {
    ...
    fn move_folder(&self, folder_key: &str, input: NoteFolderMoveRequest) -> ServerResult<()> {
        ...
        if target_parent_id.as_deref() == Some(folder_id.as_str()) {
            return Err(ServerError::conflict(
                "APP_NOTE_FOLDER_MOVE_SELF",
                "note folder cannot be moved into itself",
            ));
        }
        ...
        if let Some(target_parent_id) = target_parent_id.as_deref() {
            if self.is_descendant_folder(&document.folders, target_parent_id, &folder_id)? {
                return Err(ServerError::conflict(
                    "APP_NOTE_FOLDER_MOVE_DESCENDANT",
                    "note folder cannot be moved into one of its descendants",
                ));
            }
        }
    ...
    fn publish_note(
...

        {
            ...
            if note.deleted_at.is_some() {
                return Err(ServerError::conflict(
                    "APP_NOTE_TRASHED",
                    "trashed notes cannot be published",
                ));
            }
...
fn require_note_folder_name(value: &str, field_name: &str) -> ServerResult<String> {
    ...
    {
        return Err(ServerError::bad_request(
            "APP_NOTE_FOLDER_NAME_INVALID",
            format!("{field_name} contains characters not allowed in note folder names"),
        ));
    }
    Ok(name)
}
