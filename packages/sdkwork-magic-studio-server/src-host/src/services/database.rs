...
pub trait DatabaseService: Send + Sync {
    ...
    fn execute_batch(&self, db_path: String, sql_batch: String) -> ServerResult<()>;
}

pub struct SqliteDatabaseService {
    file_system_service: std::sync::Arc<dyn FileSystemService>,
...
impl SqliteDatabaseService {
    ...
    fn ensure_db_path_allowed(&self, db_path: &str, access: PathAccessType) -> ServerResult<()> {
        self.policy_service
...
impl Default for SqliteDatabaseService {
    fn default() -> Self {
        Self {
            file_system_service: std::sync::Arc::new(LocalFileSystemService::default()),
...
impl DatabaseService for SqliteDatabaseService {
    fn execute(
        ...
        let connection = Connection::open(&db_path)
            .map_err(|error| ServerError::internal("DATABASE_OPEN_FAILED", error.to_string()))?;
        ...
        let affected_rows = connection
            ...
            .map_err(|error| {
                ServerError::bad_request("DATABASE_EXECUTE_FAILED", error.to_string())
            })?;
    ...
    fn query(
        ...
        if !self.file_system_service.exists(db_path.clone())? {
        ...
        let mut statement = connection.prepare(&sql).map_err(|error| {
            ServerError::bad_request("DATABASE_PREPARE_FAILED", error.to_string())
        })?;
        ...
        let rows = statement
            ...
            .map_err(|error| {
                ServerError::bad_request("DATABASE_QUERY_FAILED", error.to_string())
            })?;

        let mut output = Vec::new();
        for row in rows {
            output.push(row.map_err(|error| {
                ServerError::internal("DATABASE_ROW_READ_FAILED", error.to_string())
            })?);
        }
    ...
    fn execute_batch(&self, db_path: String, sql_batch: String) -> ServerResult<()> {
        ...
        connection.execute_batch(&sql_batch).map_err(|error| {
            ServerError::bad_request("DATABASE_EXECUTE_BATCH_FAILED", error.to_string())
        })
    }
}
