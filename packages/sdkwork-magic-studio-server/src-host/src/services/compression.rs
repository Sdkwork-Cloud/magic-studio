...
impl CompressionService for NativeCompressionService {
    fn unzip(&self, zip_path: String, target_dir: String) -> ServerResult<()> {
        let file = File::open(&zip_path).map_err(|error| {
            ServerError::internal("COMPRESSION_OPEN_ZIP_FAILED", error.to_string())
        })?;
        let mut archive = zip::ZipArchive::new(file).map_err(|error| {
            ServerError::internal("COMPRESSION_READ_ZIP_FAILED", error.to_string())
        })?;

        for index in 0..archive.len() {
            let mut file = archive.by_index(index).map_err(|error| {
                ServerError::internal("COMPRESSION_READ_ZIP_ENTRY_FAILED", error.to_string())
            })?;
            ...
            if file.name().ends_with('/') {
                ...
                let mut outfile = File::create(&outpath).map_err(|error| {
                    ServerError::internal("COMPRESSION_CREATE_FILE_FAILED", error.to_string())
                })?;
                io::copy(&mut file, &mut outfile).map_err(|error| {
                    ServerError::internal("COMPRESSION_COPY_ENTRY_FAILED", error.to_string())
                })?;
            }
        }
    ...
    fn zip_bytes(&self, source_paths: Vec<String>) -> ServerResult<Vec<u8>> {
        if source_paths.is_empty() {
            return Err(ServerError::bad_request(
                "COMPRESSION_SOURCE_PATHS_EMPTY",
                "sourcePaths must contain at least one path",
            ));
        }
        ...
        let mut buffer = Vec::new();
        {
            ...
            for source_path in source_paths {
                if source_path.trim().is_empty() {
                    return Err(ServerError::bad_request(
                        "COMPRESSION_SOURCE_PATH_EMPTY",
                        "source path cannot be empty",
                    ));
                }
                if !self.file_system_service.exists(source_path.clone())? {
                    return Err(ServerError::bad_request(
                        "COMPRESSION_SOURCE_PATH_NOT_FOUND",
                ...
                for entry in walk_dir.into_iter().filter_map(|entry| entry.ok()) {
                    ...
                    let name = path.strip_prefix(parent_dir).map_err(|error| {
                        ServerError::internal("COMPRESSION_STRIP_PREFIX_FAILED", error.to_string())
                    })?;
                    ...
                    if path.is_file() {
                        ...
                        zip.add_directory(name_str, options).map_err(|error| {
                            ServerError::internal(
                                "COMPRESSION_ADD_ZIP_DIR_FAILED",
                                error.to_string(),
                            )
                        })?;
                    }
            ...
            zip.finish().map_err(|error| {
                ServerError::internal("COMPRESSION_FINISH_ZIP_FAILED", error.to_string())
            })?;
        }
