...
impl FileBackedPortalService {
    ...
    fn load_from_disk(&self) -> ServerResult<PortalDocument> {
        ...
        match fs::read_to_string(path) {
            ...
            Err(error) => Err(ServerError::internal(
                "APP_PORTAL_REGISTRY_READ_FAILED",
                format!("failed to read portal registry {}: {error}", path.display()),
            )),
        }
    }
    ...
    fn persist_to_disk(&self, document: &PortalDocument) -> ServerResult<()> {
        let parent = self
            ...
            .ok_or_else(|| {
                ServerError::internal(
                    "APP_PORTAL_REGISTRY_PARENT_MISSING",
                    "portal registry parent directory could not be resolved",
                )
            })?;

        fs::create_dir_all(parent).map_err(|error| {
            ServerError::internal(
                "APP_PORTAL_REGISTRY_DIR_CREATE_FAILED",
                format!(
        ...
        let contents = serde_json::to_string_pretty(document).map_err(|error| {
            ServerError::internal(
                "APP_PORTAL_REGISTRY_SERIALIZE_FAILED",
                format!("failed to serialize portal registry: {error}"),
            )
        })?;

        fs::write(self.storage_paths.portal_registry_file(), contents).map_err(|error| {
            ServerError::internal(
                "APP_PORTAL_REGISTRY_WRITE_FAILED",
                format!(
    ...
    fn lock_document(&self) -> ServerResult<MutexGuard<'_, ()>> {
        self.lock.lock().map_err(|_| {
            ServerError::internal("APP_PORTAL_LOCK_FAILED", "portal registry lock is poisoned")
        })
    }
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(
                    "APP_PORTAL_FEED_NOT_FOUND",
                    format!("portal feed {trimmed} was not found"),
                )
            })
    }
    ...
    fn delete_feed(&self, feed_key: &str) -> ServerResult<()> {
        ...
        if feed.author_uuid != actor.user_uuid {
            return Err(ServerError::forbidden(
                "APP_PORTAL_FEED_DELETE_FORBIDDEN",
                "only the author can delete this portal feed",
            ));
        }
...
fn require_non_empty_text(value: String, code: &str, message: &str) -> ServerResult<String> {
    normalize_optional_text(Some(value)).ok_or_else(|| ServerError::bad_request(code, message))
}
