...
impl MigrationService {
    ...
    pub fn status(&self, db_path: String) -> ServerResult<MigrationStatus> {
        ...
        let connection = Connection::open(&db_path).map_err(|error| {
            ServerError::internal("MIGRATION_OPEN_DB_FAILED", error.to_string())
        })?;
        ...
        if let Some(parent) = Path::new(&db_path).parent() {
            std::fs::create_dir_all(parent).map_err(|error| {
                ServerError::internal(
                    "MIGRATION_CREATE_PARENT_DIR_FAILED",
                    format!("failed to create database parent dir: {error}"),
                )
            })?;
        }
    ...
    fn sqlite_millis_from_u64(value: u64) -> ServerResult<i64> {
        if value > i64::MAX as u64 {
            return Err(ServerError::internal(
                "MIGRATION_TIMESTAMP_OUT_OF_RANGE",
                format!("timestamp value exceeds SQLite INTEGER range: {value}"),
            ));
        }
    ...
    fn sqlite_millis_to_u64(value: i64) -> ServerResult<u64> {
        if value < 0 {
            return Err(ServerError::internal(
                ...
                format!("timestamp value cannot be negative: {value}"),
            ));
        }
