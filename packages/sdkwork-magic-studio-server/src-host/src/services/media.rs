...
impl SystemMediaService {
    ...
    fn ensure_input_path(&self, path: &str) -> ServerResult<()> {
        ...
        if !Path::new(trimmed).exists() {
            return Err(ServerError::bad_request(
                "MEDIA_INPUT_PATH_NOT_FOUND",
                format!("input path does not exist: {trimmed}"),
            ));
        }
        Ok(())
    }
        ...
        if let Some(parent) = Path::new(trimmed).parent() {
            if !parent.as_os_str().is_empty() {
                fs::create_dir_all(parent).map_err(|error| {
                    ServerError::internal("MEDIA_CREATE_OUTPUT_PARENT_FAILED", error.to_string())
                })?;
            }
        }
        Ok(())
    ...
    fn build_video_concat_manifest_path(&self) -> ServerResult<std::path::PathBuf> {
        let manifest_path = std::env::temp_dir().join(format!(
            ...
            std::time::SystemTime::now()
                ...
                .map_err(|error| {
                    ServerError::internal("MEDIA_VIDEO_CONCAT_TIME_FAILED", error.to_string())
                })?
                .as_nanos()
        ));
    ...
    fn build_image_resize_args(&self, request: ImageResizeRequest) -> ServerResult<Vec<String>> {
        ...
        if request.width == 0 {
            return Err(ServerError::bad_request(
                "MEDIA_IMAGE_RESIZE_WIDTH_INVALID",
                "image resize width must be greater than zero",
            ));
        }
        if request.height == 0 {
            return Err(ServerError::bad_request(
                "MEDIA_IMAGE_RESIZE_HEIGHT_INVALID",
                "image resize height must be greater than zero",
            ));
        }
    ...
    fn build_audio_mix_args(&self, request: AudioMixRequest) -> ServerResult<Vec<String>> {
        if request.inputs.is_empty() {
            return Err(ServerError::bad_request(
                "MEDIA_AUDIO_MIX_INPUTS_EMPTY",
                "audio mix requires at least one input",
            ));
        }
...
impl MediaService for SystemMediaService {
    ...
    fn video_concat_controlled(
        ...
        if input_paths.is_empty() {
            return Err(ServerError::bad_request(
                "MEDIA_VIDEO_CONCAT_INPUTS_EMPTY",
                "concat requires at least one input path",
            ));
        }
        ...
        fs::write(&manifest_path, manifest_text).map_err(|error| {
            ServerError::internal(
                "MEDIA_VIDEO_CONCAT_MANIFEST_WRITE_FAILED",
                error.to_string(),
            )
        })?;
    ...
    fn media_command_execute_controlled(
        ...
        let mut child = Command::new("ffmpeg")
            ...
            .spawn()
            .map_err(|error| {
                ServerError::internal("MEDIA_COMMAND_EXEC_FAILED", "media command failed to start")
                    .with_detail(error.to_string())
            })?;

        let stdout = child.stdout.take().ok_or_else(|| {
            ServerError::internal("MEDIA_COMMAND_STDOUT_MISSING", "stdout pipe missing")
        })?;
        let stderr = child.stderr.take().ok_or_else(|| {
            ServerError::internal("MEDIA_COMMAND_STDERR_MISSING", "stderr pipe missing")
        })?;
    ...
    fn media_probe(&self, input: String) -> ServerResult<Value> {
        ...
        let output = Command::new("ffprobe")
            ...
            .map_err(|error| {
                ServerError::internal(
                    ...
                    "media probe command failed to start",
                )
                .with_detail(error.to_string())
            })?;
        ...
        if !output.status.success() {
            return Err(
                ServerError::bad_request("MEDIA_PROBE_FAILED", "media probe failed")
...
mod tests {
    ...
    fn media_probe_empty_input_uses_neutral_error_contract() {
        ...
        assert_eq!(error.message, "media probe input is required");
    ...
    fn media_probe_non_zero_exit_uses_neutral_error_contract() {
        ...
        assert_eq!(error.message, "media probe failed");
    ...
    fn media_probe_invalid_json_uses_neutral_error_contract() {
        ...
        assert_eq!(error.message, "media probe response could not be parsed");
        assert!(error
            .detail
            .as_deref()
            .unwrap_or_default()
    ...
    fn media_command_empty_args_still_rejects_invalid_invocation() {
        ...
        assert_eq!(error.message, "media command args cannot be empty");
