...
impl CanonicalAudioExecutionService {
    ...
    fn configuration(&self) -> ServerResult<&AiProviderConfig> {
        self.config.as_ref().ok_or_else(|| {
            ServerError::internal(
                "APP_AI_AUDIO_PROVIDER_NOT_CONFIGURED",
                "AI audio execution is not configured. Set MAGIC_STUDIO_AI_BASE_URL together with MAGIC_STUDIO_AI_API_KEY or MAGIC_STUDIO_AI_AUTH_TOKEN plus MAGIC_STUDIO_AI_ACCESS_TOKEN.",
            )
        })
    }
    ...
    fn execute_audio_text_call(
        ...
        let client = Client::builder()
            ...
            .map_err(|error| {
                ServerError::internal(
                    "APP_AI_AUDIO_HTTP_CLIENT_CREATE_FAILED",
                    format!("failed to create AI audio http client: {error}"),
                )
            })?;
        ...
        let file_part = Part::bytes(source.bytes.clone())
            ...
            .map_err(|error| {
                ServerError::bad_request(
                    "APP_AI_AUDIO_SOURCE_MIME_INVALID",
                    format!("audio source mime type {mime_type} is invalid: {error}"),
                )
            })?;
        ...
        run_async(async move {
            ...
            let response = request.send().await.map_err(|error| {
                ServerError::internal(
                    "APP_AI_AUDIO_TEXT_REQUEST_FAILED",
                    format!("AI {operation_label} request failed: {error}"),
                )
            })?;
            let status = response.status();
            let body = response.bytes().await.map_err(|error| {
                ServerError::internal(
                    "APP_AI_AUDIO_TEXT_RESPONSE_READ_FAILED",
                    format!("failed to read AI {operation_label} response: {error}"),
                )
            })?;

            if !status.is_success() {
                return Err(ServerError::internal(
                    "APP_AI_AUDIO_TEXT_UPSTREAM_STATUS",
                    format!(
            ...
            serde_json::from_slice::<TranscriptionResponse>(&body).map_err(|error| {
                ServerError::internal(
                    "APP_AI_AUDIO_TEXT_RESPONSE_PARSE_FAILED",
                    format!("failed to parse AI {operation_label} response: {error}"),
                )
            })
        })
    }
    ...
    fn build_audio_text_query(
        ...
        if let Some(prompt) = input.prompt.clone().and_then(normalize_optional_text) {
    ...
    fn persist_speech_output(
        ...
        fs::create_dir_all(&namespace_dir).map_err(|error| {
            ServerError::internal(
                "APP_AI_SPEECH_OUTPUT_DIR_CREATE_FAILED",
                format!(
        ...
        fs::write(&output_path, bytes).map_err(|error| {
            ServerError::internal(
                "APP_AI_SPEECH_OUTPUT_WRITE_FAILED",
                format!(
    ...
    fn build_speech_artifact(
        ...
        if let Some(language) = input.language.clone().and_then(normalize_optional_text) {
            metadata.insert("language".to_string(), Value::String(language));
        }
        if let Some(voice) = input.voice.clone().and_then(normalize_optional_text) {
    ...
    fn build_audio_text_artifact(
        ...
        let text = response
            ...
            .ok_or_else(|| {
                ServerError::internal(
                    "APP_AI_AUDIO_TEXT_EMPTY_RESPONSE",
                    format!(
        ...
        if let Some(source_url) = source.source_url.clone().and_then(normalize_optional_text) {
            metadata.insert("sourceAudioUrl".to_string(), Value::String(source_url));
        }
        if let Some(source_path) = source.source_path.clone().and_then(normalize_optional_text) {
        ...
        if let Some(language) = response
            ...
            .clone()
            .and_then(normalize_optional_text)
            .or_else(|| input.language.clone().and_then(normalize_optional_text))
            .or_else(|| {
                input
                    .source_language
                    .clone()
    ...
    fn execute_audio_text(
        ...
        if let Some(language) = response.language.clone().and_then(normalize_optional_text) {
...
impl AudioExecutionService for CanonicalAudioExecutionService {
    ...
    fn synthesize(&self, input: SpeechExecutionRequest) -> ServerResult<SpeechExecutionResult> {
        ...
        if bytes.is_empty() {
            return Err(ServerError::internal(
                "APP_AI_SPEECH_EMPTY_RESPONSE",
                "AI speech execution returned an empty response body",
            ));
        }
...
impl CanonicalGenerationExecutionService {
    ...
    fn configuration(&self) -> ServerResult<&AiProviderConfig> {
        self.config.as_ref().ok_or_else(|| {
            ServerError::internal(
                "APP_AI_GENERATION_PROVIDER_NOT_CONFIGURED",
                "AI generation execution is not configured. Set MAGIC_STUDIO_AI_BASE_URL together with MAGIC_STUDIO_AI_API_KEY or MAGIC_STUDIO_AI_AUTH_TOKEN plus MAGIC_STUDIO_AI_ACCESS_TOKEN.",
            )
        })
    }
    ...
    fn build_image_request(
        ...
        let prompt = input.prompt.clone().and_then(normalize_optional_text);
        let reference_images = resolve_media_input_reference_values(&input.input_refs)?;
        if prompt.is_none() {
            return Err(ServerError::bad_request(
                "APP_AI_IMAGE_PROMPT_REQUIRED",
        ...
        Ok(AiImageGenerationRequest {
            ...
            quality: input.quality.clone().and_then(normalize_optional_text),
            size: build_size_string(input.width, input.height),
            style: input.style.clone().and_then(normalize_optional_text),
            user: None,
    ...
    fn execute_image_multipart_call(
        ...
        let client = Client::builder()
            ...
            .map_err(|error| {
                ServerError::internal(
                    "APP_AI_IMAGE_HTTP_CLIENT_CREATE_FAILED",
                    format!("failed to create AI image http client: {error}"),
                )
            })?;
        ...
        run_async(async move {
            ...
            let body = response.bytes().await.map_err(|error| {
                ServerError::internal(
                    "APP_AI_IMAGE_RESPONSE_READ_FAILED",
                    format!("failed to read AI {operation_label} response: {error}"),
                )
            })?;
            ...
            serde_json::from_slice::<ImageGenerationResponse>(&body).map_err(|error| {
                ServerError::internal(
                    "APP_AI_IMAGE_RESPONSE_PARSE_FAILED",
                    format!("failed to parse AI {operation_label} response: {error}"),
                )
            })
        })
    ...
    fn build_multipart_binary_part(&self, source: &ResolvedBinarySource) -> ServerResult<Part> {
        ...
        Part::bytes(source.bytes.clone())
            ...
            .map_err(|error| {
                ServerError::bad_request(
                    "APP_AI_IMAGE_SOURCE_MIME_INVALID",
                    format!("image source mime type {mime_type} is invalid: {error}"),
                )
            })
    }
    ...
    fn execute_image_variation_call(
        ...
        let source = input
            ...
            .ok_or_else(|| {
                ServerError::bad_request(
                    "APP_AI_IMAGE_REFERENCE_REQUIRED",
                    "image variation requires at least one reference image",
                )
            })
    ...
    fn execute_image_edit_call(
        ...
        let source = input
            ...
            .ok_or_else(|| {
                ServerError::bad_request(
                    "APP_AI_IMAGE_SOURCE_REQUIRED",
                    "image edit requires a source image",
                )
            })
            .and_then(resolve_binary_source)?;
        let prompt = input
            ...
            .ok_or_else(|| {
                ServerError::bad_request(
                    "APP_AI_IMAGE_EDIT_PROMPT_REQUIRED",
                    "image edit requires a prompt",
                )
            })?;
        let mut form = Form::new()
            .part("image", self.build_multipart_binary_part(&source)?)
    ...
    fn build_image_artifact(
        ...
        let (url, mime_type, file_name, local_path) = if let Some(url) =
            image.url.clone().and_then(normalize_optional_text)
        {
        ...
        } else if let Some(b64_json) = image.b64_json.clone().and_then(normalize_optional_text) {
            let bytes = BASE64_STANDARD
                .decode(b64_json.as_bytes())
                .map_err(|error| {
                    ServerError::internal(
                        "APP_AI_IMAGE_RESPONSE_BASE64_INVALID",
                        format!("failed to decode AI image base64 payload: {error}"),
                    )
                })?;
            ...
            return Err(ServerError::internal(
                "APP_AI_IMAGE_RESPONSE_EMPTY",
                "AI image generation response did not contain a usable image artifact",
            ));
        };
        ...
        if let Some(aspect_ratio) = input.aspect_ratio.clone().and_then(normalize_optional_text) {
    ...
    fn execute_host_image_upscale(
        ...
        if !self.media_runtime_ready() {
            return Err(ServerError::internal(
                "APP_HOST_IMAGE_UPSCALE_RUNTIME_UNAVAILABLE",
                "image upscale requires the host-local media toolkit, but ffmpeg/ffprobe are not available in the current runtime",
            ));
        }

        if input.batch_size == Some(0) {
            return Err(ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_BATCH_INVALID",
                "image upscale batch size must be greater than zero",
            ));
        }

        if input.batch_size.unwrap_or(1) > 1 {
            return Err(ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_BATCH_UNSUPPORTED",
                "image upscale currently supports exactly one output artifact per task",
            ));
        }

        let source_input = input.input_refs.first().ok_or_else(|| {
            ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_SOURCE_REQUIRED",
                "image upscale requires a source image",
            )
        })?;
        ...
        {
            return Err(ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_SOURCE_INVALID",
                "image upscale requires an image input source",
            ));
        }
        ...
        let (source_width, source_height) =
            extract_probed_image_dimensions(&probe).ok_or_else(|| {
                ServerError::bad_request(
                    "APP_HOST_IMAGE_UPSCALE_DIMENSIONS_UNAVAILABLE",
                    "image upscale source must resolve to an image with probeable dimensions",
                )
            })?;
        ...
        if let Some(model_hint) = input.model.clone().and_then(normalize_optional_text) {
            provider_payload.insert("requestedModelHint".to_string(), Value::String(model_hint));
        }
        if let Some(source_url) = source.source_url.and_then(normalize_optional_text) {
            provider_payload.insert("sourceUrl".to_string(), Value::String(source_url));
        }
        if let Some(source_path) = source.source_path.and_then(normalize_optional_text) {
    ...
    fn build_video_request(
        ...
        Ok(AiVideoGenerationRequest {
            ...
            seconds: input.duration.clone().and_then(normalize_optional_text),
            size: input.resolution.clone().and_then(normalize_optional_text),
            ...
            aspect_ratio: input.aspect_ratio.clone().and_then(normalize_optional_text),
    ...
    fn build_video_artifact(
        ...
        let output_url = video
            ...
            .ok_or_else(|| {
                ServerError::internal(
                    "APP_AI_VIDEO_RESPONSE_EMPTY",
                    "AI video generation response did not contain an output url",
                )
            })?;
        let artifact_id = format!("{}-artifact", input.task_id);
        ...
        if let Some(size) = video.size.clone().and_then(normalize_optional_text) {
            metadata.insert("size".to_string(), Value::String(size));
        }
        if let Some(quality) = video.quality.clone().and_then(normalize_optional_text) {
    ...
    fn build_music_request(
        ...
        if let Some(title) = input.title.clone().and_then(normalize_optional_text) {
    ...
    fn build_music_artifact(
        ...
        let output_url = music
            ...
            .ok_or_else(|| {
                ServerError::internal(
                    "APP_AI_MUSIC_RESPONSE_EMPTY",
                    "AI music generation response did not contain an audio url",
                )
            })?;
        let artifact_id = format!("{}-artifact", input.task_id);
        let title = input
            ...
            .clone()
            .and_then(normalize_optional_text)
            .or_else(|| music.title.clone().and_then(normalize_optional_text))
        ...
        if let Some(style) = input.style.clone().and_then(normalize_optional_text) {
            metadata.insert("style".to_string(), Value::String(style));
        }
        if let Some(lyrics) = input.lyrics.clone().and_then(normalize_optional_text) {
...
impl GenerationExecutionService for CanonicalGenerationExecutionService {
    ...
    fn generate_image(
        ...
        let image = response
            ...
            .ok_or_else(|| {
                ServerError::internal(
                    "APP_AI_IMAGE_RESPONSE_EMPTY",
                    "AI image generation response did not contain image data",
                )
            })?;
        ...
        if let Some(revised_prompt) = image.revised_prompt.and_then(normalize_optional_text) {
    ...
    fn generate_video(
        ...
        let remote_job_id = video.id.clone().and_then(normalize_optional_text);
        ...
        provider_payload.insert("mode".to_string(), Value::String(input.mode.clone()));
        provider_payload.insert("responseFormat".to_string(), Value::String(response_format));
        if let Some(status) = video.status.clone().and_then(normalize_optional_text) {
    ...
    fn generate_music(
        ...
        let remote_job_id = music.id.clone().and_then(normalize_optional_text);
        ...
        if let Some(status) = music.status.clone().and_then(normalize_optional_text) {
            provider_payload.insert("status".to_string(), Value::String(status));
        }
        if let Some(remote_job_id) = remote_job_id.clone() {
            provider_payload.insert("remoteJobId".to_string(), Value::String(remote_job_id));
    ...
    match Handle::try_current() {
        ...
        Err(_) => {
            let runtime = Builder::new_current_thread()
                ...
                .map_err(|error| {
                    ServerError::internal(
                        "APP_AI_RUNTIME_CREATE_FAILED",
                        format!("failed to create tokio runtime for AI execution: {error}"),
                    )
                })?;
...
fn map_audio_sdk_error(error: SdkworkError) -> ServerError {
    match error {
        SdkworkError::HttpStatus { status, body } => ServerError::internal(
            "APP_AI_AUDIO_UPSTREAM_STATUS",
            format!("AI audio upstream returned status {status}"),
        )
        .with_detail(body),
        other => ServerError::internal(
            "APP_AI_AUDIO_EXECUTION_FAILED",
            format!("AI audio execution failed: {other}"),
        ),
    }
}
...
fn map_speech_sdk_error(error: SdkworkError) -> ServerError {
    match error {
        SdkworkError::HttpStatus { status, body } => ServerError::internal(
            "APP_AI_SPEECH_UPSTREAM_STATUS",
            format!("AI speech upstream returned status {status}"),
        )
        .with_detail(body),
        other => ServerError::internal(
            "APP_AI_SPEECH_EXECUTION_FAILED",
            format!("AI speech execution failed: {other}"),
        ),
    }
}
...
fn map_source_download_sdk_error(error: SdkworkError) -> ServerError {
    match error {
        SdkworkError::HttpStatus { status, body } => ServerError::internal(
            "APP_AI_AUDIO_SOURCE_DOWNLOAD_STATUS",
            format!("audio source download returned status {status}"),
        )
        .with_detail(body),
        other => ServerError::internal(
            "APP_AI_AUDIO_SOURCE_DOWNLOAD_FAILED",
            format!("failed to download audio source bytes: {other}"),
        ),
    }
}
...
fn map_generation_sdk_error(error: SdkworkError) -> ServerError {
    match error {
        SdkworkError::HttpStatus { status, body } => ServerError::internal(
            "APP_AI_GENERATION_UPSTREAM_STATUS",
            format!("AI generation upstream returned status {status}"),
        )
        .with_detail(body),
        other => ServerError::internal(
            "APP_AI_GENERATION_EXECUTION_FAILED",
            format!("AI generation execution failed: {other}"),
        ),
    }
}
...
fn resolve_binary_source(input: &MediaInputRefRecord) -> ServerResult<ResolvedBinarySource> {
    ...
    if let Some(url) = candidate_url.clone() {
        ...
        if normalized.to_ascii_lowercase().starts_with("file://") {
            let path = file_url_to_path(&normalized).ok_or_else(|| {
                ServerError::bad_request(
                    "APP_AI_SOURCE_FILE_URL_INVALID",
                    "source file url could not be resolved to a local path",
                )
            })?;
            let bytes = fs::read(&path).map_err(|error| {
                ServerError::not_found(
                    "APP_AI_SOURCE_FILE_READ_FAILED",
                    format!("failed to read source file {path}: {error}"),
                )
            })?;
            return Ok(ResolvedBinarySource {
                bytes,
    ...
    if let Some(path) = candidate_path.clone() {
        ...
        let bytes = fs::read(&local_path).map_err(|error| {
            ServerError::not_found(
                "APP_AI_SOURCE_PATH_READ_FAILED",
                format!("failed to read source path {local_path}: {error}"),
            )
        })?;
        return Ok(ResolvedBinarySource {
            bytes,
    ...
    Err(ServerError::bad_request(
        "APP_AI_SOURCE_UNRESOLVABLE",
        "source input must provide a resolvable url or path for canonical host execution",
    ))
}
...
fn decode_data_url(value: &str) -> ServerResult<Vec<u8>> {
    ...
    if !trimmed.to_ascii_lowercase().starts_with("data:") {
        return Err(ServerError::bad_request(
            "APP_AI_AUDIO_SOURCE_DATA_URL_INVALID",
            "audio source data url must start with data:",
        ));
    }
    ...
    let Some(comma_index) = trimmed.find(',') else {
        return Err(ServerError::bad_request(
            "APP_AI_AUDIO_SOURCE_DATA_URL_INVALID",
            "audio source data url must contain a comma separator",
        ));
    };
    ...
    if header.to_ascii_lowercase().contains(";base64") {
        return BASE64_STANDARD.decode(payload).map_err(|error| {
            ServerError::bad_request(
                "APP_AI_AUDIO_SOURCE_DATA_URL_INVALID",
                format!("failed to decode base64 audio data url payload: {error}"),
            )
        });
    }
...
fn resolve_media_input_reference_value(
    ...
    let Some(candidate) = candidate.and_then(normalize_optional_text) else {
        return Ok(None);
    };
    ...
    let resolved_path = if lowercase.starts_with("file://") {
        file_url_to_path(&normalized).ok_or_else(|| {
            ServerError::bad_request(
                "APP_AI_REFERENCE_SOURCE_FILE_URL_INVALID",
                "reference source file url could not be resolved to a local path",
            )
        })?
        ...
        return Err(ServerError::bad_request(
            "APP_AI_REFERENCE_SOURCE_UNSUPPORTED",
            format!("reference source {normalized} uses an unsupported protocol"),
        ));
    } else {
        return Err(ServerError::not_found(
            "APP_AI_REFERENCE_SOURCE_NOT_FOUND",
            format!("reference source {normalized} could not be resolved"),
        ));
    };

    let bytes = fs::read(&resolved_path).map_err(|error| {
        ServerError::not_found(
            "APP_AI_REFERENCE_SOURCE_READ_FAILED",
            format!("failed to read reference source {resolved_path}: {error}"),
        )
    })?;
    let mime_type = input
        .mime_type
...
fn prepare_binary_output_path(
    ...
    fs::create_dir_all(&namespace_dir).map_err(|error| {
        ServerError::internal(
            "APP_AI_OUTPUT_DIR_CREATE_FAILED",
            format!(
...
fn ensure_media_command_succeeded(
    ...
    Err(ServerError::bad_request(code, message).with_detail(command.stderr.clone()))
}
...
fn resolve_image_upscale_dimensions(
    ...
    let requested_width = match input.width {
        Some(0) => {
            return Err(ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_WIDTH_INVALID",
                "image upscale target width must be greater than zero",
            ))
        }
        other => other,
    };
    let requested_height = match input.height {
        Some(0) => {
            return Err(ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_HEIGHT_INVALID",
                "image upscale target height must be greater than zero",
            ))
        }
        other => other,
    };
    let requested_scale = match input.scale {
        Some(0) => {
            return Err(ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_SCALE_INVALID",
                "image upscale scale must be greater than zero",
            ))
        }
        other => other,
    };
...
fn project_preserved_dimension(
    ...
    if source_primary == 0 || source_secondary == 0 {
        return Err(ServerError::bad_request(
            "APP_HOST_IMAGE_UPSCALE_SOURCE_RATIO_INVALID",
            "image upscale source dimensions must be greater than zero",
        ));
    }
...
fn multiply_dimension(value: u32, scale: u32) -> ServerResult<u32> {
    ...
    if multiplied == 0 || multiplied > u32::MAX as u64 {
        return Err(ServerError::bad_request(
            "APP_HOST_IMAGE_UPSCALE_DIMENSION_OVERFLOW",
            "image upscale target dimensions exceed the supported range",
        ));
    }
...
fn resolve_image_output_extension(
    ...
    if let Some(format) = requested_format.and_then(normalize_optional_text) {
        return normalize_image_output_extension(format).ok_or_else(|| {
            ServerError::bad_request(
                "APP_HOST_IMAGE_UPSCALE_FORMAT_UNSUPPORTED",
                "image upscale output format must be one of png, jpg, jpeg, or webp",
            )
        });
    }
