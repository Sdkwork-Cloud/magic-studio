...
impl FileBackedGenerationService {
    ...
    fn acquire_lock(&self) -> ServerResult<MutexGuard<'_, ()>> {
        self.lock.lock().map_err(|_| {
            ServerError::internal(
                "APP_GENERATION_LOCK_POISONED",
                "generation registry lock was poisoned",
            )
        })
    }
    ...
    fn persist_to_disk(&self, document: &GenerationRegistryDocument) -> ServerResult<()> {
        ...
        let contents = serde_json::to_string_pretty(document).map_err(|error| {
            ServerError::internal(
                "APP_GENERATION_SERIALIZE_FAILED",
                format!("failed to serialize generation registry: {error}"),
            )
        })?;
        fs::write(registry_file, contents).map_err(|error| {
            ServerError::internal(
                "APP_GENERATION_WRITE_FAILED",
                format!(
    ...
    fn create_task_record(
        ...
        GenerationTaskRecord {
            ...
            prompt: prompt.and_then(normalize_optional_text),
            negative_prompt: negative_prompt.and_then(normalize_optional_text),
        ...
        document
            ...
            .ok_or_else(|| {
                ServerError::not_found(
                    ...
                    format!("generation task {normalized_task_id} was not found"),
                )
            })
    }
    ...
    fn list_tasks_for_history(
        ...
        let _guard = self.acquire_lock()?;
        let document = self.load_from_disk()?;
        let status_filter = status.and_then(normalize_optional_text);
    ...
    fn optimize_prompt_scene(&self, input: &PromptOptimizeRequest) -> String {
        if let Some(scene) = input.scene.clone().and_then(normalize_optional_text) {
            return scene;
        }
    ...
    fn optimize_prompt_input_text(&self, input: &PromptOptimizeRequest) -> String {
        ...
        if let Some(style) = input.target_style.clone().and_then(normalize_optional_text) {
...
impl GenerationService for FileBackedGenerationService {
    ...
    fn create_image_variation_task(
        ...
        if input_refs.is_empty() {
            return Err(ServerError::bad_request(
                "APP_GENERATION_IMAGE_REFERENCE_REQUIRED",
        ...
        let prompt = input
            ...
            .ok_or_else(|| {
                ServerError::bad_request(
                    "APP_GENERATION_AUDIO_PROMPT_REQUIRED",
                    "audio text-to-speech requires a prompt",
                )
            })
            .and_then(|value| {
    ...
    fn create_audio_transcription_task(
        ...
        let Some(source_audio) = input.source_audio.clone() else {
            return Err(ServerError::bad_request(
                "APP_GENERATION_AUDIO_SOURCE_REQUIRED",
                "audio transcription requires a sourceAudio reference",
            ));
        };
        let parameters = json_map(vec![
            optional_json_string("mode", input.mode.clone()),
    ...
    fn create_audio_translation_task(
        ...
        let Some(source_audio) = input.source_audio.clone() else {
            return Err(ServerError::bad_request(
                "APP_GENERATION_AUDIO_SOURCE_REQUIRED",
                "audio translation requires a sourceAudio reference",
            ));
        };
        if input
            .target_language
        ...
        {
            return Err(ServerError::bad_request(
                "APP_GENERATION_AUDIO_TARGET_LANGUAGE_REQUIRED",
                "audio translation requires a targetLanguage",
            ));
        }
        let parameters = json_map(vec![
            optional_json_string("mode", input.mode.clone()),
    ...
    fn create_music_extend_task(
        ...
        let normalized_style = style.and_then(normalize_optional_text);
    ...
    fn enhance_prompt(
        ...
        let style = input.style.and_then(normalize_optional_text);
        let language = input.language.and_then(normalize_optional_text);
...
fn music_source_title(input: &MediaInputRefRecord) -> Option<String> {
    media_input_metadata_string(input, "title")
        .or_else(|| input.name.clone().and_then(normalize_optional_text))
}
...
fn compose_character_generation_prompt(input: &CharacterGenerationRequest, prompt: &str) -> String {
    ...
    if let Some(archetype) = input.archetype.clone().and_then(normalize_optional_text) {
        descriptors.push(format!("archetype {archetype}"));
    }
    if let Some(gender) = input.gender.clone().and_then(normalize_optional_text) {
    ...
    if let Some(outfit) = input.outfit.clone().and_then(normalize_optional_text) {
    ...
    if let Some(hairstyle) = input.hairstyle.clone().and_then(normalize_optional_text) {
        appearance.push(format!("hairstyle {hairstyle}"));
    }
    if let Some(hair_color) = input.hair_color.clone().and_then(normalize_optional_text) {
        appearance.push(format!("hair color {hair_color}"));
    }
    if let Some(eye_color) = input.eye_color.clone().and_then(normalize_optional_text) {
        appearance.push(format!("eye color {eye_color}"));
    }
    if let Some(skin_tone) = input.skin_tone.clone().and_then(normalize_optional_text) {
        appearance.push(format!("skin tone {skin_tone}"));
    }
    if let Some(accessories) = input.accessories.clone().and_then(normalize_optional_text) {
    ...
    if let Some(avatar_mode) = input.avatar_mode.clone().and_then(normalize_optional_text) {
...
fn parse_generation_product(value: &str) -> ServerResult<GenerationProduct> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err(ServerError::bad_request(
            "APP_GENERATION_TASK_PRODUCT_INVALID",
            format!(
...
fn parse_generation_task_status(value: &str) -> ServerResult<GenerationTaskStatus> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err(ServerError::bad_request(
            "APP_GENERATION_TASK_STATUS_INVALID",
            format!(
...
fn normalize_generation_task_list_query(
    ...
    let product = match (required_product, requested_product) {
        (Some(required), Some(requested)) if required != requested => {
            return Err(ServerError::bad_request(
                "APP_GENERATION_TASK_PRODUCT_CONFLICT",
                format!(
