...
import { loadMagicStudioStorageConfigFromStorage } from '@sdkwork/magic-studio-core/storage';
...
export const ideConfigService = {
    ...
    isInstalled: async (toolId: string): Promise<boolean> => {
        ...
        return await runtime.system.commandExists(tool.executable);
    },
    ...
    _getPlatformConfig: async (toolId: string) => {
        ...
        const osType = await runtime.system.os();
    ...
    _replaceVariables: async (rawPath: string, projectRoot?: string): Promise<string> => {
        ...
        if (resolved.includes('${APPDATA}')) {
            const appData = await runtime.system.path('appData');
