...
describe('promptLibraryService', () => {
  ...
  it('accepts canonical success envelopes when the prompt sdk returns code 0', async () => {
    defaultPromptApi.listPrompts.mockResolvedValueOnce({
      code: 0,
      data: {
        content: [
          {
  ...
  it('throws the business failure message when the prompt sdk returns message instead of msg', async () => {
    defaultPromptApi.deletePromptHistory.mockResolvedValueOnce({
      code: '5000',
      message: 'delete prompt history failed',
    } as any);

    const { promptLibraryService } = await import('../promptLibraryService');

    await expect(promptLibraryService.deletePromptHistory(23)).rejects.toThrow(
      'delete prompt history failed'
    );
  });
});
