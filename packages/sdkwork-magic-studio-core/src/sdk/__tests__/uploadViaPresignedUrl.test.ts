...
describe('uploadViaPresignedUrl', () => {
  ...
  it('rejects legacy register fallback paths and requires client.upload.registerPresigned', async () => {
    const getPresignedUrl = vi.fn(async () => ({
      code: '2000',
      data: {
        url: 'https://upload.example.com/presigned',
    ...
    ).rejects.toThrow('SDK upload API is unavailable: registerPresigned is required.');

    expect(registerPresignedUpload).not.toHaveBeenCalled();
    expect(httpPost).not.toHaveBeenCalled();
  });

  it('accepts canonical success envelopes when the backend returns code 0', async () => {
    const getPresignedUrl = vi.fn(async () => ({
      code: 0,
      data: {
        url: 'https://upload.example.com/presigned',
  ...
  it('throws the business failure message when registerPresigned returns message instead of msg', async () => {
    const getPresignedUrl = vi.fn(async () => ({
      code: '2000',
      data: {
        url: 'https://upload.example.com/presigned',
    ...
    await expect(
      uploadViaPresignedUrl({
        ...
        file: new Uint8Array([1, 2, 3]),
        fileName: 'message-error.png',
        type: 'IMAGE',
        path: 'assets/test',
      })
    ).rejects.toThrow('register upload failed');
  });
});
