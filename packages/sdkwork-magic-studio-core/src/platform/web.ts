...
const requireIndexedDb = (): IDBFactory => {
    if (typeof indexedDB === 'undefined') {
        throw new Error('[Platform] indexedDB is unavailable in the current runtime');
    }
    return indexedDB;
};
...
class WebFileSystem {
    ...
    private initDB(): Promise<IDBDatabase> {
        return new Promise((resolve, reject) => {
            const request = requireIndexedDb().open(this.dbName, 2);
...
let webFileSystem: WebFileSystem | null = null;

const getWebFileSystem = (): WebFileSystem => {
    if (!webFileSystem) {
        webFileSystem = new WebFileSystem();
    }
...
export const webPlatform: PlatformAPI = {
        ...
        input.onchange = async (e) => {
            ...
            if (files && files.length > 0) {
                ...
                for(let i=0; i<files.length; i++) {
                    ...
                    await getWebFileSystem().writeFile(vPath, f);
                    paths.push(vPath);
                }
                resolve(paths);
            } else {
  ...
  readDir: (path: string) => getWebFileSystem().readDir(path),
  readFile: (path: string) => getWebFileSystem().readFile(path),
  writeFile: (path: string, content: string) => getWebFileSystem().writeFile(path, content),
  
  readFileBinary: (path: string) => getWebFileSystem().readFileBinary(path),
  writeFileBinary: (path: string, content: Uint8Array) => getWebFileSystem().writeFile(path, content),

  readFileBlob: (path: string) => getWebFileSystem().readFileBlob(path),
  writeFileBlob: (path: string, content: Blob) => getWebFileSystem().writeFile(path, content),
  
  stat: (path: string) => getWebFileSystem().stat(path),
  createDir: (path: string) => getWebFileSystem().createDir(path),
  delete: (path: string) => getWebFileSystem().delete(path),
  rename: (oldPath: string, newPath: string) => getWebFileSystem().rename(oldPath, newPath),
  
  copyFile: async (src: string, dst: string) => {
      const blob = await getWebFileSystem().readFileBlob(src);
      await getWebFileSystem().writeFile(dst, blob);
  },
