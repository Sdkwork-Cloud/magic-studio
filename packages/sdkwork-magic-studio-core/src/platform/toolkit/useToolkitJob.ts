...
export const useToolkitJob = (
  ...
  const [lastOperation, setLastOperation] =
    useState<ToolkitOperation | null>(null);
  ...
  }, [options.onError, options.onTerminal, options.onUpdate]);
  ...
  const startWatch = useCallback(
    async (
      ...
      if (!supported) {
        throw new Error('[useToolkitJob] watch requires rust server runtime support');
      }
  ...
  const submit = useCallback(
    async (
      ...
      if (!supported) {
        throw new Error('[useToolkitJob] submit requires rust server runtime support');
      }
  ...
  const run = useCallback(
    async (
      ...
      if (!supported) {
        throw new Error('[useToolkitJob] run requires rust server runtime support');
      }
