...
import {
  MAGIC_STUDIO_SYSTEM_LIBRARY_DIR_ORDER,
...
describe('magicStudioPaths', () => {
  ...
  it('builds canonical system and user directories for the MagicStudio filesystem contract', () => {
    ...
    expect(listMagicStudioSystemLibraryDirs(rootLayout)).toEqual([
      '/Users/demo/.sdkwork/magicstudio/system/library/video',
      '/Users/demo/.sdkwork/magicstudio/system/library/image',
      '/Users/demo/.sdkwork/magicstudio/system/library/audio',
      '/Users/demo/.sdkwork/magicstudio/system/library/text',
      '/Users/demo/.sdkwork/magicstudio/system/library/other',
    ]);
  });
  ...
  it('maps content types into one canonical system-library taxonomy', () => {
    const rootLayout = buildMagicStudioRootLayout({
      rootDir: '/Users/demo/.sdkwork/magicstudio',
    });

    expect(MAGIC_STUDIO_SYSTEM_LIBRARY_DIR_ORDER).toEqual([
      'video',
      'image',
      'audio',
      'text',
    ...
    expect(resolveMagicStudioSystemLibraryDir('image')).toBe('image');
    expect(resolveMagicStudioSystemLibraryDir('voice')).toBe('audio');
    expect(resolveMagicStudioSystemLibraryDir('SUBTITLE')).toBe('text');
    expect(resolveMagicStudioSystemLibraryDir('MODEL_3D')).toBe('other');
    expect(resolveMagicStudioSystemLibraryAbsoluteDir(rootLayout, 'subtitle')).toBe(
      '/Users/demo/.sdkwork/magicstudio/system/library/text'
    );
    expect(resolveMagicStudioSystemLibraryAbsoluteDir(rootLayout, 'character')).toBe(
      '/Users/demo/.sdkwork/magicstudio/system/library/other'
    );
  });
});
