...
export const initializeMoveSession = (
    ...
    elements.forEach((element) => {
        ...
        if (nodesToMove.has(elementKey)) {
            initialPositions.set(elementKey, {
                x: element.x,
                y: element.y,
                width: element.width,
    ...
    elements.forEach((element) => {
        ...
        affectedConnections.push({
            id: resolveCanvasEntityKey(element),
            source,
            target,
...
export const computeMoveDeltaWithSnap = (
    ...
    const snapCandidates = rawCandidates.filter((element) => {
        return !movingIds.has(resolveCanvasEntityKey(element)) && element.type !== 'connector';
    });
