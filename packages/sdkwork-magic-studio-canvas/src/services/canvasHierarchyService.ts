...
export const collectSelectionWithHierarchyAndConnectors = (
    ...
    elements.forEach((element) => {
        elementMap.set(resolveCanvasEntityKey(element), element);
    });
    ...
    elements.forEach((element) => {
        ...
        if (includeIds.has(from) && includeIds.has(to)) {
            includeIds.add(resolveCanvasEntityKey(element));
        }
    });

    return elements.filter((element) => includeIds.has(resolveCanvasEntityKey(element)));
};
...
export const detectGroupCycles = (elements: CanvasElement[]): string[] => {
    ...
    elements.forEach((element) => {
        if (element.type === 'group') {
            groupById.set(resolveCanvasEntityKey(element), element);
        }
    });
