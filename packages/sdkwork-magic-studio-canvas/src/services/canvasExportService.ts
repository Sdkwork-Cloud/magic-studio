...
import { thumbnailGenerator } from '@sdkwork/magic-studio-core/services';
...
import { CutProject, NormalizedState } from '@sdkwork/magic-studio-magiccut';
import { hasPersistentEntityId, type EntityIdentityLike } from '@sdkwork/magic-studio-types/entity';
import { MediaResourceType } from '@sdkwork/magic-studio-types/vocabulary';
import { vfs } from '@sdkwork/magic-studio-fs';
import { assetService } from '@sdkwork/magic-studio-assets/services';
import {
...
const ignoreCanvasExportError = (_error: unknown): void => undefined;
...
export class CanvasExportService {
    ...
    public async importFromJson(jsonContent: string): Promise<CanvasBoard> {
        try {
            ...
            const boardIdentity = createLocalCanvasEntityIdentity();
    ...
    public async exportToMagicCut(board: CanvasBoard, projectName?: string, exportMode: CanvasExportMode = 'video_only'): Promise<string> {
        ...
        // 3. Persist - Note: magicCutProjectService would need to be imported from @sdkwork/magic-studio-magiccut
        // For now, we'll just return the project ID
        const projectKey = this.resolveStableEntityKey(project, 'canvas-project');
        console.log(`[CanvasExport] Project ready with ID: ${projectKey}`);
    ...
    private async processProjectAssets(project: CutProject): Promise<void> {
        ...
        const rootLayout = await resolveRuntimeMagicStudioRootLayout(getPlatformRuntime());
        const thumbsDir = rootLayout.systemThumbnailCacheDir;
        const assetsDir = rootLayout.systemLibraryImageDir;
        const videoDir = rootLayout.systemLibraryVideoDir;

        try { await vfs.createDir(thumbsDir); } catch (error) { ignoreCanvasExportError(error); }
        try { await vfs.createDir(assetsDir); } catch (error) { ignoreCanvasExportError(error); }
        try { await vfs.createDir(videoDir); } catch (error) { ignoreCanvasExportError(error); }
        ...
        const processResource = async (res: any) => {
            try {
                ...
                if (
                    ...
                    res.path.length > 0 &&
                    (
                        isManagedAssetLocator(res.path) ||
                        isExplicitLocalAssetLocator(res.path) ||
                        isLocalFilePath(res.path)
                    )
                ) {
                    res.url = '';
                }
    ...
    private async externalizeData(res: any, targetDir: string, defaultExt: string) {
        ...
        if (matches && matches.length === 3) {
            ...
            const safeName = this.hashString(this.resolveStableEntityKey(res, 'canvas-resource')) + '_' + generateUUID().slice(0, 8);
    ...
    private async generateVideoThumbnail(res: any, thumbsDir: string, hintUrl?: string) {
        ...
        try {
            ...
            if (!sourceUrl && hasPersistentEntityId(res.id)) {
            ...
            if (sourceUrl) {
                ...
                if (thumbBlob) {
                    const safeId = this.hashString(this.resolveStableEntityKey(res, 'canvas-resource'));
                 ...
                 console.warn(
                    `[CanvasExport] Could not resolve source URL for video thumbnail: ${this.resolveStableEntityKey(res, 'canvas-resource')}`
                );
            }
        } catch (err) {
    ...
    private resolveStableEntityKey(
        entity: EntityIdentityLike | null | undefined,
        fallback: string
    ): string {
        const normalizedUuid = typeof entity?.uuid === 'string' ? entity.uuid.trim() : '';
        if (normalizedUuid) {
            return normalizedUuid;
        }

        const normalizedId = typeof entity?.id === 'string' ? entity.id.trim() : '';
        if (normalizedId) {
            return normalizedId;
        }
