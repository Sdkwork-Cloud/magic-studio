...
import { QuadTree, Rect } from '@sdkwork/magic-studio-commons/algorithms';
...
import type { AffectedConnection, MoveElementStartPosition } from '../services/canvasInteractionService';
import { matchesCanvasEntityKey, resolveCanvasEntityKey } from '../utils/canvasIdentity';
...
export const CanvasBoard: React.FC = () => {
    ...
    const { visibleGroups, visibleNodes } = useMemo(() => {
        ...
        visibleRaw.forEach(el => visibleIds.add(resolveCanvasEntityKey(el)));
        selectedIds.forEach(id => visibleIds.add(id));
        
        const renderOrder = getRenderList();
        
        const visibleElements = renderOrder.filter(el => visibleIds.has(resolveCanvasEntityKey(el))).map(el => ({
            ...el,
            selected: selectedIds.has(resolveCanvasEntityKey(el))
        }));
    ...
    const selectionBounds = useMemo<Rect | null>(() => {
        ...
        selectedIds.forEach(id => {
            const el = elements.find(e => matchesCanvasEntityKey(e, id));
    ...
    }, [viewport, selectedIds, spatialIndex, connectionDraft, marquee]);
    ...
    const resetMovePreviewStyles = useCallback((elementsSnapshot: CanvasElement[]) => {
        ...
        elementsSnapshot.forEach((element) => {
            elementById.set(resolveCanvasEntityKey(element), element);
        });
    ...
    const handleNodeContextMenu = (e: React.MouseEvent, id: string) => {
        ...
        e.stopPropagation();
        const element = elements.find(el => matchesCanvasEntityKey(el, id));
    ...
    const handlePortMouseDown = (e: React.MouseEvent, nodeId: string, type: 'in' | 'out') => {
        ...
        const element = elements.find(el => matchesCanvasEntityKey(el, nodeId));
        if (!element) return;
                ...
                if (dragRef.current.mode === 'rubber-band' && currentMarquee) {
                    ...
                    hitElements.forEach(el => {
                        if (el.x < x + width && el.x + el.width > x && el.y < y + height && el.y + el.height > y && el.type !== 'connector') {
                            hitIds.push(resolveCanvasEntityKey(el));
                        }
                    });
    ...
    const handleMenuSelect = async (type: string) => {
        ...
        if (dropMenu.sourceId && dropMenu.portType) {
            const newElementKey = resolveCanvasEntityKey(newElement);
            const fromId = dropMenu.portType === 'out' ? dropMenu.sourceId : newElementKey;
            const toId = dropMenu.portType === 'in' ? dropMenu.sourceId : newElementKey;
            addConnection(fromId, toId);
        }
        setDropMenu(null);
    };
            ...
            >
                 <div className="absolute inset-0 pointer-events-none" style={{ zIndex: Z_LAYERS.GROUPS }}>
                    {visibleGroups.map(el => (
                        ...
                        <CanvasGroupPanel 
                            key={elementKey}
                            ref={(node) => { if (node) groupRefs.current.set(elementKey, node); else groupRefs.current.delete(elementKey); }}
                            element={el}
                 ...
                 <div className="absolute inset-0 pointer-events-none" style={{ zIndex: Z_LAYERS.NODES }}>
                    {visibleNodes.map((el, index) => {
                        const elementKey = resolveCanvasEntityKey(el);
                        const isInteracting = selectedIds.has(elementKey);
                        const computedZ = isInteracting ? Z_LAYERS.ACTIVE_NODE + index : undefined;

                        return (
                            <CanvasNode 
                                key={elementKey}
                                ref={(node) => { if (node) nodeRefs.current.set(elementKey, node); else nodeRefs.current.delete(elementKey); }}
                                element={el}
                                onMouseDown={handleNodeMouseDown}
