...
export const VoiceLeftGeneratorPanel: React.FC = () => {
    ...
    const speechCapability = useRuntimeMagicStudioExecutionOperationCapability(
        'voice-speech-tasks',
        'create-speech-task',
        {
            feature: 'VoiceLeftGeneratorPanel'
        }
    );
    const generateDisabledReason = !isModeEnabled
        ? modeAvailability[mode]?.reason || null
        : speechCapability.disabledReason;
    const canGenerate =
        isModeEnabled
        && speechCapability.ready
        && canGenerateByVoicePanelSchema(runtimeState);
    ...
    return (
        <>
            ...
            <VoiceGenerateFooter
                ...
                isCapabilityLoading={speechCapability.isLoading}
                onGenerate={generate}
            />
            ...
            <ChooseAssetModal
                ...
                onConfirm={(assets) => {
                    ...
                    const referenceAudio = toVoiceInputResourceRefFromAsset(first, 'audio');
                    if (referenceAudio) {
                        setConfig({
                            referenceAudio
                        });
