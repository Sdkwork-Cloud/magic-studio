...
import { Button, AudioUpload, findByIdOrFirst } from '@sdkwork/magic-studio-commons';
import {
    Dialog,
    DialogContent,
    DialogClose,
...
import { AudioRecorder } from '@sdkwork/magic-studio-audio/recorder';
...
import type { VoiceLabDraft } from './types';
...
interface VoiceLabModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (draft: VoiceLabDraft) => void | Promise<void>;
...
export const VoiceLabModal: React.FC<VoiceLabModalProps> = ({
    ...
    const cloneCapability = useRuntimeMagicStudioExecutionOperationCapability(
        'voice-clone-tasks',
        'create-clone-task',
        {
            enabled: isOpen && mode === 'clone',
            feature: 'VoiceLabModal'
        }
    );
    const cloneUnavailableReason = mode === 'clone' && !cloneCapability.ready
        ? cloneCapability.disabledReason
        : null;
    const baseCanSave = name.trim().length > 0 && (mode === 'design' || !!referenceAsset);
    const canSave = baseCanSave && (mode !== 'clone' || cloneCapability.ready);
    ...
    const handleSave = async () => {
        ...
        if (mode === 'clone' && !cloneCapability.ready) {
            return;
        }
        ...
        await onSave({
            ...
            referenceAudio:
                mode === 'clone'
                    ? toVoiceInputResourceRefFromAsset(referenceAsset, 'audio')
                    : undefined,
            referencePreviewUrl:
                mode === 'clone'
                    ? (referencePreviewUrl || STORAGE_SAFE_PREVIEW)
                    : undefined
        });
        closeModal();
    };
        ...
        <Dialog open onOpenChange={(open) => !open && closeModal()}>
            <DialogContent className="w-full max-w-4xl h-[84vh] rounded-2xl border border-[#2f2f34] bg-[#121214] shadow-2xl overflow-hidden flex flex-col p-0" showCloseButton={false}>
                            ...
                            <Field label={t('voice.lab.cloneReference.label', 'Clone Reference Audio *')}>
                                <div className="space-y-3">
                                    ...
                                    {cloneUnavailableReason && (
                                        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-[10px] text-amber-100">
                                            {cloneUnavailableReason}
                                        </div>
                                    )}
                                </div>
                ...
                <div className="h-16 border-t border-[#2a2a30] bg-[#18181b] px-6 flex items-center justify-between">
                    <span className={`text-xs ${cloneUnavailableReason ? 'text-amber-200' : 'text-gray-500'}`}>
                        {cloneUnavailableReason || (mode === 'clone'
