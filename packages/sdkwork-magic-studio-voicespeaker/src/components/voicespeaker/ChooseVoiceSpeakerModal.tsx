...
import type { ChooseVoiceSpeakerModalProps, IVoice, VoiceLabDraft } from './types';
...
export const ChooseVoiceSpeakerModal: React.FC<ChooseVoiceSpeakerModalProps> = ({
    ...
    const customVoicesRef = useRef<IVoice[]>([]);
    ...
    const loadCustom = useCallback(async () => {
        ...
        try {
            const voices = await speakerService.getCustomVoices();
            if (!mountedRef.current) {
                return;
            }
            customVoicesRef.current = voices;
    ...
    }, [initialView, isOpen, loadCustom, loadMarket, loadWorkspace, selectedId, stopAudio]);
    ...
    const sortedVoices = useMemo(() => {
        ...
        list.sort((left, right) => {
            if (matchesEntityKey(left, currentSelectedId)) {
                return -1;
            }
            if (matchesEntityKey(right, currentSelectedId)) {
                return 1;
            }
    ...
    const selectedVoice = useMemo(
        () => allVoices.find((voice) => matchesEntityKey(voice, currentSelectedId)) || null,
        [allVoices, currentSelectedId]
    );

    const isLoading =
        loadingMarket || (activeCollection === 'mine' && (loadingWorkspace || loadingCustom));
    ...
    const handleVoiceCreated = async (draft: VoiceLabDraft) => {
        try {
            const createdVoice = await speakerService.createCustomVoice({
                name: draft.name,
                language: draft.language,
                gender: draft.gender,
                style: draft.style,
                previewText: draft.previewText,
                previewUrl: draft.mode === 'clone' ? draft.referencePreviewUrl : undefined,
                avatarUrl: draft.avatarUrl,
                description: draft.description,
                tags: [draft.mode === 'clone' ? 'clone' : 'design', speakerService.safeString(draft.style, 'neutral')],
                referenceAudio: draft.referenceAudio,
                provider:
                    draft.mode === 'clone'
                        ? 'magic-studio-voice-clone'
                        : 'magic-studio-voice-lab'
            });

            let selectedVoice = createdVoice;

            if (draft.mode === 'clone' && (draft.referenceAudio || draft.referencePreviewUrl)) {
                const cloneTaskId = await speakerService.submitCloneTask({
                    speakerId: createdVoice.id,
                    sampleAudio: draft.referenceAudio,
                    sampleAudioUrl: draft.referencePreviewUrl,
                    language: draft.language,
                    previewText: draft.previewText,
                    idempotencyKey: `magic-studio-voice-clone:${createdVoice.id}`,
                    autoUpdatePreview: true
                });

                if (cloneTaskId) {
                    const result = await speakerService.waitForCloneTaskResult(cloneTaskId);
                    const resolvedSpeakerId = speakerService.safeString(result?.speakerId, createdVoice.id);
                    const refreshedSpeaker = await speakerService.readVoiceSpeaker(resolvedSpeakerId);
                    if (refreshedSpeaker) {
                        selectedVoice = refreshedSpeaker;
                    }
                }
            }

            const refreshed = await speakerService.getCustomVoices();
            if (!mountedRef.current) {
                return;
            }
            customVoicesRef.current = refreshed;
            setCustomVoices(refreshed);
            setActiveCollection('mine');
            setCurrentSelectedId(selectedVoice.uuid || selectedVoice.id);
        } catch (error) {
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                {sortedVoices.map((voice) => (
                                    <VoiceCard
                                        key={voice.id}
                                        voice={voice}
                                        isSelected={matchesEntityKey(voice, currentSelectedId)}
                                        isPlaying={playingVoiceId === voice.id}
                                        onClick={() => setCurrentSelectedId(voice.uuid || voice.id)}
                                        onPlay={(event) => {
