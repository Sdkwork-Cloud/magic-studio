...
export const WorkspaceStoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const loadWorkspaces = useCallback(async () => {
    const res = await workspaceService.findAll({ page: 0, size: 50 });
  ...
  const addWorkspace = useCallback(async (name: string, description?: string, icon?: string) => {
    const res = await workspaceService.save({ id: name.toLowerCase().replace(/\s+/g, '-'), name, description, icon } as Partial<StudioWorkspace> & { id: string });
    if (res.success) {
      await loadWorkspaces();
    }
  }, [loadWorkspaces]);
  ...
  const editWorkspace = useCallback(async (uuid: string, name: string) => {
    const res = await workspaceService.save({ id: uuid, name } as Partial<StudioWorkspace> & { id: string });
    if (res.success) {
      await loadWorkspaces();
    }
  }, [loadWorkspaces]);
  ...
  const removeWorkspace = useCallback(async (uuid: string) => {
    await workspaceService.deleteAll([uuid]);
    await loadWorkspaces();
  }, [loadWorkspaces]);
  ...
  const addProject = useCallback(async (name: string, type: ProjectType, description: string, coverImage?: { data: Uint8Array, name: string }) => {
    ...
    const res = await workspaceService.saveProject(currentWorkspace.id, { name, type, description, coverImage } as any);
    if (res.success) {
      await loadWorkspaces();
    }
  ...
  const removeProject = useCallback(async (workspaceUuid: string, projectUuid: string) => {
    await workspaceService.deleteProject(workspaceUuid, projectUuid);
    await loadWorkspaces();
  }, [loadWorkspaces]);
