...
export class TimelineEditService {
    ...
    private static getTrackClips(clip: CutClip, state: NormalizedState): CutClip[] {
        const track = findMagicCutEntityByKey(state.tracks, resolveEntityKey(clip.track));
        if (!track) return [];

        return track.clips
            .map(ref => {
                const clipKey = resolveMagicCutRecordKey(state.clips, ref);
                return clipKey ? state.clips[clipKey] : undefined;
            })
            .filter((candidate): candidate is CutClip => Boolean(candidate))
            .sort((a, b) => a.start - b.start || resolveEntityKey(a).localeCompare(resolveEntityKey(b)));
    }

    private static getSourceDuration(clip: CutClip, state: NormalizedState): number | null {
        const resourceKey = resolveMagicCutRecordKey(state.resources, clip.resource);
        const resource = (resourceKey ? state.resources[resourceKey] : undefined) as { duration?: number; metadata?: { duration?: number } } | undefined;
    ...
    static calculateRippleTrim(
        ...
        const clipIndex = trackClips.findIndex(c => resolveEntityKey(c) === clipKey);
    ...
    static calculateRippleDelete(
        ...
        const clipKey = resolveEntityKey(clip);
        const result: EditOperationResult = { clipsToUpdate: [], clipsToDelete: [clipKey] };
        ...
        const trackClips = track.clips
            .map(ref => {
                const clipStateKey = resolveMagicCutRecordKey(state.clips, ref);
                return clipStateKey ? state.clips[clipStateKey] : undefined;
            })
            .filter((candidate): candidate is CutClip => Boolean(candidate))
            .sort((a, b) => a.start - b.start || resolveEntityKey(a).localeCompare(resolveEntityKey(b)));
    ...
    static findClipAtTime(
        ...
        const track = findMagicCutEntityByKey(state.tracks, trackId);
        if (!track) return null;
        
        for (const ref of track.clips) {
            const clipKey = resolveMagicCutRecordKey(state.clips, ref);
            const clip = clipKey ? state.clips[clipKey] : undefined;
            if (!clip) continue;
