...
export const useClipDrag = (options: UseClipDragOptions) => {
    const {
        ...
        setInteraction, applyClipMoves, insertTrackAndMoveClipGroup, autoScrollSpeed: autoScrollSpeedRef,
        prepareSnapPoints
    } = options;
    ...
    const processDragFrame = useCallback((clientX: number, clientY: number) => {
        ...
        const movePlan = activeClip && (result.trackId || result.insertIndex !== null)
            ? resolveMovePlan(
                resolveEntityKey(activeClip),
                result.trackId || resolveEntityKey(activeClip.track),
                result.time,
    ...
    }, [containerRef, pixelsPerSecond, autoScrollSpeedRef, buildContext, clipsMap, resolveMovePlan, setInteraction]);

    const startDrag = (clipId: string, clientX: number, clientY: number) => {
        const clip = clipsMap[clipId];
        if (!clip || !containerRef.current) return;
        const trackKey = resolveEntityKey(clip.track);
        ...
        options.tracks.forEach(track => {
            ...
            track.clips.forEach(ref => {
                const currentClip = clipsMap[resolveEntityKey(ref)];
                if (currentClip) {
                    index.insert({
                        id: resolveEntityKey(currentClip),
                        start: currentClip.start,
                        end: currentClip.start + currentClip.duration
                    });
                }
            });
            indicesRef.current.set(resolveEntityKey(track), index);
        });
            ...
            if (!state.isDragging) {
                ...
                if (dx > DRAG_THRESHOLD_PX || dy > DRAG_THRESHOLD_PX) {
                    ...
                    setInteraction({
                        ...
                        initialTrackId: trackKey,
                        initialOffset: clip.offset || 0,
                        isSnapping: false,
                        linkedMoves: [{ clipId, targetTrackId: trackKey, newStart: clip.start }]
                    });
                }
            }
            ...
            if (dragStateRef.current.isDragging) {
                if (strategyRef.current && containerRef.current) {
                    ...
                    const movePlan = clip && (result.trackId || result.insertIndex !== null)
                        ? resolveMovePlan(
                            cId,
                            result.trackId || resolveEntityKey(clip.track),
                            result.time,
                            result.insertIndex !== null,
                            context
                        )
