...
import { CutClip, CutTrackType } from '@sdkwork/magic-studio-types/magiccut';
import { resolveEntityKey } from '@sdkwork/magic-studio-types/entity';
import { mediaService } from '@sdkwork/magic-studio-core/services';
...
export const WaveformOverlay: React.FC<WaveformOverlayProps> = React.memo(({
    ...
    useEffect(() => {
        ...
        const loadWaveforms = async () => {
             for (const clip of clips) {
                if (!clip.resource) continue;

                const resource = getResource(resolveEntityKey(clip.resource));
                ...
                if (requestedIdsRef.current.has(resourceKey)) continue;
                requestedIdsRef.current.add(resourceKey);
                ...
                try {
                    ...
                    if (isMounted && peaks) {
                        setPeaksMap(prev => new Map(prev).set(resourceKey, peaks));
                    }
                } catch (e) {
    ...
    useEffect(() => {
        ...
        clips.forEach(clip => {
            if (draggedClipId === resolveEntityKey(clip)) return;
            
            const resource = getResource(resolveEntityKey(clip.resource));
            if (!resource) return;

            const peaks = peaksMap.get(resolveEntityKey(resource));
