...
export const useTimeline = (
    ...
    const timelineTracks = useMemo(() => {
        ...
        return activeTimeline.tracks.map(ref => trackMap[resolveEntityKey(ref)]).filter(Boolean);
    ...
    const trackLayouts = useMemo(() => {
        ...
        return timelineTracks.map(track => {
            ...
            const layout = { id: resolveEntityKey(track), top: currentTop, height };
    ...
    const handleDropEmpty = useCallback(async (e: React.DragEvent) => {
            ...
            try {
                ...
                if (tracksContainerRef.current && resources.length > 0) {
                     ...
                     if (result.isValid) {
                        ...
                        const resourceMap = new Map(resources.map((item) => [resolveEntityKey(item), item] as const));
        ...
        if (dragOperation.payload.type === MediaResourceType.EFFECT) {
            ...
            if (targetClipId) {
                addEffectToClip(targetClipId, resolveEntityKey(dragOperation.payload));
            }
        ...
        if (dragOperation.payload.type === MediaResourceType.TRANSITION) {
            ...
            if (targetTransition) {
                addTransitionToClip(
                    targetTransition.fromClipId,
                    resolveEntityKey(dragOperation.payload),
                    dragOperation.duration
                );
            }
