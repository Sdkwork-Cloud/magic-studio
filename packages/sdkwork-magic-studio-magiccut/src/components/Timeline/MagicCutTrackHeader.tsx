...
import { downloadService, thumbnailGenerator, uploadHelper } from '@sdkwork/magic-studio-core/services';
...
export const MagicCutTrackHeader: React.FC<MagicCutTrackHeaderProps> = React.memo(({
    ...
    const { project, resizeTrack, updateTrack, removeTrack, selectedTrackId, selectTrack, state, getResource } = useMagicCutStore();
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            resizeTrack(trackKey, height + e.movementY);
        };
        const handleMouseUp = () => {
            setIsResizing(false);
    ...
    }, [isResizing, height, trackKey, resizeTrack]);
    ...
    const generateTrackKeyframes = async () => {
        ...
        const trackClips = track.clips
            ...
            .filter((clip) => {
                const r = getResource(resolveEntityKey(clip.resource));
        ...
        const extractionPromises = tasks.map(async (task) => {
            const resource = getResource(resolveEntityKey(task.clip.resource));
    ...
    const handleSelectCover = async (url: string) => {
        ...
        try {
            const imported = await importMagicCutTrackCoverFromUrl(
                url,
                `magiccut_track_cover_${trackKey}_${Date.now()}.png`,
                {
            ...
            updateTrack(trackKey, { coverImage: imported.reference });
            setCoverMenuPos(null);
        } catch (error) {
        ...
        try {
            ...
            if (files.length > 0) {
                ...
                const imported = await importMagicCutTrackCoverFile({
                    name: file.name || `magiccut_track_cover_${trackKey}.png`,
    ...
    const handleDragStart = (e: React.DragEvent) => {
        e.dataTransfer.setData('magic-cut/track-id', trackKey);
    ...
    const isSelected = selectedTrackId === trackKey;
    const { url: coverImageUrl } = useAssetUrl(track.coverImage || null, {
            ...
            <div className="flex-1 flex flex-col justify-center min-w-0 h-full py-1">
                ...
                <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                    <ControlBtn
                        ...
                        onClick={() => updateTrack(trackKey, { locked: !track.locked })}
                    ...
                    {isVisual && (
                        <ControlBtn
                            ...
                            onClick={() => updateTrack(trackKey, { visible: !track.visible })}
                    ...
                    {(canHaveVolume) && (
                        <ControlBtn
                            ...
                            onClick={() => updateTrack(trackKey, { muted: !track.muted })}
                ...
                {isMain && (
                    ...
                    >
                        {track.coverImage ? (
                            <>
                                {coverImageUrl ? (
                                    <img src={coverImageUrl} className="w-full h-full object-cover" alt={t('trackHeader.coverAlt')} />
                                ) : (
