...
import { createKeyframePoint } from '@sdkwork/magic-studio-types/magiccut';
...
export const KeyframeCurveEditor: React.FC<KeyframeCurveEditorProps> = ({
    ...
    const drawCurve = useCallback(() => {
        ...
        sortedKeyframes.forEach(kf => {
            const x = timeToX(kf.time);
            const y = valueToY(kf.value);
            const isSelected = matchesEntityKey(kf, selectedKeyframe);
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        for (const kf of sortedKeyframes) {
            ...
            if (distance < 12) {
                setSelectedKeyframe(resolveEntityKey(kf));
                setIsDragging(true);
                return;
            }
        }
    ...
    const addKeyframe = (time: number, value: number) => {
        const newKf: KeyframePoint = createKeyframePoint({
            time,
            value,
            easing: 'linear'
        });
    ...
    const updateEasing = (easing: EasingType) => {
        ...
        const existingKfs = clip?.keyframes?.[propertyName] || [];
        const newKeyframes = existingKfs.map(kf => 
            matchesEntityKey(kf, selectedKeyframe) ? { ...kf, easing } : kf
        );
    ...
    const deleteSelectedKeyframe = () => {
        ...
        const existingKfs = clip?.keyframes?.[propertyName] || [];
        const newKeyframes = existingKfs.filter(kf => !matchesEntityKey(kf, selectedKeyframe));
    ...
    const selectedKf = sortedKeyframes.find(kf => matchesEntityKey(kf, selectedKeyframe));
