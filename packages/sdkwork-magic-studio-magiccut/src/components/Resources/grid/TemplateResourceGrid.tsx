...
import type { CutTemplate } from '../../../entities/magicCut.entity';
...
const TemplateGridCard: React.FC<{
    template: CutTemplate;
    onLoadTemplate: (template: CutTemplate) => void;
}> = ({ template, onLoadTemplate }) => {
    const thumbnailSource = resolveMagicCutThumbnailReference(template);
    const { url: thumbnail } = useAssetUrl(thumbnailSource);

    return (
        <div className="group relative bg-[#252526] border border-[#333] hover:border-[#555] rounded-xl overflow-hidden cursor-pointer transition-all hover:shadow-lg flex flex-col">
            <div className="aspect-video bg-[#1e1e1e] relative flex items-center justify-center overflow-hidden">
                {thumbnail ? (
                    <img src={thumbnail} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" alt={template.name} />
                ) : (
            ...
            <div className="p-2.5">
                <h4 className="text-[10px] font-bold text-gray-200 truncate" title={template.name}>{template.name}</h4>
                ...
                <div className="mt-1.5 pt-1.5 border-t border-[#333] flex justify-between items-center text-[8px] text-gray-600">
                     ...
                     <span className="bg-[#333] px-1 py-0.5 rounded text-gray-400">
                         v{template.projectData.version}
                     </span>
                </div>
            </div>
        </div>
