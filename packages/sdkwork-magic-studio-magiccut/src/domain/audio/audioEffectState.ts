...
import { createAudioEffectConfig } from '@sdkwork/magic-studio-types/magiccut';
...
const NORMALIZE_PRESET: ManagedAudioEffect[] = [
  {
    id: null,
    uuid: `${NORMALIZE_PRESET_ID}-compressor`,
    type: 'compressor',
    enabled: true,
    params: {
      ...DEFAULT_COMPRESSOR,
  ...
  {
    id: null,
    uuid: `${NORMALIZE_PRESET_ID}-limiter`,
    type: 'limiter',
    enabled: true,
    params: {
      ...DEFAULT_LIMITER,
...
export function updateEqBandGain(
  ...
  const nextEq: AudioEffectConfig = createAudioEffectConfig({
    id: Object.prototype.hasOwnProperty.call(existingEq || {}, 'id')
      ? (existingEq?.id ?? null)
      : null,
    uuid: existingEq?.uuid ?? undefined,
    type: 'eq',
    enabled: true,
    params: {
      ...DEFAULT_EQ,
