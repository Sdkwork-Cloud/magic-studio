...
import { resolveMagicCutRecordKey } from '@sdkwork/magic-studio-types/magiccut';
...
const collectLinkedClipIds = (
  ...
  const resolveClip = (clipKey: string) => {
    const recordKey = resolveMagicCutRecordKey(state.clips, clipKey);
    return recordKey ? state.clips[recordKey] : null;
  };

  if (!linkedSelectionEnabled) {
    const clip = resolveClip(primaryClipId);
    return clip ? [resolveEntityKey(clip)] : [];
  }
  ...
  while (queue.length > 0) {
    ...
    const clip = resolveClip(clipId);
    if (!clip) continue;

    const resolvedClipId = resolveEntityKey(clip);
    if (collectedIds.has(resolvedClipId)) {
      continue;
    }
    collectedIds.add(resolvedClipId);
    ...
    if (clip.linkGroupId) {
      Object.values(state.clips).forEach((candidate) => {
        if (
          ...
          !collectedIds.has(resolveEntityKey(candidate))
        ) {
          queue.push(resolveEntityKey(candidate));
        }
      });
    }
  }
...
export const resolveLinkedClipMovePlan = ({
  ...
  const primaryClip = resolveClip(primaryClipId);
  if (!primaryClip) {
    return {
      moves: [],
  ...
  const primaryClipKey = resolveEntityKey(primaryClip);
  const targetTrackRecordKey = resolveMagicCutRecordKey(state.tracks, targetTrackId);
  const canonicalTargetTrackId = targetTrackRecordKey
    ? resolveEntityKey(state.tracks[targetTrackRecordKey])
    : targetTrackId;
  ...
  const moves = movementClips
    .map((clip) => ({
      ...
      targetTrackId: resolveEntityKey(clip) === primaryClipKey ? canonicalTargetTrackId : resolveEntityKey(clip.track),
      newStart: Math.max(0, clip.start + boundedDelta),
    }))
    .sort((left, right) => {
      if (left.clipId === primaryClipKey) return -1;
      if (right.clipId === primaryClipKey) return 1;
      return left.clipId.localeCompare(right.clipId);
    });
  ...
  const hasExternalCollision =
    ...
    moves.some((move) => {
      ...
      const clip = resolveClip(move.clipId);
