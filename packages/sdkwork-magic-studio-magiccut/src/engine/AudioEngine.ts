...
const resolveTrackFromRef = (
    tracks: Record<string, CutTrack>,
    trackRef: EntityIdentityLike | null | undefined
): CutTrack | null => findMagicCutTrackByRef({ tracks }, trackRef);

const resolveClipFromRef = (
    clips: Record<string, CutClip>,
    clipRef: EntityIdentityLike | null | undefined
): CutClip | null => findMagicCutClipByRef({ clips }, clipRef);

const resolveClipFromKey = (
    clips: Record<string, CutClip>,
    clipKey: string | null | undefined
): CutClip | null => findMagicCutClipByKey({ clips }, clipKey);

const resolveResourceFromRef = (
    resources: Record<string, AnyMediaResource>,
    resourceRef: EntityIdentityLike | null | undefined
): AnyMediaResource | null => findMagicCutResourceByRef({ resources }, resourceRef);
...
export class AudioEngine {
    ...
    public async loadResource(resource: AnyMediaResource): Promise<AudioBuffer | null> {
        ...
        if (this.bufferCache.has(resourceKey)) return this.bufferCache.get(resourceKey)!;
        if (this.pendingLoads.has(resourceKey)) return this.pendingLoads.get(resourceKey)!;
        ...
        const task = (async () => {
            try {
                ...
                this.bufferCache.set(resourceKey, audioBuffer);
                return audioBuffer;
            } catch (e) {
                console.warn(`[AudioEngine] Failed to load ${resourceKey}`, e);
                return null;
            }
        })();

        this.pendingLoads.set(resourceKey, task);
        return task.finally(() => this.pendingLoads.delete(resourceKey));
    }
    ...
    public schedule(
        ...
        const audibleTrackIds = resolveAudibleTrackIds(
            timeline.tracks
                .map((trackRef) => resolveTrackFromRef(tracks, trackRef))
        ...
        timeline.tracks.forEach(trackRef => {
            const track = resolveTrackFromRef(tracks, trackRef);
            const trackKey = track ? resolveEntityKey(track) : null;

            if (!track || !trackKey || !audibleTrackIds.has(trackKey)) return;

            track.clips.forEach((clipRef) => {
                const clip = resolveClipFromRef(clips, clipRef);
                if (!clip) return;
        ...
        for (const [id, scheduled] of this.scheduledSources) {
            if (!activeClipIds.has(id)) {
                ...
                const clip = resolveClipFromKey(clips, id);
                if (!clip) continue;
                const track = resolveTrackFromRef(tracks, clip.track);
                if (track) {
    ...
    private scheduleClipReverse(
        ...
        const clipKey = resolveEntityKey(clip);
        if (this.scheduledSources.has(clipKey)) return;

        const resource = resolveResourceFromRef(resources, clip.resource);
        if (!resource) return;
        const resourceKey = resolveEntityKey(resource);

        const buffer = this.bufferCache.get(resourceKey);
        if (!buffer) return;
        ...
        const reversed = this.getReversedBuffer(resourceKey, buffer);
        ...
        try {
            ...
            this.scheduledSources.set(clipKey, scheduled);

            source.onended = () => {
                try {
                    if (this.scheduledSources.has(clipKey)) {
                        const current = this.scheduledSources.get(clipKey);
                        if (current && current.source === source) {
                            this.scheduledSources.delete(clipKey);
                            source.disconnect();
                            gain.disconnect();
                            if (effectChain) {
    ...
    private scheduleClip(
        ...
        const timeSinceStart = Math.max(0, -timeUntilStart);
        const startOffset = (clip.offset || 0) + (timeSinceStart * speed);
        const duration = timeUntilStart > 0
            ? clip.duration
            : Math.max(0, clip.duration - timeSinceStart);
        ...
        const absStartTime = contextTime + Math.max(0, timeUntilStart);
    ...
    private clearActivePreview(): void {
        if (this.previewSource) {
            try {
                ...
                // Preview source may already be stopped.
            }
            try {
                this.previewSource.disconnect();
            } catch (_e) {
                // Preview source may already be disconnected.
            }
        ...
        if (this.previewGain) {
            try {
                ...
                // Preview gain may already be disconnected.
            }
    ...
    public async renderTimelineOffline(
        ...
        await Promise.all(
            Object.values(clips).map((clip) => {
                const resource = resolveResourceFromRef(resources, clip.resource);
                return resource ? this.loadResource(resource) : Promise.resolve(null);
            })
        );
        ...
        timeline.tracks.forEach(trackRef => {
            ...
            track.clips.forEach((clipRef) => {
                const clip = resolveClipFromRef(clips, clipRef);
                if (!clip) return;
                const res = resolveResourceFromRef(resources, clip.resource);
                if (!res) return;
                const buffer = this.bufferCache.get(resolveEntityKey(res));
