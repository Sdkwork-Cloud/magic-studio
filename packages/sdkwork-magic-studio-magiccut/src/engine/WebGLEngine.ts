...
import { createMagicCutPreviewClip } from '../domain/preview/previewIdentity';
...
export class WebGLEngine {
    ...
    public renderSingleResource(resource: AnyMediaResource, time: number) {
        ...
        const dummyClip = createMagicCutPreviewClip({
            resource,
            time,
            projectWidth: this.projectW,
            projectHeight: this.projectH
        });
        const clipId = dummyClip.uuid;
    ...
    private syncVideoState(
        ...
        if (timeline) {
            timeline.tracks.forEach(tr => {
                const track = tracks[resolveEntityKey(tr)];
                ...
                track.clips.forEach((cr) => {
                    const clip = clips[resolveEntityKey(cr)];
                    const clipKey = clip ? resolveEntityKey(clip) : null;
                    const activationStart = clip
                        ? resolveClipActivationStart({
                            clipStart: clip.start,
                            incomingTransitionLeadIn: incomingTransitionLeadIns.get(clipKey!) ?? 0,
                    ...
                    if (clip &&
                        ...
                        const res = resources[resolveEntityKey(clip.resource)];
                        if (res && res.type === MediaResourceType.VIDEO) {
                            const url = this.resourceManager.resolveResourceUrl(res);
                            if (url) {
                                const video = this.resourceManager.getVideoElement(clipKey!, url);
                                ...
                                this.resourceManager.syncVideoTime(video, resourceTime, shouldPlay, clipKey!, speed);
                            }
                        }
                    }
                });
                    ...
                    if (res && res.type === MediaResourceType.VIDEO) {
                        ...
                        if (url) {
                            const p = new Promise<void>(resolve => {
                                const v = this.resourceManager.getVideoElement(clipKey, url);
