...
describe('buildIncomingTransitionLeadIns', () => {
  it('maps each target clip to the incoming transition duration on its track', () => {
    ...
    const transitionAB = createTransitionLayer(clipA.id, resolveEntityKey(clipB), 1.2);
    const transitionBC = createTransitionLayer(clipB.id, resolveEntityKey(clipC), 0.35);
    ...
    const leadIns = buildIncomingTransitionLeadIns({
      ...
      clips: {
        ...
        [resolveEntityKey(clipC)]: clipC,
      },
      layers: {
        [resolveEntityKey(transitionAB)]: transitionAB,
        [resolveEntityKey(transitionBC)]: transitionBC,
      },
    });

    expect(leadIns).toEqual(
      new Map([
        [resolveEntityKey(clipB), 1.2],
        [resolveEntityKey(clipC), 0.35],
      ])
    );
  });
});
