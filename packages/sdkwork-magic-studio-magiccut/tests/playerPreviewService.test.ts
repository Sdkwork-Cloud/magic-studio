...
describe('PlayerPreviewService', () => {
  ...
  it('clears a matching audio preview by resource id', () => {
    ...
    const resource = createResource(MediaResourceType.MUSIC, 'music-preview');
    const resourceKey = resolveEntityKey(resource);

    service.previewResource(resource, 0);

    expect((service as any).isPreviewingResource?.(resourceKey)).toBe(true);
    expect((service as any).clearPreviewForResource?.('other-resource')).toBe(false);
    expect(audioPreview.stop).not.toHaveBeenCalled();

    expect((service as any).clearPreviewForResource?.(resourceKey)).toBe(true);
    expect(audioPreview.stop).toHaveBeenCalledTimes(1);
    expect((service as any).isPreviewingResource?.(resourceKey)).toBe(false);
  });
  ...
  it('clears a matching visual preview by resource id', () => {
    ...
    service.registerPlayer({ current: player } as any);
    const resource = createResource(MediaResourceType.VIDEO, 'video-preview');
    const resourceKey = resolveEntityKey(resource);

    service.previewResource(resource, 3);
