> Migrated from `docs/plans/2026-03-19-voice-speaker-ui-design.md` on 2026-06-24.
...
- The new `@sdkwork/magic-studio-commons/ui` package exports `Dialog`, `Button`, `Input`, `Select`, `Textarea`, etc., but the editors still need a path alias and package export to resolve it.
...
- Update the TypeScript path mapping and the commons package `exports` so `@sdkwork/magic-studio-commons/ui` is resolvable in both apps and tests.
...
## Testing
- Run `pnpm test -- packages/sdkwork-magic-studio-voicespeaker/src/components/voicespeaker/__tests__/VoiceLabModal.test.tsx packages/sdkwork-magic-studio-voicespeaker/src/components/voicespeaker/__tests__/ChooseVoiceSpeakerModal.test.tsx`.
